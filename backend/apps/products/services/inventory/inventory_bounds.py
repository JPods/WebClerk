...
Computes recommended min (reorder point) and max
...
def recommend_inventory_bounds(
    ...
    *,
    ...
    """Recommend min and max for an item.
    ...
    Formula:
        ...
        min = (lead_time × avg_daily_demand) + safety_stock + pending_buffer
        max = min + economic_order_quantity
    ...
    Args:
        ...
        apply: If True, write results to Item.quantity.min/max
    ...
    # min = reorder point
    min = lead_time_demand + safety_stock + pending_buffer

    # Economic order quantity for max
    ...
    if apply:
        try:
            ...
            if not isinstance(item.quantity, dict):
                item.quantity = {}
            item.quantity['min'] = min
            item.quantity['max'] = max
