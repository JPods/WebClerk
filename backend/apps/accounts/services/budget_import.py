...
Required columns: period, dt_period_start, dt_period_end, account_debit, debit, credit, account_credit
...
REQUIRED_COLUMNS = {'period', 'dt_period_start', 'dt_period_end', 'account_debit', 'debit', 'credit', 'account_credit'}
...
def _parse_json(content: str) -> list[dict]:
    ...
    Supports two JSON shapes:
      Flat: {"account_debit": "6100-SALARY", "debit": 12500, "credit": 12500, "account_credit": "1000-CASH", ...}
      Nested: {"debit": {"account": "6100-SALARY", "amount": 12500}, "credit": {"account": "1000-CASH", "amount": 12500}, ...}
    Both are normalized to flat format for validation.
    """
    ...
    normalized = []
    for row in data:
        if isinstance(row.get('debit'), dict) and isinstance(row.get('credit'), dict):
            normalized.append({
                'period': row.get('period', ''),
                'dt_period_start': row.get('dt_period_start', 0),
                'dt_period_end': row.get('dt_period_end', 0),
                'account_debit': row['debit']['account'],
                'debit': row['debit']['amount'],
                'credit': row['credit']['amount'],
                'account_credit': row['credit']['account'],
...
def _validate_row(row: dict, row_num: int, valid_accounts: set) -> tuple[dict, list[str]]:
    ...
    try:
        ...
        account_debit = str(row['account_debit']).strip()
        debit = Decimal(str(row['debit']).strip())
        credit = Decimal(str(row['credit']).strip())
        account_credit = str(row['account_credit']).strip()
    except (ValueError, InvalidOperation, TypeError) as e:
        errors.append(f"Row {row_num}: invalid value — {e}")
        return row, errors

    # Validate both accounts exist in chart of accounts
    if valid_accounts and account_debit not in valid_accounts:
        errors.append(f"Row {row_num}: account_debit '{account_debit}' not found in chart of accounts")
    if valid_accounts and account_credit and account_credit not in valid_accounts:
        errors.append(f"Row {row_num}: account_credit '{account_credit}' not found in chart of accounts")

    # Validate debit equals credit (balanced entry)
    if debit != credit:
        errors.append(f"Row {row_num}: debit ({debit}) must equal credit ({credit}) — entry must balance")
