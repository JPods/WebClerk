...
QUANTITY_CANONICAL_KEYS = {"on_hand", "allocated", "available", "on_so", "on_po", "on_p", "on_reciept", "on_in", "on_wo", "min", "max", "rate_expected", "rate_actual"}
...
class Item(StatsMixin, BaseModel):
    ...
    def _merge_json_defaults(self):
        ...
        validate_gl_map(self.gls, owner=f'item {self.ida or "(new)"} gls')
    ...
    def _normalize_quantity(self):
        ...
        if oh is not None and alloc is not None and self.quantity.get('available') is None:
            computed = available_from_quantity(self.quantity)
            if computed is not None:
                self.quantity['available'] = computed
