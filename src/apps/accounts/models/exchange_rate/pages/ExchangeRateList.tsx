...
export default function ExchangeRateList() {
  ...
  const handleDatabaseSearch = useCallback(async (terms: string[]) => {
    try {
      ...
      const res = await fetchExchangeRates({ search: searchQuery });
      if (res.status === 200) {
  ...
  const columnCtx = useColumnContextMenu("exchange_rate_list", userColumns);
  return (
    <>
      <ButtonToolbar
        pageTitle="Exchange Rate List"
        title="Exchange Rate"
        modelKey="exchange_rate"
