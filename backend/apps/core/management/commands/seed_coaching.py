...
COACHING = {
    ...
    'ledger': {  # payment coaching stored under ledger (payment not in registry)
        ...
        'tips': [
            {'id': 'pay-01', 'title': 'Two types of payments', 'body': "cash_in = money in (customer pays us, applied to Invoice). cash_out = money out (we pay vendor, applied to Receipt). Same model, different type.", 'type': 'concept', 'level': 'beginner'},
        ...
        'warnings': ['Cash-out payments should link to a receipt via the receipt field.'],
        'code_examples': [
            {'title': 'Record cash-in payment', 'language': 'javascript', 'code': "await saveRecord('payment', {\n  type: 'cash_in', amount: 500,\n  invoice_id: X, contact_id: Y,\n  gateway: 'manual', status: 'completed'\n});"},
        ],
        'api_reference': {
            'list': "GET /wcapi/get/?model_name=payment&type=cash_in&ordering=-dt_payment",
            'save': "POST /wcapi/save/ {model_name: 'payment', type: 'cash_in', amount: 500}",
        },
    },
