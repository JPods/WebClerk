...
Computes two cash options for any transaction:
...
and cash-processing concern, not a line-level concern.
...
See: common/schemas/cash.py — DualPricingConfig, DualPricingProjection
"""
import logging
...
def get_dual_pricing_config() -> dict:
    ...
    from apps.transactions.models.cash_pydantic import DualPricingConfig
...
def is_exempt_method(method: str) -> bool:
    """Check if a cash method is exempt from surcharging (gets cash price).
...
def compute_cash_amount(
    transaction_totals: dict,
    cash_method: str,
) -> dict:
    """Determine the actual cash amount based on method.
    ...
    if is_exempt_method(cash_method):
        return {
            'amount': projection['cash_total'],
            'is_cash_price': True,
            'surcharge': 0.0,
