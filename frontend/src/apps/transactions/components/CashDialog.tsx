...
/**
 * CashDialog — Full-featured cash creation/edit dialog.
 *
 * Enhanced version of AddCashModal with fields from the legacy wc2
 * diaMakePay dialog:
 *   • Cash type selection (auto-detected from card number)
 *   • Total / Cash / Difference auto-calculation
 ...
 *
 * Can be used standalone or to replace AddCashModal where a richer
 * dialog is needed.
 */
...
const CashDialog: React.FC<CashDialogProps> = ({
  ...
  // Cash mode: 'manual' (check/wire/ACH) or 'card' (Spreedly hosted fields)
  const [cashMode, setCashMode] = useState<'manual' | 'card'>('manual');

  // Options
  const [cashMethods, setCashMethods] = useState<CashMethodOption[]>([]);
  ...
  const handleSave = useCallback(async () => {
    ...
    if (!contact_id) {
      dispatch(showToast({ message: 'Contact is required to record a cash entry', type: 'error' }));
      return;
    }
    ...
    try {
      ...
      if (invoice_id) {
        cashData.invoice_id = invoice_id;
      }

      const response = await apiClient.post('/wcapi/cash/', cashData);

      if (response.data) {
        const displayAmount = isCredit ? -parsedAmount : parsedAmount;
        dispatch(
          showToast({
            message: `Cash of ${formatCurrency(Math.abs(displayAmount))} recorded`,
            type: 'success',
          }),
        );
        onCashAdded?.();
    ...
    amount, isCredit, contact_id, cashDate, cashMethodId,
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700 shrink-0">
          <h2 className="text-lg font-semibold text-slate-900 dark:text-white flex items-center gap-2">
            ...
            {isCredit ? 'Credit Cash' : 'Make Cash'}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-4 space-y-4 overflow-y-auto flex-1">
          ...
          {/* Card cash via Spreedly hosted fields */}
          {cashMode === 'card' && invoice_id ? (
            <SpreedlyCardForm
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={labelClass}>Cash Method</label>
              <select data-wc="select"
                value={cashMethodId ?? ''}
                onChange={(e) =>
                  setCashMethodId(e.target.value ? parseInt(e.target.value) : null)
                }
            ...
            <div>
              <label className={labelClass}>Cash Date</label>
              <input
                type="date"
                value={cashDate}
          ...
          <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-200 dark:border-slate-700 shrink-0">
            ...
            >
              {saving ? (
                ...
                <>
                  <FaCheck size={14} />
                  {isCredit ? 'Apply Credit' : 'Add Cash'}
                </>
              )}
            </button>
          </div>
...
export default withDevIdentifier(CashDialog, 'CashDialog', 'rose', 'apps/transactions/components/CashDialog.tsx');
