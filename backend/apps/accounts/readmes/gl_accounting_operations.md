...
GlJournal
  ...
  ├── source_id            → FK to originating record (invoice, cash, etc.)
  ├── source_model         → 'invoice' | 'cash' | 'purchase' | 'bom_build' | 'adjustment'
...
```
Ledger (one per invoice installment period + one per cash)
  ├── value_original       → immutable initial amount
  ├── value_available      → current unpaid balance (decreases with cash_entries)
  ├── dt_due               → when cash is owed — drives aging
...
Invoice ledgers: positive. Cash ledgers: negative. Sum = current balance.
...
| `apps/accounts/services/terms_ledger.py` | Cash schedule → ledger records |
