...
const CommonTab: React.FC<{ data?: OrgFinancialCommon; currency: string }> = ({ data }) => {
...
const CustomerTab: React.FC<{ data?: OrgFinancialCustomer; currency: string }> = ({ data }) => {
  ...
  return (
    <div className="space-y-2">
      ...
      <Section title="Cash History" icon={<FaMoneyCheckAlt size={12} />} defaultOpen={false}>
        <LineItem label="Avg Days to Pay" value={`${data.cash?.days_avg_paid ?? '--'} days`} />
        <LineItem label="Terms (Days)" value={`${data.cash?.days_pay ?? '--'} days`} />
        <LineItem label="Last Cash" value={formatDate(data.cash?.dt_last_cash)} />
        <LineItem label="Last Amount" value={formatCurrency(data.cash?.last_cash_amount)} />
      </Section>
...
const VendorTab: React.FC<{ data?: OrgFinancialVendor; currency: string }> = ({ data }) => {
  ...
  return (
    <div className="space-y-2">
      ...
      <Section title="Cash Made" icon={<FaMoneyCheckAlt size={12} />} defaultOpen={false}>
        <LineItem label="Paid MTD" value={formatCurrency(data.cash_entries_made?.mtd)} />
        <LineItem label="Paid YTD" value={formatCurrency(data.cash_entries_made?.ytd)} />
        <LineItem label="Last Cash" value={formatDate(data.cash_entries_made?.dt_last_cash)} />
      </Section>
...
const RepTab: React.FC<{ data?: OrgFinancialRep; currency: string }> = ({ data }) => {
...
const EmployeeTab: React.FC<{ data?: OrgFinancialEmployee; currency: string }> = ({ data }) => {
...
const ManufacturerTab: React.FC<{ data?: OrgFinancialManufacturer; currency: string }> = ({ data }) => {
