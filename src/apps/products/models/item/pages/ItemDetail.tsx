...
function DataFieldGrid({ children, columns = 2 }: DataFieldGridProps) {
  ...
  return <dl className={`grid gap-x-6 gap-y-1 ${gridClass}`}>{children}</dl>;
}
...
function ImagePanel({
  ...
  const handleAddToGallery = async () => {
    ...
    if (await saveImages(newImages)) {
      ...
      dispatch(
        showToast({ message: "Image added to gallery", type: "success" }),
      );
    }
  };
  ...
  const displayPrimary =
    primaryUrl || (galleryUrls.length > 0 ? galleryUrls[0] : "");
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="mt-3 space-y-1 text-xs font-mono bg-slate-50 dark:bg-slate-800/50 rounded p-2">
        <div className="flex items-center gap-2">
          <span className="text-slate-400 dark:text-slate-500 w-16">
            primary:
          </span>
          <span
            className="text-slate-600 dark:text-slate-300 truncate flex-1"
            title={primaryUrl || "(none)"}
          >
            {primaryUrl || (
        ...
        <div className="flex items-center gap-2">
          ...
          <span
            className="text-slate-600 dark:text-slate-300 truncate flex-1"
            title={galleryUrls.join(", ") || "(empty)"}
          >
            {galleryUrls.length > 0 ? (
              `[${galleryUrls.length}] ${galleryUrls[0]}${
...
function ItemDataView({ data, isAdmin, onDataChange }: ItemDataViewProps) {
  ...
  const getPrice = (
    price: PriceData | number | undefined,
  ): number | undefined => {
  ...
  return (
    <div className="space-y-4">
            ...
            {typeof data.price === "object" && data.price ? (
              <>
                <DataFieldGrid columns={4}>
                  <DataField
                    label=".base"
                    value={(data.price as PriceData).base}
                    highlight
                    isCurrency
                  />
                  <DataField
                    label=".msrp"
                    value={(data.price as PriceData).msrp}
                    isCurrency
                  />
                  <DataField
                    label=".retail"
                    value={(data.price as PriceData).retail}
                    isCurrency
                  />
                  <DataField
                    label=".wholesale"
                    value={(data.price as PriceData).wholesale}
                    isCurrency
                  />
                </DataFieldGrid>
                <DataFieldGrid columns={4}>
                  <DataField
                    label=".distributor"
                    value={(data.price as PriceData).distributor}
                    isCurrency
                  />
                  <DataField
                    label=".sample"
                    value={(data.price as PriceData).sample}
                    isCurrency
                  />
                  <DataField
                    label=".currency"
                    value={(data.price as PriceData).currency || "USD"}
                  />
                  <DataField
                    label=".history"
                    value={(data.price as PriceData).history?.length ?? 0}
                  />
                </DataFieldGrid>
                {/* Tiers */}
                {(data.price as PriceData).tiers &&
                  (data.price as PriceData).tiers!.length > 0 && (
                    <div className="mt-2">
                ...
                {/* Qty Breaks */}
                {(data.price as PriceData).qty_breaks &&
                  (data.price as PriceData).qty_breaks!.length > 0 && (
                    <div className="mt-2">
                      <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">
                        Qty Breaks
                      </p>
                      <div className="overflow-x-auto">
                        <table className="min-w-full text-sm">
                          <thead>
                            <tr className="border-b border-gray-200 dark:border-gray-700">
                              <th className="px-2 py-1 text-left text-xs font-semibold text-gray-500">
                                Min Qty
                              </th>
                              <th className="px-2 py-1 text-left text-xs font-semibold text-gray-500">
                                Unit Price
                          ...
                          <tbody>
                            {(data.price as PriceData).qty_breaks!.map(
                              (b, i) => (
                                <tr
                                  key={i}
          ...
          {data.cost && typeof data.cost === "object" && (
            ...
            >
              <DataFieldGrid columns={3}>
                <DataField
                  label=".standard"
                  value={(data.cost as CostData).standard}
                  isCurrency
                />
                <DataField
                  label=".last"
                  value={(data.cost as CostData).last}
                  isCurrency
                />
                <DataField
                  label=".avg"
                  value={(data.cost as CostData).avg}
                  isCurrency
                />
              </DataFieldGrid>
              <DataFieldGrid columns={2}>
                <DataField
                  label=".landed"
                  value={(data.cost as CostData).landed}
                  isCurrency
                />
                <DataField
                  label=".currency"
                  value={(data.cost as CostData).currency || "USD"}
                />
              </DataFieldGrid>
              {/* Components */}
              {(data.cost as CostData).components &&
                Object.keys((data.cost as CostData).components!).length > 0 && (
                  <div className="mt-2">
                    <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">
                      Components
                    </p>
                    <div className="bg-gray-50 dark:bg-slate-800 rounded p-2 text-sm">
                      {Object.entries((data.cost as CostData).components!).map(
                        ([k, v]) => (
              ...
              {/* Qty Breaks */}
              {(data.cost as CostData).qty_breaks &&
                (data.cost as CostData).qty_breaks!.length > 0 && (
                  <div className="mt-2">
                    <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">
                      Cost Breaks
                    </p>
      ...
      {data.id && <BOMSection itemId={data.id} defaultOpen={false} />}
      ...
      {data.tax_code && (
        ...
        >
          <DataFieldGrid columns={3}>
            <DataField
              label="code"
              value={(data.tax_code as TaxCodeData).code}
            />
            <DataField
              label="jurisdiction"
              value={(data.tax_code as TaxCodeData).jurisdiction}
            />
            <DataField
              label="category"
              value={(data.tax_code as TaxCodeData).category}
            />
          </DataFieldGrid>
          <DataFieldGrid columns={2}>
            <DataField
              label="rate"
              value={(data.tax_code as TaxCodeData).rate}
            />
            <DataField
              label="exemptions"
              value={(data.tax_code as TaxCodeData).exemptions?.join(", ")}
            />
          </DataFieldGrid>
          {/* Jurisdiction Params */}
          {(data.tax_code as TaxCodeData).jurisdiction_params &&
            (data.tax_code as TaxCodeData).jurisdiction_params!.length > 0 && (
              <div className="mt-2">
                <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">
                  Jurisdiction Params
                </p>
                <div className="overflow-x-auto">
                  <table className="min-w-full text-sm">
                    ...
                    <tbody>
                      {(data.tax_code as TaxCodeData).jurisdiction_params!.map(
                        (jp, i) => (
                          <tr
                            key={i}
                            className="border-b border-gray-100 dark:border-gray-800"
      ...
      {data.catalog && (
        ...
        >
          {/* Categories */}
          {(data.catalog as CatalogData).categories &&
            (data.catalog as CatalogData).categories!.length > 0 && (
              <DataFieldGrid columns={2}>
                <div className="sm:col-span-2">
                  <DataField
                    label="categories"
                    value={(data.catalog as CatalogData).categories!.join(
                      " > ",
                    )}
                  />
                </div>
          ...
          {(data.catalog as CatalogData).web && (
            <DataFieldGrid columns={4}>
              <DataField
                label="web.slug"
                value={(data.catalog as CatalogData).web?.slug}
              />
              <DataField
                label="web.title"
                value={(data.catalog as CatalogData).web?.title}
              />
              <DataField
                label="web.short"
                value={(data.catalog as CatalogData).web?.short}
              />
              <DataField
                label="web.seo"
                value={
                  (data.catalog as CatalogData).web?.seo
                    ? JSON.stringify((data.catalog as CatalogData).web!.seo)
                    : null
                }
              />
            </DataFieldGrid>
          ...
          {/* Attributes */}
          {(data.catalog as CatalogData).attributes &&
            Object.keys((data.catalog as CatalogData).attributes!).length >
              0 && (
              <div className="mt-2">
                <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">
                  Attributes
                ...
                <dl className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-1 text-sm">
                  {Object.entries(
                    ...
                    <div key={k} className="flex items-baseline gap-1">
                      <dt className="text-xs font-mono text-gray-500 dark:text-gray-400">
                        {k}:
                      </dt>
          ...
          {/* Catalog Flags */}
          {(data.catalog as CatalogData).flags &&
            Object.keys((data.catalog as CatalogData).flags!).length > 0 && (
              <div className="mt-2">
                <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">
                  Flags
                </p>
                <div className="flex flex-wrap gap-2">
                  {Object.entries((data.catalog as CatalogData).flags!).map(
                    ([k, v]) => (
                      <Badge key={k} color={v ? "success" : "light"}>
                        {k}
                      </Badge>
                    ),
                  )}
...
function dataToFormValues(d: ItemData | null): ItemFormValues {
  if (!d) {
    return {
      ...
      qty_allocated: undefined,
      qty_available: undefined,
      qty_on_so: undefined,
      qty_on_po: undefined,
      qty_on_p: undefined,
      qty_on_reciept: undefined,
      qty_on_in: undefined,
      qty_on_wo: undefined,
      flag_back_order_allowed: false,
      flag_discountable: false,
      flag_linked: false,
      flag_not_tracked: false,
      flag_pacing: false,
      flag_print_suppressed: false,
      flag_serialized: false,
      flag_tally_by_type: false,
      gls_inventory: "",
      gls_cogs: "",
      gls_revenue: "",
      gls_variance: "",
  ...
  return {
    ...
    cost_standard: c.standard,
    cost_last: c.last,
    cost_avg: c.avg,
    cost_landed: c.landed,
    cost_currency: c.currency || "USD",
    qty_on_hand: q.on_hand,
    qty_allocated: q.allocated,
    qty_available: q.available,
    qty_on_so: q.on_so,
    qty_on_po: q.on_po,
    qty_on_p: q.on_p,
    qty_on_reciept: q.on_reciept,
    qty_on_in: q.on_in,
    qty_on_wo: q.on_wo,
    ...
    gls_inventory: g.inventory || "",
    gls_cogs: g.cogs || "",
    gls_revenue: g.revenue || "",
    gls_variance: g.variance || "",
    tax_code_code: t.code || "",
    tax_code_jurisdiction: t.jurisdiction || "",
    tax_code_category: t.category || "",
    tax_code_rate: t.rate,
...
function formValuesToPayload(
  ...
  return {
    ...
    kind: fd.kind,
    uom: fd.uom,
    base_uom: fd.base_uom,
    description: fd.description,
    specification_id: fd.specification_id,
    ...
    cost: {
      ...(orig?.cost || {}),
      standard: fd.cost_standard,
      last: fd.cost_last,
      avg: fd.cost_avg,
      landed: fd.cost_landed,
      currency: fd.cost_currency,
    },
    quantity: {
      ...(orig?.quantity || {}),
      on_hand: fd.qty_on_hand,
      allocated: fd.qty_allocated,
      available: fd.qty_available,
      on_so: fd.qty_on_so,
      on_po: fd.qty_on_po,
      on_p: fd.qty_on_p,
      on_reciept: fd.qty_on_reciept,
      on_in: fd.qty_on_in,
      on_wo: fd.qty_on_wo,
    },
    flags: {
      ...(orig?.flags || {}),
      back_order_allowed: fd.flag_back_order_allowed,
      discountable: fd.flag_discountable,
      linked: fd.flag_linked,
      not_tracked: fd.flag_not_tracked,
      pacing: fd.flag_pacing,
      print_suppressed: fd.flag_print_suppressed,
      serialized: fd.flag_serialized,
      tally_by_type: fd.flag_tally_by_type,
    },
    gls: {
      ...(orig?.gls || {}),
      inventory: fd.gls_inventory,
      cogs: fd.gls_cogs,
      revenue: fd.gls_revenue,
      variance: fd.gls_variance,
    },
    tax_code: {
      ...(orig?.tax_code || {}),
      code: fd.tax_code_code,
      jurisdiction: fd.tax_code_jurisdiction,
      category: fd.tax_code_category,
      rate: fd.tax_code_rate,
    },
    catalog: {
      ...(orig?.catalog || {}),
      categories: fd.catalog_categories
        ? fd.catalog_categories
            .split(",")
            .map((s: string) => s.trim())
            .filter(Boolean)
      ...
      web: {
        ...
        slug: fd.catalog_web_slug,
        title: fd.catalog_web_title,
        short: fd.catalog_web_short,
      },
    },
  };
}
...
function ItemDetail({
    ...
    if (itemIdFromUrl && itemIdFromUrl !== fetchedData?.id) {
      ...
      getRecord("item", itemIdFromUrl)
        ...
        .catch((err) => {
          ...
          dispatch(
            showToast({ message: "Failed to load item", type: "error" }),
          );
        })
  ...
  useEffect(() => {
    ...
    const normalizedItem = {
      ...
      // Catalog fields
      catalog_categories: (data.catalog as CatalogData)?.categories
        ? (data.catalog as CatalogData).categories!.join(", ")
        : "",
      ...
      tax_code_exemptions: taxObj.exemptions
        ? taxObj.exemptions.join(", ")
        : "",
      tax_code_jurisdiction_params_json: JSON.stringify(
        taxObj.jurisdiction_params || [],
      ),
    };
  ...
  const onSubmit = async (formData: any) => {
    try {
      ...
      const priceObj: any = {};
      if (
        formData.price_base !== undefined &&
        formData.price_base !== null &&
        formData.price_base !== ""
      )
        priceObj.base = Number(formData.price_base);
      if (
        formData.price_msrp !== undefined &&
        formData.price_msrp !== null &&
        formData.price_msrp !== ""
      )
        priceObj.msrp = Number(formData.price_msrp);
      if (
        formData.price_retail !== undefined &&
        formData.price_retail !== null &&
        formData.price_retail !== ""
      )
        priceObj.retail = Number(formData.price_retail);
      if (
        formData.price_wholesale !== undefined &&
        formData.price_wholesale !== null &&
        formData.price_wholesale !== ""
      )
        priceObj.wholesale = Number(formData.price_wholesale);
      if (
        formData.price_distributor !== undefined &&
        formData.price_distributor !== null &&
        formData.price_distributor !== ""
      )
        priceObj.distributor = Number(formData.price_distributor);
      if (
        formData.price_sample !== undefined &&
        formData.price_sample !== null &&
        formData.price_sample !== ""
      )
        priceObj.sample = Number(formData.price_sample);
      ...
      const costObj: any = {};
      if (
        formData.cost_standard !== undefined &&
        formData.cost_standard !== null &&
        formData.cost_standard !== ""
      )
        costObj.standard = Number(formData.cost_standard);
      if (
        formData.cost_last !== undefined &&
        formData.cost_last !== null &&
        formData.cost_last !== ""
      )
        costObj.last = Number(formData.cost_last);
      if (
        formData.cost_avg !== undefined &&
        formData.cost_avg !== null &&
        formData.cost_avg !== ""
      )
        costObj.avg = Number(formData.cost_avg);
      if (
        formData.cost_landed !== undefined &&
        formData.cost_landed !== null &&
        formData.cost_landed !== ""
      )
        costObj.landed = Number(formData.cost_landed);
      ...
      if (formData.tax_code_jurisdiction)
        taxCodeObj.jurisdiction = formData.tax_code_jurisdiction;
      if (formData.tax_code_category)
        taxCodeObj.category = formData.tax_code_category;
      if (
        formData.tax_code_rate !== undefined &&
        formData.tax_code_rate !== null &&
        formData.tax_code_rate !== ""
      )
        taxCodeObj.rate = Number(formData.tax_code_rate);
      if (formData.tax_code_exemptions) {
        taxCodeObj.exemptions = String(formData.tax_code_exemptions)
          .split(",")
          .map((s: string) => s.trim())
          .filter(Boolean);
      }
            ...
            <div className="mt-4">
              {activeTab === "actions" && (
                <ActionsPanel
                  entityType="item"
                  entityId={data.id}
                  data={data?.actions?.items || data?.actions}
                  isEditing={false}
                />
              )}
              {activeTab === "comments" && (
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {shouldRenderField("uom") && (
              <FieldRow label="UOM" htmlFor="uom" hint="Unit of measure">
                <Input
                  type="text"
                  id="uom"
                  placeholder="ea, lb, kg …"
                  {...register("uom")}
                  disabled={isFieldDisabled("uom")}
                />
              </FieldRow>
            )}
            {shouldRenderField("base_uom") && (
              <FieldRow label="Base UOM" htmlFor="base_uom">
                <Input
                  ...
                  {...register("base_uom")}
                  disabled={isFieldDisabled("base_uom")}
                />
              </FieldRow>
            )}
            ...
            {shouldRenderField("specification_id") && (
              <FieldRow label="Spec ID" htmlFor="specification_id">
                <Input
                  ...
                  disabled={isFieldDisabled("specification_id")}
                />
              </FieldRow>
            )}
          ...
          >
            ...
            <FieldRow label="Jurisdiction" htmlFor="tax_code_jurisdiction">
              <Input
                type="text"
                id="tax_code_jurisdiction"
                {...register("tax_code_jurisdiction")}
                disabled={isFieldDisabled("tax_code")}
              />
            </FieldRow>

            <FieldRow label="Tax Code Category" htmlFor="tax_code_category">
              <Input
                type="text"
                id="tax_code_category"
                {...register("tax_code_category")}
                disabled={isFieldDisabled("tax_code")}
              />
            </FieldRow>
            ...
            <FieldRow label="Rate" htmlFor="tax_code_rate">
              <Input
                ...
                id="tax_code_rate"
                step="0.01"
                {...register("tax_code_rate", { valueAsNumber: true })}
                disabled={isFieldDisabled("tax_code")}
              />
            </FieldRow>
            ...
            <FieldRow
              label="Exemptions"
              htmlFor="tax_code_exemptions"
              hint="Comma-separated exemptions"
            >
              <Input
                type="text"
                id="tax_code_exemptions"
                {...register("tax_code_exemptions")}
                disabled={isFieldDisabled("tax_code")}
              />
            </FieldRow>

            <FieldRow
              label="Jurisdiction Params (JSON)"
              htmlFor="tax_code_jurisdiction_params_json"
              hint="JSON array of params"
            >
              <textarea
          ...
          >
            <FieldRow
              label="Categories"
              htmlFor="catalog_categories"
              hint="Comma-separated categories"
            >
              <Input
                type="text"
                id="catalog_categories"
                {...register("catalog_categories")}
            ...
            <FieldRow
              ...
              htmlFor="catalog_attributes_json"
              hint='JSON object: {"color":"red"}'
            >
              <textarea
            ...
            <FieldRow label="Web Title" htmlFor="catalog_web_title">
              <Input
                type="text"
                id="catalog_web_title"
                {...register("catalog_web_title")}
                disabled={isFieldDisabled("catalog")}
              />
            </FieldRow>
            ...
            <FieldRow label="Web Short" htmlFor="catalog_web_short">
              <Input
                type="text"
                id="catalog_web_short"
                {...register("catalog_web_short")}
                disabled={isFieldDisabled("catalog")}
              />
            </FieldRow>
            ...
            <FieldRow
              label="Flags (JSON)"
              htmlFor="catalog_flags_json"
              hint='JSON object of flags e.g. {"featured":true}'
            >
              <textarea
            ...
            {shouldRenderField("cost") && (
              <>
                ...
                <FieldRow
                  ...
                  htmlFor="cost_components_json"
                  hint='JSON object: {"partA":1.23}'
                >
                  <textarea
                ...
                <FieldRow
                  label="Cost Breaks (JSON)"
                  htmlFor="cost_qty_breaks_json"
                  hint='JSON array: [{"min_qty":1,"unit_cost":1.23}]'
                >
                  <textarea
          ...
          >
            {shouldRenderField("price") && (
              <>
                ...
                <FieldRow
                  label="Qty Breaks (JSON)"
                  htmlFor="price_qty_breaks_json"
                  hint='JSON array: [{"min_qty":1,"unit_price":9.99}]'
                >
                  <textarea
                ...
                <FieldRow
                  label="History (JSON)"
                  htmlFor="price_history_json"
                  hint="JSON array of history objects"
                >
                  <textarea
