...
def _handle_transaction(msg: dict, context: dict) -> bool:
    ...
    if model == "invoice" and balance < 0:
        _create_observation(
            ...
            detail="Negative balance indicates overpayment. Check cash applications.",
            dedup_key=f"neg_balance_invoice_{record_id}",
            priority=1,
        )
        return True

    # Flag: cash without matching invoice context
    if model == "cash" and event == "created":
        if not context.get("invoice_id"):
            # Log to alice_log for pattern tracking, but don't observe every cash entry
            _log_to_alice("observe", "cash", f"Cash #{record_id} created, status={status}")
