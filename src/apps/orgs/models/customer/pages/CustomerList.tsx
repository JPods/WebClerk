...
export default function CustomerList() {
  ...
  const [selectedCustomer, setSelectedCustomer] = useState<dynamicData | null>(
    null
  );
  ...
  const getLocationData = useCallback(async (customerId?: number) => {
    setLoading(true);
    try {
      ...
      if (customerId) {
        const contactRes = await getRecord("customer", customerId);
        setSelectedCustomer(contactRes.record);
