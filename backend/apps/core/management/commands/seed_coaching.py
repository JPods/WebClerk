...
COACHING = {
    ...
    'invoice': {
        'tips': [
            ...
            {'id': 'inv-02', 'title': 'Balance tracks cash', 'body': 'Balance = Total - Cash. When it reaches 0, the invoice is fully paid.', 'type': 'concept', 'level': 'beginner'},
        ...
        'field_help': {
            ...
            'balance': 'Total minus cash received. Read-only. Updated when cash are applied.',
            'status': 'planned = draft. released = sent to customer. complete = paid and closed.',
            'terms': 'Cash terms — determines due date on the ledger record.',
        ...
        'actions': {
            ...
            'cash': 'Go to Cash → Apply Cash. Select this invoice, enter amount.',
    ...
    'purchase': {
        'tips': [
            ...
            {'id': 'po-02', 'title': 'PO → Receiving → Cash', 'body': 'Receive goods against the PO, then create a disbursement (cash type=disbursed) to pay the vendor.', 'type': 'workflow', 'level': 'beginner'},
        ],
        'field_help': {
            ...
            'terms': 'Vendor cash terms — when you pay them.',
            'totals': 'Subtotal, tax, shipping, total. Computed from lines.',
    ...
    'ledger': {  # cash coaching stored under ledger (cash not in registry)

        'tips': [
            {'id': 'pay-01', 'title': 'Two types of cash_entries', 'body': "cash_in = money in (customer pays us, applied to Invoice). cash_out = money out (we pay vendor, applied to Receipt). Same model, different type.", 'type': 'concept', 'level': 'beginner'},
            {'id': 'pay-02', 'title': 'Cash reduces balance', 'body': 'Applying cash to an invoice reduces its balance. When balance = 0, invoice is paid.', 'type': 'concept', 'level': 'beginner'},
        ],
        'field_help': {
            'type': 'cash_in = AR (customer pays us → Invoice). cash_out = AP (we pay vendor → Receipt).',
        ...
        'actions': {
            'apply': 'Go to Apply Cash to match cash to an invoice.',
            'reconcile': 'After bank statement arrives, mark cash as reconciled.',
        },
        'warnings': ['Cash-out cash should link to a receipt via the receipt field.'],
        'code_examples': [
            {'title': 'Record cash-in cash', 'language': 'javascript', 'code': "await saveRecord('cash', {\n  type: 'cash_in', amount: 500,\n  invoice_id: X, contact_id: Y,\n  gateway: 'manual', status: 'completed'\n});"},
        ],
        'api_reference': {
    ...
    'gl_journal': {
        'tips': [
            {'id': 'gl-01', 'title': 'Journals record financial events', 'body': 'Every invoice, cash, and purchase creates GL journal entries. Debit always equals credit.', 'type': 'concept', 'level': 'beginner'},
        ...
        'field_help': {
            ...
            'type': 'Source type — sales, purchase, cash, adjustment.',
            'source_model': 'Which record created this entry (invoice, cash, purchase).',
...
PAGE_HELP = {
    ...
    'FINANCIALS': 'Financial summary — GL posting status, journal entries, cash history.',
    ...
    'btn-related': 'Show records linked to this one — orders, invoices, cash, actions.',
    # List toolbar
...
## Step 5: Apply Cash
When the customer pays, go to Cash → Apply Cash. Select the invoice and enter the amount.
...
''',
    ...
    },
    {
        ...
        'description': 'How cash works: apply to invoices, early-payment discounts, write-off differences, unapplied cash, AR aging.',
        'body': '''# Cash Application

## The One Rule
Every dollar gets its own Cash record. Cash, discounts, and write-offs are all Cash records with their own GL posting and audit trail.

## Three Cash Types

| Type | When | Example |
|------|------|---------|
| **Cash/check/card** | Normal cash | Customer pays $980 |
...
Invoice has terms "2/10 Net 30". Customer pays within 10 days. System auto-creates a discount Cash for 2% of the invoice total. Two Cash records close the invoice: cash + discount.

## Write-Off Difference
Check the "Write off difference" box when applying cash. Any remaining balance becomes a write-off Cash posted to the write-off GL account.

## Unapplied Cash
Cash with no invoice number → sits as unapplied. Reduces customer balance for credit check. Does NOT clear past-due invoices. Apply to specific invoices later.
