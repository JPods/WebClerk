...
import { createVariant, updateVariant } from "../services/variantApi";
import { showToast } from "../../../../../store/slices/toastSlice";
import { useDispatch } from "react-redux";
import { useLocation, useNavigate } from "react-router";
import { variantSchema } from "../utils/variantSchema";
import { VariantAddProps } from "../types/variantType";
import { Palette, Package, FileText, Settings2, CheckSquare, MessageSquare, FileIcon, History, Link, Code } from "lucide-react";
...
const STORAGE_KEY = "variantDetail_columnCount";
...
function VariantDetail({
  ...
  // Tab navigation
  const { activeTab, setActiveTab } = useDetailTabs("variant_detail", "actions", [
  ...
  const onSubmit = async (formData: z.infer<typeof variantSchema>) => {
    setIsSaving(true);
    try {
      const res =
        currentMode === "add"
          ? await createVariant(formData)
          : await updateVariant({ ...formData, id: data && data.id });
      if (res) {
        dispatch(
          showToast({
            message: `Variant ${
  ...
  return (
    <>
      ...
      <SimpleDetailHeader
        ...
        mode={currentMode}
        backUrl={inline ? undefined : "/products/variants"}
      />
        ...
        <div className="mt-4 space-y-0">
          <ScalarCard
            ...
            fields={[
              ...
              { label: "attributes", value: data.attributes },
...
export default withDevIdentifier(VariantDetail, 'VariantDetail');
