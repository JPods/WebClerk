...
Actions are the work log. Each action has `refs.links` pointing to the transactions it touched — orders, invoices, purchases, cash, customers. The dashboard pulls those links and denormalizes the essential numbers (totals, balances, status) into summary cards. Clicking a card opens that transaction in a new window.
...
For each action: read action.refs.links
    ...
    refs.links.cash:  [{ id, ida, amount, status }]
...
│                                                         │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐   │
│  │ ORDERS   │ │ INVOICES │ │PURCHASES │ │ CASH_ENTRIES │   │
│  │    3     │ │    2     │ │    1     │ │    4     │   │
...
│  │ Process cash #4421      cash $300  applied   ││
│  └─────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────┘
```
