...
export default function OrgPage({ orgType }: OrgPageProps) {
  ...
  // Columns from config
  const allColumns = useMemo(() => config ? buildColumns(config, (row) => {
    setRecord(row);
    setFormMode('edit');
    setEditData({ ...row });
    setDetailKey((k) => k + 1);
  ...
  }, [orgType, editData, config, dispatch, fetchData]);
  ...
  }, [orgType, config, record, dispatch, fetchData]);
  ...
  }, [selected, orgType, config, dispatch, fetchData]);
