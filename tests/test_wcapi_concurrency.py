...
class WcapiConcurrencyTests(TestCase):
    ...
    def test_update_with_matching_version_succeeds_and_bumps(self):
        ...
        resp = self.save({'table_name':'contacts','id':self.contact.id,'expected_version':v,'name_first':'Updated'})
    ...
    def test_update_with_stale_version_conflicts(self):
        ...
        ok = self.save({'table_name':'contacts','id':self.contact.id,'expected_version':v,'name_last':'One'})
        self.assertEqual(ok.status_code, 200)
        new_v = ok.json()['version']
        self.assertEqual(new_v, v+1)
        # attempt with stale version v
        conflict = self.save({'table_name':'contacts','id':self.contact.id,'expected_version':v,'name_last':'Two'})
