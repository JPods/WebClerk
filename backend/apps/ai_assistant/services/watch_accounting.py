"""
Alice Accounting Watchdog — Detect GL imbalances and cash anomalies.

Scans for:
1. GL journal batches where debits != credits
2. Invoices with balance != (total - cash_entries applied)
3. Cash not applied to any invoice
...
def run_accounting_watchdog() -> dict:
    ...
    results['unapplied_cash_entries'] = check_unapplied_cash_entries()
...
def check_invoice_balances() -> dict:
    """Check that invoice.balance == invoice.total - sum(cash_entries applied)."""
    ...
    for inv in invoices[:500]:  # batch limit
        ...
        # Sum cash applied to this invoice
        pay_filter = Q(invoice_id=inv.id)
        cash_entries_applied = (
            Cash.objects.filter(pay_filter)
            .aggregate(total_paid=Sum('amount'))['total_paid'] or 0
        )
        cash_entries_applied = Decimal(str(cash_entries_applied))

        expected_balance = total - cash_entries_applied
        ...
        if diff > Decimal('0.01'):
            ...
            _create_observation(
                ...
                detail=f"Total: ${total:.2f}, Cash applied: ${cash_entries_applied:.2f}. "
...
def check_unapplied_cash_entries() -> dict:
    """Find cash that aren't applied to any invoice."""
    from apps.transactions.models import Cash

    observations = 0

    # Cash with status=completed but no invoice link
    completed_cash_entries = Cash.objects.filter(
        status='completed',
    ).only('id', 'ida', 'amount', 'status', 'invoice_id', 'refs')

    unapplied_count = 0
    for cash in completed_cash_entries[:200]:
        # Check FK first, then refs.links
        has_invoice = bool(cash.invoice_id)
        if not has_invoice:
        ...
        if not has_invoice:
            ...
            _create_observation(
                ...
                model_name='cash',
                record_id=cash.id,
                message=f"Cash #{cash.id} ({cash.ida}) completed but not applied to any invoice",
                detail=f"Status: {cash.status}, Amount: {cash.amount}. "
                       f"This cash may need to be applied to an invoice.",
                dedup_key=f"unapplied_cash_{cash.id}",
                priority=1,
            )
            observations += 1
