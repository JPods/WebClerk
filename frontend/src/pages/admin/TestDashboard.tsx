    ...
    try {
      ...
      for (const l of (invLines?.results || [])) {
        ...
        if (qty) {
          ...
          let parentIda = '';
          try { const p = await getRecord('invoice', parentId) as any; parentIda = p?.record?.ida || `#${parentId}`; } catch { /* no-op */ }
      ...
      for (const l of (ordLines?.results || [])) {
        ...
        if (qty) {
          ...
          let parentIda = '';
          try { const p = await getRecord('order', parentId) as any; parentIda = p?.record?.ida || `#${parentId}`; } catch { /* no-op */ }
      ...
      for (const l of (poLines?.results || [])) {
        ...
        if (qty) {
          ...
          let parentIda = '';
          try { const p = await getRecord('purchase', parentId) as any; parentIda = p?.record?.ida || `#${parentId}`; } catch { /* no-op */ }
