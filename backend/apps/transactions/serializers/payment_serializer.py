...
from apps.transactions.models import Cash, PaymentApplication, Invoice
from apps.accounts.models import Term
from apps.core.models import Contact


class CashSerializer(serializers.ModelSerializer):
    """Serializer for Cash (formerly Payment) transactions.

    Full CRUD with FK resolution for invoice, contact, payment_term (accounts.Term).
    ...
    class Meta:
        ...
        fields = [
            ...
            "dt_payment",
            "method", "payment_term_id", "reference_number",
    ...
    def create(self, validated_data):
        ...
        if payment_term_id:
            try:
                term = Term.objects.get(pk=payment_term_id)
                validated_data["payment_term"] = term
            except Term.DoesNotExist:
                raise serializers.ValidationError({"payment_term_id": "Invalid term id"})
...
class PaymentApplicationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'payment', 'invoice', 'amount', 'applied_at',
            'dt_created', 'dt_modified', 'version'
        ]
        read_only_fields = ['id', 'dt_created', 'dt_modified', 'version']


CashEntrySerializer = CashSerializer  # alias
PaymentSerializer = CashSerializer    # backwards-compatible alias
