...
- Cash ledgers: negative value_available (money received FROM customer)
...
from apps.accounts.services.gl_defaults import get_invoice_cash_staging_defaults
...
def update_org_balances(org: 'OrgBase', save: bool = True) -> Dict[str, Any]:
    ...
    AUDIT: This is the REAL-TIME balance update, called after every invoice/cash save.
    ...
    - Balance due appears on statements and affects cash terms
...
def _stage_invoice_gl_accounts(invoice) -> None:
    defaults = get_invoice_cash_staging_defaults()
...
def _stage_cash_gl_accounts(cash) -> None:
    method_name = getattr(cash, 'method', '') or ''
    defaults = get_invoice_cash_staging_defaults(
        cash_method_name=method_name,
        cash_method_metadata=None,
    )
    amount = _as_decimal_amount(getattr(cash, 'amount', 0))
    abs_amount = abs(amount)
    is_received = amount > 0  # positive = money in, negative = money out (refunds, disbursements)

    metadata = getattr(cash, 'metadata', None) or {}
    if not isinstance(metadata, dict):
        metadata = {}
    ...
    cash.metadata = metadata
    try:
        now_ms = int(timezone.now().timestamp() * 1000)
        cash.__class__.objects.filter(pk=cash.pk).update(metadata=metadata, dt_modified=now_ms)
    except Exception:
        pass
...
def on_invoice_save(invoice, replace_ledgers: bool = True) -> None:
    ...
    PIPELINE:
    1. Create/replace Ledger records based on cash terms
    ...
    - value_available tracks current unpaid balance (mutable via cash)
...
def on_cash_save(cash) -> None:
    """
    Handle cash save: create cash ledger and update org balances.
    
    AUDIT: This is the PRIMARY entry point for cash-related ledger creation.
    Called via signal or directly after cash.save().
    ...
    - Cash ledgers: NEGATIVE value (money received FROM customer)
    - Balance = Sum of all ledgers = Invoices - Cash
    
    FINANCIAL IMPACT:
    - Cash decreases org balance_due
    - Credit available increases (may release holds)
    - May shift items out of aging buckets (depending on application)
    
    AUDIT TRAIL:
    - Cash ledger has parent_id pointing to cash.id
    - Cash ledger has model_name='cash' for type identification
    - Negative values in value_original and value_available
    - Links to invoice via invoice_id FK
    
    Args:
        cash: The saved Cash instance
    """
    from .terms_ledger import record_cash

    _stage_cash_gl_accounts(cash)

    Ledger = dj_apps.get_model('accounts', 'Ledger')
    
    # AUDIT: Delete existing ledgers for this cash before recreating
    ...
    # AUDIT: Cash ledger value = −available (unapplied), original = −amount.
    ...
    if getattr(cash, 'amount', None) is not None:
        record_cash(
            ...
            dt_paid=getattr(cash, 'dt_cash', None) or timezone.now(),
            cash=cash
        )

    # Update org balances — the cash's own customer, else its invoice's customer
    OrgBase = dj_apps.get_model('orgs', 'OrgBase')
    org_id = getattr(cash, 'customer_id', None) or getattr(invoice, 'customer_id', None)
...
def rebuild_org_ledgers(org: 'OrgBase') -> Dict[str, int]:
    ...
    recreates them from source invoices and cash_entries. This should only
    ...
    3. For each invoice: recreate ledger(s) based on cash terms
    4. For each cash entry: recreate negative ledger record
    5. Update org.financial balances
    6. COMMIT TRANSACTION
    
    AUDIT TRAIL:
    - New ledgers will have fresh created timestamps
    - Any cash application history is reconstructed, not preserved
    ...
    Returns:
        ...
        - cash_ledgers: Number of cash ledgers created
    """
    from .terms_ledger import apply_terms_for_invoice, record_cash
    ...
    with transaction.atomic():
        ...
        # =======================================================================
        # STEP 3: Recreate cash ledgers from source documents  
        # =======================================================================
        cash_entries = Cash.objects.filter(invoice__customer_id=org_id)
        for cash in cash_entries:
            pay_available = getattr(cash, 'available', 0)
    ...
    # invoice_ledgers + cash_ledgers should roughly match deleted count
