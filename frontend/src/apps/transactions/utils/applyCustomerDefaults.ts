 ...
 */
import { getRecord, manageAction } from '@/api/wcapi';
...
export interface CustomerDefaults {
  ...
  addresses: {
    bill_to: { contact_id?: number; company: string; attention: string; full_address: string; instructions: string };
    ship_to: { contact_id?: number; company: string; attention: string; full_address: string; instructions: string };
  };
  emails: {
...
export async function applyCustomerDefaults(
  ...
  // The address block displays addresses.<role>.company — without it the
  // card shows "—" even though the customer has a name.
  const billCompany = orgAddrs.bill_to?.company || company;
  const shipCompany = orgAddrs.ship_to?.company || billCompany;
  ...
  const defaults: CustomerDefaults = {
    ...
    // JSON aspects (source of truth)
    addresses: {
      bill_to: { contact_id: billContactId, company: billCompany, attention: billAttn, full_address: billAddr, instructions: billInstr },
      ship_to: { contact_id: shipContactId, company: shipCompany, attention: shipAttn, full_address: shipAddr, instructions: shipInstr },
    },
    emails: {
  ...
  if (isExempt) {
    ...
    // One lookup for every case (backend tax_resolve): the customer's own
    // jurisdiction, else the ship-to ZIP, else the company default jurisdiction.
    try {
      const zip = (shipAddr.match(/\b(\d{5})(?:-\d{4})?\s*$/) || [])[1] || '';
      const res = await manageAction('lookup_tax_jurisdiction', {
        jurisdiction_id: cust.tax_jurisdiction_id || 0,
        zip_code: zip,
      });
      const tj = res?.data?.data ?? res?.data ?? res;
      if (tj && tj.sales_rate != null && (tj.id || tj.name)) {
        defaults.finance = {
          sales_tax_rate: tj.sales_rate || 0,
          sales_tax_name: tj.tax_name || tj.name || '',
          sales_tax_id: tj.id || 0,
          tax_gl_account: tj.gl_account_payable || '',
        };
        defaults.tax = {
          sales_rate: tj.sales_rate || 0,
          cost_rate: tj.cost_rate || 0,
          shipping: tj.shipping_rate || 0,
        };
      }
    } catch { /* lookup failed — user sets rate manually */ }
  }
