...
interface PortalRecord {
  ...
  ida?: string;
  status?: string;
  totals?: { total?: number; balance?: number; cash_state?: string };
  dt_created?: string;
  company?: string;
  display_name?: string;
  name?: string;
    ...
    <>
          ...
          columns={[
            ...
            { key: 'kanban', label: 'inventory level', render: r => (
              <KanbanBar
                ...
                min={r.quantity?.min ?? 0}
                max={r.quantity?.max ?? 100}
              />
            )},
      ...
      <RecordList
        ...
        columns={[
          ...
          { key: 'name', label: 'item' },
          { key: 'kanban', label: 'inventory level', render: r => {
            const min = r.quantity?.min ?? 0;
            const max = r.quantity?.max ?? 100;
            return min > 0
