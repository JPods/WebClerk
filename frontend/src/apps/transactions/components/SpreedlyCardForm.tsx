...
import { fetchGatewayConfig, processGatewayCash } from '../models/cash/services/cashApi';
...
export default function SpreedlyCardForm({
  ...
  useEffect(() => {
    ...
    loadSpreedlySDK()
      .then(() => {
        ...
        S.on('paymentMethod', (token: unknown) => {
  ...
  const handleTokenReceived = useCallback(async (paymentMethodToken: string) => {
    try {
      const result = await processGatewayCash(invoiceId, amount, paymentMethodToken);
