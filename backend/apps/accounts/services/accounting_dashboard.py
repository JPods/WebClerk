...
def _get_journal_status() -> Dict[str, Any]:
    ...
    last_cash_gl = (
        GlJournal.objects.filter(source_model='cash')
        .order_by('-dt_created').values('source_id', 'dt_created').first()
    )
    ...
    return {
        ...
        'last_cash_journalized': last_cash_gl,
...
def _get_locked_records() -> Dict[str, Any]:
    ...
    result = {}
    # Result keys stay 'invoice' / 'cash' (what the dashboard reads);
    # the cash model is Cash since the rename.
...
def get_journal_exceptions(year: int = None, month: int = None) -> Dict[str, Any]:
    ...
    2. **skipped** — hold cash, consigned invoices, zero-amount cash
