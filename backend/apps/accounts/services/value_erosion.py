...
Usage:
    from apps.accounts.services.value_erosion import detect_margin_erosion, detect_late_payment

    # Called after invoice save (inside on_invoice_save or Phase 5 of transaction_save)
    events = detect_margin_erosion(invoice)

    # Called after cash save
    events = detect_late_payment(cash)
...
def detect_margin_erosion(invoice) -> list:
    ...
    for ancestor in ancestors:
        ...
        erosion = Erosion.objects.create(
            ...
            comments=_process_comment(f"Margin dropped ${margin_loss} ({pct_loss}pp) from {anc_model_name} #{ancestor.id}"),
            is_auto=True,
        )
...
def detect_late_payment(cash, daily_rate: D = D('0.0005')) -> Optional['Erosion']:
    """
    If cash arrives after ledger due date, record carrying cost as erosion.

    daily_rate: daily cost of capital (default 0.05% = ~18% annual).
    Only fires if there's a ledger record with dt_due < cash date.
    ...
    cash_amount = _safe_decimal(getattr(cash, 'amount', 0))
    if not cash_amount or cash_amount <= 0:
        return None

    dt_paid = getattr(cash, 'dt_cash', None) or getattr(cash, 'dt_created', None)
    if dt_paid is None:
        return None
    ...
    carrying_cost = cash_amount * daily_rate * days_late

    # Replace existing auto late-payment erosion for this cash
    Erosion.objects.filter(
        source_model='cash',
        source_id=cash.id,
    ...
    erosion = Erosion.objects.create(
        ...
        dt_event=dt_paid,
        comments=_process_comment(f"Cash {days_late} days late on ${cash_amount}. "
              f"Carrying cost at {float(daily_rate * 100):.3f}%/day = ${carrying_cost:.2f}"),
        is_auto=True,
    )

    logger.info(
        "Late cash erosion: cash %s — %d days late, $%s carrying cost",
        cash.id, days_late, carrying_cost,
    )
    return erosion
...
def detect_discount_erosion(transaction) -> Optional['Erosion']:
    ...
    erosion = Erosion.objects.create(
        ...
        source_id=transaction.id,
        dt_event=datetime.now(timezone.utc),
        comments=_process_comment(f"Discount of ${discount} applied on {model_name} #{transaction.id}"),
        is_auto=True,
    )
    return erosion
        ...
        for entry in entries:
            ...
            try:
                erosion = Erosion.objects.create(
                    ...
                    comments=_process_comment(note),
                    is_auto=False,
                )
