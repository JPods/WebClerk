...
REACT_ROOT = Path('/Users/williamjames/Documents/CommerceExpert/WebClerk/frontend/src')

# Anti-patterns to detect
ANTI_PATTERNS = [
    {
        'id': 'custom-table',
        'pattern': r'<table\b(?![^>]*data-wc)',
    ...
    },
    {
        ...
        'pattern': r'fontSize:\s*\d+|fontSize:\s*["\']?\d+px',
        'check': 'hardcoded_font_size',
        'exclude_files': ['DataGrid.tsx', 'PrintDocumentLayout.tsx', 'ProjectGanttPanel.tsx'],
        'exclude_dirs': ['components/print/', 'print/'],
        'severity': 'info',
        'message': 'Hardcoded fontSize — use baseFontSize prop or CSS variable (--sp-fs, --dg-fs, --db-font-size) so font size responds to user preference.',
        'fix': 'Read font size from getUI("theme.${active}.font.size") and use baseFontSize +/- offsets, or CSS calc(var(--sp-fs) - Npx).',
    },
    {
        'id': 'hardcoded-font-size-css',
        'pattern': r'font-size:\s*\d+px',
        'check': 'hardcoded_font_size_css',
        'exclude_dirs': ['components/print/', 'print/'],
        'file_type': '.css',
        'severity': 'info',
        'message': 'Hardcoded font-size in CSS — use calc() with a CSS variable so font size responds to user preference.',
        'fix': 'Use calc(var(--sp-fs) - Npx) or var(--dg-fs) instead of fixed px values.',
    },
]
            ...
            if re.search(pattern, line):
                ...
                if check == 'no_dark_variant':
                    ...
                    if 'dark:' in context or 'theme' in context or 'isDark' in context:
                        continue
                    # Skip accent/status colors — intentionally same in both themes
                    accent_colors = ('#ef4444', '#dc2626', '#22c55e', '#16a34a', '#3b82f6',
                                     '#2563eb', '#f59e0b', '#d97706', '#6b7280', '#9ca3af',
                                     '#e53e3e', '#fff', '#ffffff')
                    if any(c in line.lower() for c in accent_colors):
                        continue
                if check == 'missing_data_wc':
                    if 'data-wc' in content:
                        continue  # File has at least one data-wc
                if check == 'hardcoded_font_size':
                    # Skip if line uses baseFontSize, a CSS variable, or calc()
                    if 'baseFontSize' in line or 'fontScale' in line or 'var(' in line:
                        continue
                if check == 'hardcoded_font_size_css':
                    # Skip if line uses calc() or var()
                    if 'calc(' in line or 'var(' in line:
                        continue
