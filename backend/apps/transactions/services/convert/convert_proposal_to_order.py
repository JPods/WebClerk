...
def transfer_proposal_to_order(
    *,
    ...
    """Create the Order header from a proposal and return its lines for review (atomic).

      1. Select lines (all or by ID list)
      2. Create Order header: parent_model='proposal', parent_id, refs.source.proposal_id
      3. Return line data (not saved). Saving the order creates OrderLines with
         parent_line_id; the proposal lines' remaining/status follow from their children
         (readmes/transactions/line-quantity.md).

    preserve_proposal is accepted for caller compatibility and ignored — the proposal
    ...
    order_kwargs = {
        "status": order_status,
        # parent link: saving the order validates transfers against proposal remaining
