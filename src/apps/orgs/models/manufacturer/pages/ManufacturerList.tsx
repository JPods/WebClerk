...
import ManufacturerDisplay from "./ManufacturerDisplay";
...
export default function ManufacturerList() {
  ...
  const getManufacturerData = useCallback(async () => {
    try {
      setLoading(true);
      const list = await getRecords('manufacturer');
  ...
  const handleDelete = async (row: any) => {
    if (window.confirm(`Delete manufacturer ${row.id}?`)) {
      try {
        await deleteRecord('manufacturer', row.id);
        dispatch(showToast({ message: "Manufacturer deleted successfully", type: "success" }));
        getManufacturerData(); // Refresh data
      } catch (error) {
  ...
  const userColumns: TableColumn<any>[] = [
    ...
    { name: "Industry", selector: (row) => row.industry || "--", sortable: true, width: "30%" },
    { name: "Location", selector: (row) => row.location || "--", sortable: true, width: "20%" },
  ];
          ...
          <ComponentCard>
            ...
            <div className="overflow-x-auto bg-white text-gray-900 dark:bg-gray-900 dark:text-gray-400 rounded-md">
              <DataTable
                ...
                progressComponent={<div className="p-8 text-center">Loading manufacturers...</div>}
