...
def main():
    parser = argparse.ArgumentParser(description="Force sync remote DB data into local DB")
    ...
    try:
        ...
        with remote_conn.cursor() as remote_cur, local_conn.cursor() as local_cur:
            ...
            if args.dry_run:
                ...
                print("Remote/local connectivity OK")
                local_conn.rollback()
                remote_conn.rollback()
                return

            tables = get_tables(remote_cur)
            if not tables:
                write_status(status_file, "completed", 100, "No remote tables found")
                print("No tables found on remote")
                local_conn.rollback()
                remote_conn.rollback()
                return
            ...
            total_rows = 0
            for table in tables:
                remote_cur.execute(sql.SQL("SELECT COUNT(*) FROM {}.{}").format(sql.Identifier("public"), sql.Identifier(table)))
                table_count = int(remote_cur.fetchone()[0])
                table_counts[table] = table_count
                total_rows += table_count

            write_status(status_file, "running", 2, f"Preparing local reset for {len(tables)} tables")
            ...
            local_cur.execute(truncate_stmt)
            ...
            for table in tables:
                ...
                remote_cur.execute(select_stmt)
                ...
                while True:
                    ...
                    execute_values(local_cur, insert_stmt.as_string(local_conn), prepared_rows, page_size=1000)
            ...
            reset_sequences(local_cur)
            local_conn.commit()
            remote_conn.rollback()
            print()
            write_status(status_file, "completed", 100, "Remote data force-synced into local database")
            print("Remote data force-synced into local database")

    except Exception:
        local_conn.rollback()
        remote_conn.rollback()
        write_status(args.status_file or None, "failed", 100, "Remote-to-local sync failed")
        print()
        raise
    finally:
        remote_conn.close()
