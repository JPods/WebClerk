...
export default function LinkageDisplay({
  inline = false,
  modeProp,
  dataProp,
  onSaved,
  ...
  useEffect(() => {
    const fetchFields = async () => {
      try {
        const rec = await getRecord('linkage', 1);
        const record = rec.record || rec;
        setFields(Object.keys(record));
      } catch (error) {
        console.error("Failed to fetch fields", error);
        setFields(['id', 'source', 'target', 'type']); // fallback
      }
    };
    fetchFields();
  }, []);
  ...
  useEffect(() => {
    if (modeProp === "edit" && dataProp?.id) {
      const fetchData = async () => {
        try {
          setLoading(true);
          const rec = await getRecord('linkage', dataProp.id);
  ...
  const handleSave = async () => {
    try {
      setLoading(true);
      await saveRecord('linkage', data);
      dispatch(showToast({ message: "Linkage saved successfully", type: "success" }));
      onSaved?.();
    } catch (error) {
      console.error("Failed to save", error);
      dispatch(showToast({ message: "Failed to save linkage", type: "error" }));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-4 space-y-4">
      <h2 className="text-xl font-semibold">
        {modeProp === "add" ? "Add Linkage" : modeProp === "edit" ? "Edit Linkage" : "View Linkage"}
      </h2>
