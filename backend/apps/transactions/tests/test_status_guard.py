...
class TestJournalizedLock(TestCase):
    ...
    def test_cash_journalized_blocks_modification(self):
        obj = _make_instance(
            model_name='Cash',
            dt_journaled=1719000000000,
        )
        result = validate_modification(obj, 'cash')
        self.assertFalse(result.can_proceed)
...
class TestJournalizableModels(TestCase):
    ...
    def test_cash_journalizable(self):
        self.assertIn('cash', JOURNALIZABLE_MODELS)
