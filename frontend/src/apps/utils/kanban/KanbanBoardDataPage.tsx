...
const KanbanBoardDataPage: React.FC = () => {
  ...
  const [columnsPerRow, setColumnsPerRow] = useState<number>(() =>
    Math.min(4, Math.max(1, initialBoardState.column_order.length || 1))
  );
  ...
  const columns = useMemo(
    () =>
      board.column_order
        .map((columnId: string) => board.columns[columnId])
        .filter((column: KanbanColumnType | undefined): column is KanbanColumnType => Boolean(column)),
    [board]
  );
  ...
  const prioritySummary = useMemo(() => {
    ...
    Object.values(board.tasks).forEach((task: KanbanTask) => {
      // Map numeric or string priority to TaskPriority
      let priority: TaskPriority = 'medium';
      if (typeof (task as any).priority === 'string') {
        priority = (task as any).priority;
      } else if (typeof (task as any).priority === 'number') {
      ...
      } else if ((task as any).priority_value) {
        switch ((task as any).priority_value) {
          case 1: priority = 'low'; break;
          case 2: priority = 'medium'; break;
          case 3: priority = 'high'; break;
          case 4: priority = 'critical'; break;
  ...
  const languageSummary = useMemo(() => {
    ...
    Object.values(board.tasks).forEach((task: KanbanTask) => {
      task.language_codes?.forEach((code: string) => languages.add(code));
    });
      ...
      <DndProvider backend={HTML5Backend}>
        <div className="grid gap-5 pb-6" style={gridStyle}>
          {columns.map((column: KanbanColumnType) => {
            const tasks: KanbanTask[] = column.task_ids
              .map((taskId: string) => board.tasks[taskId])
              .filter((task: KanbanTask | undefined): task is KanbanTask => Boolean(task));
            return (
              <KanbanColumn
                key={column.id}
                column={column}
