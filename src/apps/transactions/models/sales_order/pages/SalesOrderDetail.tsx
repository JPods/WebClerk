...
const getSalesOrderTabsAfter = (data: Transaction): TransactionTab[] => {
  ...
  const pendingActions = salesOrderData.actions?.items?.filter(a => a.status === 'pending').length ?? 0;
          ...
          <dl className="space-y-3 text-sm">
            ...
            {data.is_locked && (
              <div className="flex justify-between items-center pt-2 mt-2 border-t border-slate-200 dark:border-slate-700">
                <FieldLabel label="Locked" locked className="text-slate-500 dark:text-slate-400" />
...
const SalesOrderLines: React.FC<{
...
}> = ({ lines, isEditing, isLocked = false, onDeleteLine, onUpdateLine, onUpdateFullLine, onDuplicateLine, onAddItem, onLinesChange }) => {
  ...
  const handleDoubleClick = (lineId: number, field: string, currentValue: string | number) => {
  ...
  const handleEditSave = () => {
    if (editingCell && onUpdateLine) {
      const value = editingCell.field === 'qty' || editingCell.field === 'unit_price' 
        ? Number(editValue) 
        : editValue;
  ...
  const toggleSelectAll = () => {
    if (selectedLineIds.size === lines.length) {
      ...
      setSelectedLineIds(new Set(lines.map(l => l.id).filter((id): id is number => id !== undefined)));
    }
  };
          ...
          <SalesOrderItemSearch 
            onAddItem={(item, quantity) => {
              ...
              if (typeof item.price === 'number') {
                ...
                unitPrice = Number(priceObj.base ?? priceObj.retail ?? priceObj.sell ?? priceObj.unit ?? 0);
              }
              ...
              const newLine: TransactionLine = {
                // No id for new line - backend will create one
                _dirty: true, // Mark as dirty for save
                item: {
                  item_id: itemId,
                  ida_item: idaItem,
      ...
      {!lines.length ? (
          ...
          <table className="w-full">
            <thead className="bg-slate-50 dark:bg-slate-900/50">
              <tr>
                ...
                <th className="px-3 py-3 text-center text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide w-24">Actions</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
          {lines.map((line, idx) => {
            ...
            const unitPrice = priceRecord?.sell ?? priceRecord?.unit ?? line.price?.unit ?? 0;
            const costRecord = lineRecord.cost as Record<string, unknown> | undefined;
            const unitCost = costRecord?.unit ?? line.cost?.unit ?? 0;
            const extended = priceRecord?.extended ?? line.price?.extended ?? (Number(qty) * Number(unitPrice));
            ...
            const isExpanded = typeof lineId === 'number' && expandedRows.has(lineId);
            const isSelected = typeof lineId === 'number' && selectedLineIds.has(lineId);
            ...
            const renderEditableCell = (
              ...
              const isEditingThis = editingCell?.lineId === lineId && editingCell?.field === field;
                ...
                return (
                  <td className={className}>
                    <input
                      ...
                      style={inputType === 'number' ? { textAlign: 'right' } : {}}
                    />
                  </td>
                );
              }
              ...
              return (
                <td 
                  className={`${className} ${canEditLine ? 'cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/20' : ''}`}
                  onDoubleClick={() => canEditLine && line.id && handleDoubleClick(line.id, field, value)}
            ...
            return (
              ...
              <tr 
                className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 ${isSelected ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}
              >
                ...
                {canEdit && (
                  <td className="px-2 py-3 text-center">
                    <input
                      type="checkbox"
                      checked={isSelected}
                      onChange={() => typeof lineId === 'number' && toggleLineSelection(lineId)}
                      className="rounded border-slate-300 dark:border-slate-600 text-blue-600 focus:ring-blue-500"
                    />
                  </td>
                )}
                ...
                <td className="px-2 py-3 text-center">
                  {hasNotes ? (
                    <button
                      type="button"
                      onClick={() => typeof lineId === 'number' && toggleRowExpansion(lineId)}
                ...
                <td className="px-3 py-3 text-center">
                  <div className="flex items-center justify-center gap-1">
                    ...
                    {/* Delete Line */}
                    {canEditLine && onDeleteLine && line.id && (
                    ...
                    <div className="flex flex-wrap gap-4 text-sm">
                      {notesObj?.public && (
                        <div className="flex-1 min-w-[200px]">
                          ...
                          <span className="text-slate-600 dark:text-slate-300">{notesObj.public}</span>
                        </div>
                      )}
                      {notesObj?.internal && (
                        <div className="flex-1 min-w-[200px]">
                          <span className="font-medium text-blue-700 dark:text-blue-400">Internal: </span>
                          <span className="text-slate-600 dark:text-slate-300">{notesObj.internal}</span>
                        </div>
                      )}
                      {notesObj?.warehouse && (
                        <div className="flex-1 min-w-[200px]">
                          <span className="font-medium text-green-700 dark:text-green-400">Warehouse: </span>
                          <span className="text-slate-600 dark:text-slate-300">{notesObj.warehouse}</span>
                        </div>
                      )}
                    </div>
                  </td>
        ...
        <tfoot className="bg-slate-50 dark:bg-slate-900/50 border-t-2 border-slate-300 dark:border-slate-600">
          ...
          <tr>
            ...
            <td colSpan={2}></td>
            <td className="px-4 py-2 text-sm font-medium text-slate-500 dark:text-slate-400 text-right">Total:</td>
...
const ActionsTable: React.FC<{
...
}> = ({ actions, isEditing, isLocked = false, onAddAction, onUpdateAction, onDeleteAction }) => {
  ...
  const handleAddSubmit = () => {
    if (newAction.what && onAddAction) {
      ...
      setNewAction({ kind: 'task', status: 'pending', priority: 'normal', what: '' });
  ...
  const getStatusColor = (status?: string) => {
    switch (status) {
      ...
      case 'canceled': return 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-400';
    ...
    <div className="space-y-4">
      ...
      {showAddForm && (
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-blue-200 dark:border-blue-800 p-4">
          <h4 className="font-medium text-slate-900 dark:text-white mb-3">New Action</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Type</label>
              <select
                value={newAction.kind || 'task'}
                onChange={(e) => setNewAction({ ...newAction, kind: e.target.value })}
            ...
            <div>
              ...
              <input
                type="date"
                value={newAction.when ? new Date(newAction.when as string).toISOString().split('T')[0] : ''}
                onChange={(e) => setNewAction({ ...newAction, when: e.target.value })}
                className="w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setNewAction({ ...newAction, who_name: e.target.value })}
                className="w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
              />
            </div>
      ...
      {!actions.length ? (
        <div className="text-center py-12 text-slate-400">
          ...
          {canEdit && (
            <p className="mt-2 text-sm">Click "Add Action" to create a task or follow-up</p>
          )}
        </div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-50 dark:bg-slate-900/50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Description</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide w-28">Due</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide w-32">Assigned</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide w-24">Priority</th>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {actions.map((action, idx) => (
                <tr 
                  key={action.id ?? idx}
                  className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 ${action.status === 'done' ? 'opacity-60' : ''}`}
                >
                  {canEdit && (
                    <td className="px-3 py-3">
                      <button
                        ...
                        title={action.status === 'done' ? 'Mark as pending' : 'Mark as complete'}
                      >
                  ...
                  <td className={`px-4 py-3 text-sm text-slate-900 dark:text-white ${action.status === 'done' ? 'line-through' : ''}`}>
                    {action.what ?? '--'}
                  </td>
                  ...
                  <td className={`px-4 py-3 text-sm font-medium capitalize ${getPriorityColor(action.priority)}`}>
            ...
            <tfoot className="bg-slate-50 dark:bg-slate-900/50 border-t-2 border-slate-300 dark:border-slate-600">
              <tr>
                <td colSpan={canEdit ? 8 : 6} className="px-4 py-2 text-sm text-slate-600 dark:text-slate-300">
...
const SalesOrderDetail: React.FC<SalesOrderDetailProps> = ({ 
  ...
  const renderCustomTab = useCallback((
    ...
    const handleAddAction = (action: ActionItem) => {
      if (onFieldChange) {
        ...
        onFieldChange('actions', { ...salesOrderData.actions, items: newActions });
      }
    };
  ...
  const renderHeader = useCallback((data: Transaction, isEditing: boolean, onChange?: (field: string, value: unknown) => void) => (
    <SalesOrderHeader 
      data={data as SalesOrder} 
      isEditing={isEditing} 
      onChange={onChange as ((field: keyof SalesOrder, value: unknown) => void) | undefined}
      ...
      onDuplicateLine={(lineId) => {
        ...
        if (onLinesChange) {
          ...
          if (lineToDup) {
            const newLine = { 
              ...lineToDup, 
              id: undefined, // No ID for new line - backend will create one
              _dirty: true   // Mark as dirty for save
            };
            onLinesChange([...lines, newLine]);
          }
