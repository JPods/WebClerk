...
def test_comments_mixin_routes_to_linkage(django_user_model):
    ...
    # minimal permission for quote
    make_setting(purpose='wc:view_edit', parent_model='quote', config={'USER': {'view':['id'], 'edit':['id']}})
    prop = Quote.objects.create(name='CMT-PROP')
    pl = QuoteLine.objects.create(quote=prop, status='OPEN')
    ...
    entry = pl.add_comment(channel='public', text='hello world', model='quote_line', record_id=pl.pk)
    assert entry['text'] == 'hello world'
    lk.refresh_from_db()
    assert lk.comments['records'][f'quote_line/{pl.pk}']['public'][0]['text'] == 'hello world'
