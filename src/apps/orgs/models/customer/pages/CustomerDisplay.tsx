...
export default function CustomerDisplay({
  ...
  useEffect(() => {
    const fetchFields = async () => {
      try {
        const rec = await getRecord('customer', 1);
        const record = rec.record || rec;
        setFields(Object.keys(record));
      } catch (error) {
        console.error("Failed to fetch fields", error);
        setFields(['id', 'name', 'email', 'phone', 'address']); // fallback
      }
    };
    fetchFields();
  }, []);
  ...
  useEffect(() => {
    if (modeProp === "edit" && dataProp?.id) {
      const fetchData = async () => {
        try {
          setLoading(true);
          const rec = await getRecord('customer', dataProp.id);
  ...
  const handleSave = async () => {
    try {
      setLoading(true);
      await saveRecord('customer', data);
      dispatch(showToast({ message: "Customer saved successfully", type: "success" }));
      onSaved?.();
    } catch (error) {
      console.error("Failed to save", error);
      dispatch(showToast({ message: "Failed to save customer", type: "error" }));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-4 space-y-4">
      <h2 className="text-xl font-semibold">
        {modeProp === "add" ? "Add Customer" : modeProp === "edit" ? "Edit Customer" : "View Customer"}
      </h2>
