...
from apps.transactions.models import Payment, Invoice, Receipt
...
from apps.transactions.services.payment.payment_pending_receipt import apply_payment_to_receipt
...
class PaymentViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    @action(detail=True, methods=['post'])
    def apply_to_receipt(self, request, pk=None):
        """Apply payment to a receipt via pending record (AP mirror of apply_to_invoice)."""
        payment = self.get_object()
        ...
        try:
            result = apply_payment_to_receipt(
                payment_id=payment.pk,
            ...
            logger.error(f"Error applying payment {payment.id} to receipt {receipt_id}: {e}")
            return Response(
                {'error': 'Failed to apply payment to receipt'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
