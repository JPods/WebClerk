...
def _company_default_jurisdiction_id() -> int:
    """company profile config.tax.default_jurisdiction_id (0 when unset)."""
    Setting = apps.get_model('core', 'Setting')
    company = Setting.objects.filter(purpose='wc:company_profile', is_active=True).first()
    tax = (company.config or {}).get('tax') if company and isinstance(company.config, dict) else None
    return int((tax or {}).get('default_jurisdiction_id') or 0)
...
def lookup_tax_jurisdiction(
    ...
    Priority:
      1. TaxJurisdiction record matching the id, name, or state (local override)
      2. Company default jurisdiction (company profile config.tax.default_jurisdiction_id)
      3. Setting record (WC_HQ synced state rates)
    ...
    # Company default jurisdiction — where the company says it collects tax
    default_id = _company_default_jurisdiction_id()
    if default_id:
        result = _lookup_from_jurisdiction(jurisdiction_id=default_id)
        if result:
            result['resolved_state'] = state
            result['source'] = 'company_default'
            return result
