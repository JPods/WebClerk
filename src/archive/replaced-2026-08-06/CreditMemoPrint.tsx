/**
 * CreditMemoPrint — printable credit memo document.
 * Same invoice data, printed as a credit memo.
 * Route: /transactions/invoice/:id/creditmemo
 */
...
export default function CreditMemoPrint() {
  ...
  if (loading) return <div style={{ padding: 40, textAlign: 'center' }}>Loading credit memo...</div>;
  ...
  return (
    <div style={S.page}>
      <button className="no-print" style={S.printBtn} onClick={() => window.print()}>Print Credit Memo</button>
      ...
      <div style={S.header}>
        ...
        <div style={S.headerRight}>
          ...
          <div style={S.docTitle}>CREDIT MEMO: {inv.ida || inv.id}</div>
        </div>
      </div>
      ...
      <div style={S.metaRow}>
        <div style={S.metaItem}><span style={S.metaLabel}>Credit Date</span> <span style={S.metaValue}>{fmtDate(inv.dt_created)}</span></div>
      ...
      <div style={S.totalsWrap}>
        ...
        <div style={S.totalsRight}>
          ...
          <div style={S.totalRowFinal}><span>Credit Total</span> <span>{fmt(totals.total || inv.total)}</span></div>
          <div style={S.totalRow}><span>Credit Amount</span> <span style={{ fontWeight: 700 }}>{fmt(totals.balance || inv.balance)}</span></div>
        </div>
      </div>
      ...
      <div style={S.footer}>
        <div style={S.refRow}>
          <div style={S.refBox}><div style={S.sectionLabel}>Credit Memo #</div><div>{inv.ida || inv.id}</div></div>
