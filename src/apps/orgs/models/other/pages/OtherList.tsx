...
export default function OtherList() {
  ...
  const handleDelete = async (row: any) => {
    if (window.confirm(`Delete other ${row.id}?`)) {
      try {
        await deleteRecord('other', row.id);
        dispatch(showToast({ message: "Other deleted successfully", type: "success" }));
        getOtherData(); // Refresh data
      } catch (error) {
        dispatch(showToast({ message: "Failed to delete other", type: "error" }));
      }
    }
  };
  ...
  return (
    <>
      <PageBreadcrumb pageTitle="Other List" />
          ...
          <ComponentCard>
            ...
            <div className="overflow-x-auto bg-white text-gray-900 dark:bg-gray-900 dark:text-gray-400 rounded-md">
              <DataTable
                ...
                progressComponent={<div className="p-8 text-center">Loading others...</div>}
