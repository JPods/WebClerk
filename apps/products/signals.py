...
def _invstack_capture_prev(sender, instance: InventoryLayer, **kwargs):  # pragma: no cover - simple state capture
    if instance.pk:
        try:
...
@receiver(post_save, sender=InventoryLayer)
def _invstack_process_on_unlock(sender, instance: InventoryLayer, created: bool, **kwargs):  # pragma: no cover - integration behavior
