...
Creates a portable JSON file containing:
  ...
  2. Demo contacts, orgs, items, BOM, transactions, cash, GL entries
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--all', action='store_true',
                            help='Export every row of the demo bundle models (sanitized, original ids) '
                                 'instead of only records tagged refs.source. See apps/core/services/demo_bundle.py')
    ...
    def handle(self, *args, **options):
        ...
        # ── Demo data ──────────────────────────────────────────────────
        if options['all']:
            from apps.core.services.demo_bundle import DEMO_BUNDLE_MODELS, export_rows, privileged_contact_ids, sanitize_setting
            excluded = privileged_contact_ids()
            bundle['settings'] = [sanitize_setting(s) for s in bundle['settings']]
            bundle['mode'] = 'preserve_ids'
            bundle['models'] = DEMO_BUNDLE_MODELS
            bundle['excluded_contact_count'] = len(excluded)
            total_data = 0
            for label in DEMO_BUNDLE_MODELS:
                rows = export_rows(label, excluded)
                bundle['data'][label] = rows
                total_data += len(rows)
                self.stdout.write(f"  {label}: {len(rows)} records")
            self.stdout.write(f"\nTotal: {len(bundle['settings'])} settings + {total_data} data records "
                              f"({len(excluded)} privileged contacts excluded)")
            return self._write(bundle, options)
    ...
    def _write(self, bundle, options):
        if options['dry_run']:
