/**
 * UsageDisplay - Follows 3-column standard with tab navigation
...
const STORAGE_KEY = "usageDisplay_columnCount";
...
export default function UsageDisplay({
  ...
  // Tab navigation
  const { activeTab, setActiveTab } = useDetailTabs("usage_display", "actions", [
  ...
  useEffect(() => {
    if (modeProp === "edit" && dataProp?.id) {
      const fetchData = async () => {
        try {
          setLoading(true);
          const rec = await getRecord('usage', dataProp.id);
  ...
  const handleSave = async () => {
    try {
      setIsSaving(true);
      await saveRecord('usage', data);
      dispatch(showToast({ message: "Usage saved successfully", type: "success" }));
      onSaved?.();
      if (currentMode === "add") {
        onCancelInline?.();
      } else {
      ...
      dispatch(showToast({ message: "Failed to save usage", type: "error" }));
    } finally {
      setIsSaving(false);
    }
  };
      ...
      {!loading && (
        <ComponentCard>
          ...
          <div className={getGridClassName(columnCount)}>
            ...
            <HorizontalField label="User ID" htmlFor="user_id" icon={<User size={14} />}>
              <Input
                type="text"
                id="user_id"
                placeholder="User ID"
                value={data?.user_id || ""}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleFieldChange("user_id", e.target.value)}
                disabled={currentMode === "view"}
              />
            </HorizontalField>
            <HorizontalField label="Quantity" htmlFor="quantity_used" icon={<Hash size={14} />}>
              <Input
                type="number"
                id="quantity_used"
                placeholder="Quantity Used"
                value={data?.quantity_used ?? ""}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleFieldChange("quantity_used", Number(e.target.value))}
                disabled={currentMode === "view"}
              />
            </HorizontalField>
