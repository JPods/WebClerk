...
def _copy_header_fields(source, target, is_sell_side: bool = True, extra_fields: tuple = ()) -> None:
    ...
    for field in fields:
        ...
        if val is not None and hasattr(target, field):
            # JSON aspects are dicts: copy, so the two documents never share one object.
...
def _update_source_status(source, source_type: str) -> None:
    ...
    # A line is done when nothing is left, in either direction: quantities may go
    # negative and the user is in control (Bill, 2026-09-17).
...
def _do_convert(
    *,
    ...
    for src_line in source_lines:
        ...
        if not is_sell_side:
            # A purchase is what we pay the vendor. It starts from the item's
            # cost at the company default (unit_cost_default), never from the
    ...
    if lines_available == 0:
        raise ConversionError(
            "No convertible lines (every line has remaining 0)"
        )

    # ── Forward cash from source to target ────────────────────────
    # Invoice knows its parent (parent_id, parent_model). Find cash
    # linked to the parent order via refs.source and re-link to the invoice.
    cash_entries_forwarded = 0
    if source_type == "order" and target_type == "invoice":
        Cash = _get_model("cash")
        # Find cash whose parent is this order
        order_cash_entries = Cash.objects.filter(
            is_active=True,
            parent_model="order",
            parent_id=source_id,
        )
        for pay in order_cash_entries:
            ...
            cash_entries_forwarded += 1

        # received is NOT set here. How much of a forwarded deposit to apply, and when,
        # is a human decision (Bill, 2026-09-17). The cash is re-linked and available;
        # someone applies it, which creates the application record received is computed from.
    ...
    return {
        ...
        "cash_entries_forwarded": cash_entries_forwarded,
    }
...
def convert_order_to_purchase(
    ...
    Returns:
        {purchase_id, purchase_ida, lines_for_review, lines}
    """
    # Resolve vendor
