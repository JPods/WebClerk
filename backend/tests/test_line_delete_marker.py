...
def test_a_marked_line_is_deleted_by_the_save_and_releases_what_it_held():
...
def test_the_marker_never_reaches_the_row():
...
def test_a_marked_line_that_is_not_on_the_document_fails_hard():
    ...
    with pytest.raises(Refused, match=str(pk)) as refused:
        save_document(
            'order', {'id': order.pk, 'status': 'open'},
            [{'id': pk, '_delete': True, 'item': {'item_id': item.pk}, 'quantity': {'active': 3}}]
        )
    assert refused.value.code == 'line_not_found'
