...
def test_line_aggregation_simple(django_user_model):
    ...
    Setting.objects.create(purpose='wc:view_edit', parent_model='order_lines', is_active=True,
                           config={"USER": {"view": ["id"], "edit": []}})
...
def test_multi_model_permission_order_line(django_user_model):
    ...
    OrderLine.objects.create(order=parent, status='OPEN')
    client = _auth(user)
...
def test_scoped_aggregation(django_user_model):
    Setting.objects.create(purpose='wc:view_edit', parent_model='proposal_line', is_active=True,
                           config={"USER": {"view": ["id"], "edit": []}})
    Setting.objects.create(purpose='wc:view_edit', parent_model='invoice_line', is_active=True,
                           config={"USER": {"view": ["id"], "edit": []}})
...
def test_permissions_remaining_line_models(django_user_model):
    ...
    models_tables = ['invoice_line','purchase_lines','workorder_line','requisition_line']
    for tbl in models_tables:
        Setting.objects.create(purpose='wc:view_edit', parent_model=tbl, is_active=True,
                               config={"PUBLIC": {"view": ["id", "status"], "edit": []}})
    ...
    InvoiceLine.objects.create(invoice=inv, status='SENT')
    PurchaseLine.objects.create(purchase=pur, status='OPEN')
    WorkOrderLine.objects.create(workorder=wo, status='OPEN')
    RequisitionLine.objects.create(requisition=req, status='OPEN')
    client = _auth(user)
