...
export function IssueReporter() {
  ...
  const { backend: systemInfo } = useDataSetInfo();
      ...
      try {
        ...
        const actionPayload: Record<string, unknown> = {
          ...
          metadata: {
            ...
            reporter: {
              email: user?.email ?? "",
              name: user ? `${user.name_first} ${user.name_last}`.trim() : "",
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-3 px-5 py-4">
              ...
              <div className="rounded-lg bg-slate-50 px-3 py-2 text-[11px] text-slate-500 dark:bg-slate-800/50 dark:text-slate-400">
                ...
                {user?.email ?? "not signed in"} &middot; v{typeof __APP_VERSION__ !== "undefined" ? __APP_VERSION__ : "?"} &middot;{" "}
                {window.location.pathname} &middot; {window.innerWidth}x{window.innerHeight}
              </div>
