...
export const TabContent: React.FC<{
  ...
  switch (tabId) {
    ...
    case 'files': {
      const LazyFileUploadPanel = React.lazy(() => import('@/components/common/FileUploadPanel'));
      return (
        <React.Suspense fallback={<div className="p-4 text-xs text-slate-400">Loading...</div>}>
          <LazyFileUploadPanel
            modelName={modelName}
            recordId={data?.id}
            recordIda={data?.ida}
...
export const SummaryTabContent: React.FC<{ data: any; modelName: string }> = ({ data, modelName }) => {
  ...
  // Cash and invoice data for third column
  const [cashEntries, setCashEntries] = React.useState<any[]>([]);
  const [invoices, setInvoices] = React.useState<any[]>([]);
  React.useEffect(() => {
    if (!data?.id) return;
    (async () => {
      try {
        // Invoices linked via parent_id; cashEntries by customer (no parent_id on cash model)
        const custId = data.customer_id || data.customer;
        const [payRes, invRes] = await Promise.all([
          custId ? getRecords('cash', { customer_id: custId, limit: 20 }).catch(() => null) : null,
        ...
        setCashEntries(filterReal(payRes?.results));
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const totalCashEntries = cashEntries.reduce((s: number, d: any) => s + Number(d.totals?.total ?? 0), 0);
    ...
    <div className="grid grid-cols-3 gap-6 db-font-xs">
      ...
      <div>
        ...
        <div className="space-y-0.5">
          ...
          <div className="border-t border-[var(--db-border,#dee2e6)] my-1" />
          <div className="flex justify-between"><span className="text-[var(--db-text-muted,#6c757d)]">cashEntries</span><span className="font-mono">{data?.finance?.cash_count ?? totals.cash_count ?? 0}</span></div>
      ...
      <div>
        ...
        <div className="space-y-0.5">
          ...
          <div className="border-t border-[var(--db-border,#dee2e6)] my-1" />
          <div className="db-font-xs font-medium text-[var(--db-text-dim,#adb5bd)] mb-1">Cash</div>
          ...
          <div className="flex justify-between"><span className="text-[var(--db-text-muted,#6c757d)]">last cash</span><span className="font-mono">{fmt(customerData.last_cash_amount)}</span></div>
        </div>
      </div>

      {/* Right: Cash then Invoices */}
      <div>
        ...
        <div className="space-y-0.5">
          ...
          {/* Cash first */}
          <div className="db-font-xs font-medium text-[var(--db-text-dim,#adb5bd)] mb-0.5">Cash</div>
          {cashEntries.length > 0 ? (
            <div className="space-y-0.5 mb-2">
              {cashEntries.map((doc: any, i: number) => (
                <div
                  key={doc.id || i}
                  className="flex justify-between py-0.5 cursor-pointer hover:bg-blue-50/50 rounded px-1 -mx-1"
                  onDoubleClick={() => { if (doc.id) window.open(`/td/cash/${doc.id}`, '_blank'); }}
            ...
            <div className="text-[var(--db-text-dim,#adb5bd)] mb-2">No cash</div>
          )}
...
export const NotesTabContent: React.FC<{
  ...
  // Comment channels always come from data.comments (JSON envelope with public/process/partner keys)
  // data.notes is a separate top-level string field on some models — handled below
