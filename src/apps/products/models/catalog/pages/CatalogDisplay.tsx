...
/**
 * CatalogDisplay - Follows 3-column standard with tab navigation
...
const STORAGE_KEY = "catalogDisplay_columnCount";
...
export default function CatalogDisplay({
  ...
  // Tab navigation
  const { activeTab, setActiveTab } = useDetailTabs("catalog_display", "actions", [
  ...
  useEffect(() => {
    if (modeProp === "edit" && dataProp?.id) {
      const fetchData = async () => {
        try {
          setLoading(true);
          const rec = await getRecord('catalog', dataProp.id);
  ...
  const handleSave = async () => {
    try {
      setIsSaving(true);
      await saveRecord('catalog', data);
      dispatch(showToast({ message: "Catalog saved successfully", type: "success" }));
      onSaved?.();
      if (currentMode === "add") {
        onCancelInline?.();
      } else {
      ...
      dispatch(showToast({ message: "Failed to save catalog", type: "error" }));
    } finally {
      setIsSaving(false);
    }
  };
      ...
      {!loading && (
        <ComponentCard>
          ...
          <div className={getGridClassName(columnCount)}>
            <HorizontalField label="Name" htmlFor="name" icon={<BookOpen size={14} />}>
              <Input
                type="text"
                id="name"
            ...
            <HorizontalField label="Category" htmlFor="category" icon={<FolderTree size={14} />}>
              <Input
                type="text"
                id="category"
            ...
            <HorizontalField label="Price" htmlFor="price" icon={<DollarSign size={14} />}>
              <Input
                type="number"
                id="price"
                placeholder="Price"
                value={data?.price ?? ""}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleFieldChange("price", Number(e.target.value))}
                disabled={currentMode === "view"}
              />
            </HorizontalField>
          </div>
