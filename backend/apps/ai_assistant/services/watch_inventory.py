...
Scans for:
1. Items below min (reorder point) — understock
2. Items above max — overstock
...
def check_understock(limit: int = 1000) -> dict:
    """Find items where on_hand < min (reorder point)."""
    from apps.products.models import Item

    observations = 0
    understock_items = []

    # Items with both on_hand and min set (include demo — users learn there)
    items = Item.objects.filter(
        is_active=True,
        quantity__on_hand__isnull=False,
        quantity__min__isnull=False,
...
def check_overstock(limit: int = 1000) -> dict:
    ...
    items = Item.objects.filter(
        is_active=True,
        quantity__on_hand__isnull=False,
        quantity__max__isnull=False,
    ).only('id', 'ida', 'quantity', 'name')[:limit]

    for item in items:
        qty = item.quantity or {}
        on_hand = qty.get('on_hand', 0) or 0
        inv_max = qty.get('max', 0) or 0
