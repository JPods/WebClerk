...
import { FaEdit, FaPlus, FaSave, FaTrash, FaEnvelope, FaPhone, FaMapMarkerAlt, FaGlobe, FaChevronDown, FaChevronRight, FaColumns, FaThLarge, FaListAlt, FaLayerGroup, FaCompressAlt, FaAlignLeft, FaStar } from "react-icons/fa";
import {
  createEmail,
  updateEmail,
  deleteEmail,
...
// Lazy load alternate layouts
const ContactDetailHorizontal = lazy(() => import("./ContactDetailHorizontal"));
const ContactDetailTwoColumn = lazy(() => import("./ContactDetailTwoColumn"));
const ContactDetailMain = lazy(() => import("./ContactDetail")); // Main best-practice layout
// ContactDetailDense disabled - file has TypeScript errors that need fixing

// ------------------------------------
// Layout Types and Selector
// ------------------------------------
type LayoutStyle = "grid" | "horizontal" | "two-column" | "compact" | "dense" | "best-practice";
...
const layoutOptions: LayoutOption[] = [
  { value: "best-practice", label: "Best Practice", icon: <FaStar className="w-4 h-4" />, description: "Enterprise UX standard - collapsible sections with labels on left" },
  { value: "grid", label: "Grid", icon: <FaThLarge className="w-4 h-4" />, description: "3-column grid layout with labels above inputs" },
  { value: "compact", label: "Compact", icon: <FaCompressAlt className="w-4 h-4" />, description: "Dense 3-column with tighter spacing" },
  { value: "dense", label: "Dense", icon: <FaAlignLeft className="w-4 h-4" />, description: "Ultra-compact with inline labels - maximum density" },
  { value: "horizontal", label: "Horizontal", icon: <FaListAlt className="w-4 h-4" />, description: "2-column with label to the left of entry area" },
  { value: "two-column", label: "Two Column", icon: <FaColumns className="w-4 h-4" />, description: "Card-based two-column layout" },
];

function LayoutSelector({ value, onChange, compact = false }: { value: LayoutStyle; onChange: (layout: LayoutStyle) => void; compact?: boolean }) {
  if (compact) {
    return (
...
export default function ContactDetail(props: ContactAddProps) {
  const {
    modeProp,
    dataProp,
    hideBreadcrumb,
  ...
  const [layout, setLayout] = useState<LayoutStyle>(() => {
    if (typeof window !== "undefined") {
      ...
      if (saved && ["grid", "horizontal", "two-column", "compact", "dense"].includes(saved)) {
  ...
  // Best Practice layout - enterprise UX standard
  if (layout === "best-practice") {
    return (
      <div>
        {!hideBreadcrumb && !inline && (
          <PageBreadcrumb pageTitle={mode === "edit" ? "Edit Contact" : mode === "view" ? "View Contact" : "Add Contact"} />
        )}
        <LayoutSelector value={layout} onChange={handleLayoutChange} compact={inline} />
        <Suspense fallback={<div className="p-4 text-center">Loading...</div>}>
          <ContactDetailMain {...props} hideBreadcrumb />
        </Suspense>
      </div>
    );
  }

  // If not grid layout, render the alternate layout
  if (layout === "compact") {
    return (
      <ContactDetailGrid
        {...props}
        layoutSelector={<LayoutSelector value={layout} onChange={handleLayoutChange} compact={inline} />}
        compactMode
      />
    );
  }

  // Dense layout disabled - ContactDetailDense.tsx has TypeScript errors
  ...
  if (layout === "horizontal") {
    return (
      <div>
        ...
        <LayoutSelector value={layout} onChange={handleLayoutChange} compact={inline} />
        <Suspense fallback={<div className="p-4 text-center">Loading...</div>}>
          <ContactDetailHorizontal {...props} hideBreadcrumb />
        </Suspense>
      </div>
    );
  }
  ...
  if (layout === "two-column") {
    return (
      <div>
        ...
        <LayoutSelector value={layout} onChange={handleLayoutChange} compact={inline} />
        <Suspense fallback={<div className="p-4 text-center">Loading...</div>}>
          <ContactDetailTwoColumn {...props} hideBreadcrumb />
        </Suspense>
      </div>
    );
  }

  // Grid layout (default) - render inline below
  return (
    <ContactDetailGrid
      {...props}
      layoutSelector={<LayoutSelector value={layout} onChange={handleLayoutChange} compact={inline} />}
...
function ContactDetailGrid({
  ...
  compactMode = false,
  denseMode = false,
}: ContactAddProps & { layoutSelector?: React.ReactNode; compactMode?: boolean; denseMode?: boolean }) {
  ...
  // Compact and Dense mode styling classes
  const gridClass = denseMode
    ? "grid grid-cols-1 md:grid-cols-4 gap-x-2 gap-y-0.5 mb-1"
    : compactMode 
    ? "grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-1 mb-2" 
    : "grid grid-cols-1 md:grid-cols-3 gap-2 mb-3";
  const formClass = denseMode
    ? "space-y-1 text-xs [&_label]:text-[10px] [&_label]:mb-0 [&_input]:h-6 [&_input]:text-xs [&_input]:py-0.5 [&_select]:h-6 [&_select]:text-xs [&_textarea]:text-xs"
    : compactMode 
    ? "space-y-2 text-sm [&_label]:text-xs [&_label]:mb-0.5 [&_input]:h-8 [&_input]:text-sm [&_select]:h-8 [&_select]:text-sm [&_textarea]:text-sm" 
    : "space-y-6";
