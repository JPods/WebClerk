...
const slugifyColumn = (rawTitle: string): string => {
  const cleaned = rawTitle.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
  return `column-${cleaned || "uncategorized"}`;
};

const buildTranslations = (entries: Array<[string, string | null | undefined]>): LocalizedTextMap | undefined => {
...
const createBoardDataFromApi = (items: ApiKanbanItem[]): BoardData => {
  ...
  items.forEach((item) => {
    ...
    const columnTitle = (item.kanban_column && String(item.kanban_column).trim()) || "Uncategorized";
    const columnId = slugifyColumn(columnTitle);
    if (!columns[columnId]) {
      columns[columnId] = { id: columnId, title: columnTitle, taskIds: [] };
