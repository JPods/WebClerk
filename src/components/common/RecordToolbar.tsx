/* LastChecked: 2026-08-03 | WhereUsed: TransactionDetail, ContactDetailJson, all ui.json pages | WhoCreated: Claude */
/**
 * RecordToolbar — standard button bar for any record detail page.
 *
 * Add, Edit, Save, Cancel, Report ▾, Model Menu ▾, Delete.
 * Model-agnostic. Used by transactions, contacts, items, orgs — any ui.json page.
 */
...
const RecordToolbar: React.FC<RecordToolbarProps> = ({
  data, currentData, modelName, layout,
  isEditing, canEdit = true, saving,
  companyInfo, logos, documentText,
  onEdit, onAddNew, onSave, onCancel,
  designMode, userRole, onToggleDesign,
}) => {
  const dispatch = useDispatch();

  // Get model-specific actions from layout JSON (if present)
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 no-print">
      {/* Record info */}
      <span className="text-sm font-mono text-slate-700 dark:text-slate-200">{data.ida || `#${data.id}`}</span>
      {data.status && (
