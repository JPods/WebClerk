/**
 * Orgs Dashboard — people, companies, and relationships.
 *
 * Comparison table: rows = models, columns = period pairs (this/last).
 * All periods visible at once — no buttons to click.
 * Communications panel: simple stat cards for email, phone, address, domain.
 * Tools panel: Statement Sorter, Team, Dedup.
 */
...
const ORG_MODELS = [
  ...
  { model: 'vendor', label: 'Vendors' },
  { model: 'manufacturer', label: 'Manufacturers' },
  { model: 'employee', label: 'Employees' },
  { model: 'rep', label: 'Reps' },
];

const COMM_MODELS = [
  { model: 'communications.Email', label: 'Emails' },
  { model: 'communications.Phone', label: 'Phones' },
  { model: 'communications.Address', label: 'Addresses' },
  { model: 'communications.Domain', label: 'Domains' },
];
...
export default function OrgsDashboard() {
  ...
  const modelNames = useMemo(() => ORG_MODELS.map(m => m.model), []);
  ...
  useEffect(() => {
    Promise.all(
      COMM_MODELS.map(({ model }) =>
        apiClient.get(`/wcapi/get/?model_name=${model}&limit=1`)
          .then(r => [model, r.data?.data?.total ?? r.data?.data?.count ?? 0] as [string, number])
          .catch(() => [model, 0] as [string, number])
      )
    ).then(pairs => {
      setCommCounts(Object.fromEntries(pairs));
  ...
  const cell = (model: string, periodKey: string) => {
    ...
    if (loading) return <span className="font-mono text-sm tabular-nums">&middot;</span>;
    return (
      <button
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <h1 className="text-xl font-bold text-gray-900 mb-1">Organizations &amp; Contacts</h1>
      ...
      <Panel title="Communications">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {COMM_MODELS.map(({ model, label }) => (
            <div
              key={model}
              onClick={() => goDb(model)}
              className="bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 cursor-pointer transition-colors hover:border-blue-400 hover:bg-blue-50 text-center"
            >
              <div className="text-2xl font-bold text-slate-900">
                {commLoading ? '...' : fmtInt(commCounts[model] ?? 0)}
              </div>
      ...
      <Panel title="Tools">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div
            onClick={() => window.open('/sort', '_blank')}
            className="border border-slate-200 rounded-lg px-4 py-3 cursor-pointer transition-colors hover:border-blue-400"
          >
            <div className="text-sm font-semibold">Statement Sorter</div>
            <div className="text-xs text-slate-500 mt-0.5">Sort bank statements by business/personal</div>
          </div>
          <div
          ...
          >
            ...
            <div className="text-xs text-slate-500 mt-0.5">Team activity and assignments</div>
          </div>
          <div
            onClick={() => navigate('/contact')}
            className="border border-slate-200 rounded-lg px-4 py-3 cursor-pointer transition-colors hover:border-blue-400"
          >
            <div className="text-sm font-semibold">Dedup</div>
            <div className="text-xs text-slate-500 mt-0.5">Find and merge duplicate contacts</div>
          </div>
        </div>
      </Panel>
    </div>
