...
def check_bad_check_history(customer_id: int) -> Dict:
    """Check customer for bounced check / bad cash history.

    Looks at financial.customer.complaints for bad_check entries
    and at Cash records with status 'bounced' or 'returned'.
    ...
    # Check Cash records for bounced/returned
    try:
        Cash = dj_apps.get_model('transactions', 'Cash')
        bounced = Cash.objects.filter(
            customer_id=customer_id,
        ...
        pass  # Cash model may not have these fields yet

    warning = bad_check_count > 0
    if warning:
        message = (
            f'Bad check warning: {bad_check_count} bounced/returned cash(s) on file'
        )
    else:
