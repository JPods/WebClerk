...
import { getTimeBillingItems } from "@/api/wcapi";
...
const RateSelect: React.FC<RateSelectProps> = ({ billable, patch, disabled }) => {
...
const BillToSection: React.FC<BillToSectionProps> = ({ billable, patch, disabled }) => {
...
export const BillableWidget: React.FC<WidgetProps> = ({ value, onChange, disabled }) => {
  ...
  return (
    <div className="space-y-1">
      ...
      <BillToSection billable={billable} patch={patch} disabled={disabled} />

      {/* Row 2: name [select] rate [$] description [text] + item ida */}
      <RateSelect billable={billable} patch={patch} disabled={disabled} />
