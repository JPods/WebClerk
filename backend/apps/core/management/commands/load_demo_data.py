...
MODEL_REGISTRY = [
    ...
    ('quote', 'apps.transactions.models', 'Quote'),
    ('quote_line', 'apps.transactions.models', 'QuoteLine'),
    ...
    ('cash', 'apps.transactions.models', 'Cash'),
...
FK_FIELDS = {
    ('quote', 'customer_id'): 'org',
    ('quote', 'vendor_id'): 'org',
    ('quote', 'manufacturer_id'): 'org',
    ('quote', 'contact_id'): 'contact',
    ('order', 'customer_id'): 'org',
    ('order', 'vendor_id'): 'org',
    ('order', 'manufacturer_id'): 'org',
    ('order', 'contact_id'): 'contact',
    ('order', 'parent_id'): 'quote',
    ('invoice', 'customer_id'): 'org',
    ('invoice', 'vendor_id'): 'org',
    ('invoice', 'manufacturer_id'): 'org',
    ('invoice', 'contact_id'): 'contact',
    ('invoice', 'parent_id'): 'order',
    ('cash', 'customer_id'): 'org',
    ('cash', 'vendor_id'): 'org',
    ('cash', 'contact_id'): 'contact',
    ('cash', 'invoice_id'): 'invoice',
    ('quote_line', 'quote_id'): 'quote',
    ('quote_line', 'item_fk_id'): 'item',
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--preserve-ids', action='store_true',
                            help='Keep original primary keys (bundles packed with --all; target tables must not '
                                 'already use those ids). Chosen automatically for mode=preserve_ids bundles.')
    ...
    def handle(self, *args, **options):
        ...
        if not options.get('settings_only'):
            ...
            if options.get('preserve_ids') or bundle.get('mode') == 'preserve_ids':
                self._load_preserved(data, bundle.get('models') or list(data.keys()), demo_source,
                                     dry_run=options['dry_run'])
            else:
                self._load_data(data, demo_source, dry_run=options['dry_run'])

    def _load_preserved(self, data, labels, demo_source, dry_run=False):
        """Load rows with their original primary keys so every FK and JSON id stays valid.

        Existing rows with the same uuid are skipped. An id already used by a different
        record is a conflict: nothing is written (fail fast, whole load is atomic).
        """
        from django.apps import apps
        from django.core.management.base import CommandError
        from django.core.management.color import no_style
        from django.db import connection, transaction
        from apps.core.services.demo_bundle import INFRASTRUCTURE_MODELS
        ...
        with transaction.atomic():
            for label in labels:
                ...
                existing_uuids = {str(u) for u in Model.objects.filter(uuid__in=uuids).values_list('uuid', flat=True)}
                taken_ids = set(Model.objects.filter(pk__in=ids).exclude(uuid__in=uuids).values_list('pk', flat=True))
                if taken_ids:
                    raise CommandError(
                        f"{label}: ids {sorted(taken_ids)[:10]} are used by other records. "
                        f"Load into an empty database, or omit --preserve-ids to remap by uuid."
                    )
                objs = []
                for row in rows:
                    if str(row.get('uuid')) in existing_uuids:
                        continue
                    values = {k: v for k, v in row.items() if k in columns}
                    if 'refs' in columns and label not in INFRASTRUCTURE_MODELS:
                        refs = dict(values.get('refs') or {})
                ...
                total += len(objs)
                self.stdout.write(f"  {label}: {len(objs)} {'would load' if dry_run else 'loaded'}, "
                                  f"{len(rows) - len(objs)} already present")
            if dry_run:
