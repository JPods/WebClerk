...
def _aggregate_setting(model_name: str, lock: bool = False):
    """The per-model aggregate Setting, created on first use.

    Settings refuse creation unless authorized; this one is Alice's own
    system record, so its creation is authorized here. (Plain get_or_create
    was refused on every transaction save, and the aggregates never started.)
    """
    from apps.core.models import Setting
    qs = Setting.objects.select_for_update() if lock else Setting.objects
    setting = qs.filter(purpose=SETTING_PURPOSE, parent_model=model_name).first()
    if setting is None:
        setting = Setting(purpose=SETTING_PURPOSE, parent_model=model_name,
                          name=f'alice:aggregates:{model_name}', scope='system', config={})
...
def apply_delta(model_name: str, old_totals: dict, new_totals: dict,
    ...
    with transaction.atomic():
        setting = _aggregate_setting(model_name, lock=True)
...
def refresh_aggregates(model_name: str) -> dict:
    ...
    with transaction.atomic():
        setting = _aggregate_setting(model_name)
        setting.config = config
        setting._setting_update_authorized = True
        setting.save(update_fields=['config'])
