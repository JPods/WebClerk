...
export default function SettingDisplay({
  ...
  useEffect(() => {
    const fetchFields = async () => {
      try {
        const rec = await getRecord('setting', 1);
  ...
  useEffect(() => {
    if (modeProp === "edit" && dataProp?.id) {
      const fetchData = async () => {
        try {
          setLoading(true);
          const rec = await getRecord('setting', dataProp.id);
  ...
  const handleSave = async () => {
    try {
      setLoading(true);
      await saveRecord('setting', data);
      dispatch(showToast({ message: "Setting saved successfully", type: "success" }));
      onSaved?.();
    } catch (error) {
      console.error("Failed to save", error);
      dispatch(showToast({ message: "Failed to save setting", type: "error" }));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-4 space-y-4">
      <h2 className="text-xl font-semibold">
        <span className="mr-2 px-1.5 py-0.5 text-[10px] font-mono font-normal tracking-wide uppercase bg-indigo-100 text-indigo-600 dark:bg-indigo-900/40 dark:text-indigo-300 rounded">SettingDisplay</span>
        {modeProp === "add" ? "Add Setting" : modeProp === "edit" ? "Edit Setting" : "View Setting"}
      </h2>
