...
def backfill_company_snapshots(apps, schema_editor):
    ...
    # Fresh databases have nothing to backfill, and org columns this SQL reads
    # (phone, domain, ...) are only added by later migrations — skip when empty.
    cursor.execute("SELECT EXISTS (SELECT 1 FROM transactions_payment) OR EXISTS (SELECT 1 FROM receipt)")
