...
def quote_trip(contact, origin, destination, network_id):
    """Authoritative trip price for this rider. Returns (quote dict, None) or (None, (message, status))."""
    ...
    item = get_trip_item(origin, destination, network_id)
    if not item:
        return None, (f"No route from {origin} to {destination}", 422)
    try:
        base_price, currency = get_price(item, price_level)
    except ValueError as exc:
