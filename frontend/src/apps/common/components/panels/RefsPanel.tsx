...
const LinkList: React.FC<LinkListProps> = ({
  ...
  return (
    <div className="last:border-b-0 py-2 db-border-bottom-light">
      {/* Header */}
      <div
        className="flex items-center gap-2 cursor-pointer"
        onClick={() => setIsExpanded(!isExpanded)}
      >
        <span className="db-text-dim">{icon}</span>
        <span className="text-xs font-semibold capitalize db-text">
          {type.replace(/_/g, " ")}
        </span>
        <span className="text-xs db-text-dim">({links.length})</span>
        ...
        <div className="mt-2 ml-6 space-y-1">
          {links.map((link, idx) => (
            <div key={link.id || idx} className="flex items-center gap-2 group">
              <span className="text-xs font-mono db-text-muted">
                #{link.id}
              </span>
              {link.ida && (
                <span className="text-xs db-text-dim">[{link.ida}]</span>
              )}
              <span
                className={`text-xs flex-1 truncate db-text ${
              ...
              {link.purpose && (
                <span className="text-xs px-1.5 py-0.5 rounded db-surface-alt-muted">
                  {link.purpose}
                </span>
              )}
              {canEdit && onDelete && (
                <button
                  ...
                  className="p-1 opacity-0 group-hover:opacity-100 rounded db-text-red"
                  title="Remove link"
                >
                  <FaTrash size={10} />
                </button>
...
const Lineage: React.FC<LineageProps> = ({
  ...
  return (
    <div className="py-2 db-border-bottom-light">
      <div className="flex items-center gap-2 mb-2">
        <FaProjectDiagram className="db-text-dim" size={12} />
        <span className="text-xs font-semibold db-text">
          Lineage
        </span>
      </div>
      <div className="ml-6 space-y-1">
        {parent_id && parent_model && (
          <div className="flex items-center gap-2 text-xs">
            <span className="db-text-muted">Parent:</span>
            <span
              className={`db-text ${
                navigable && onNavigate
                  ? "cursor-pointer hover:underline"
        ...
        {source_id && source_type && (
          <div className="flex items-center gap-2 text-xs">
            <span className="db-text-muted">Source:</span>
            <span
              className={`db-text ${
                navigable && onNavigate
                  ? "cursor-pointer hover:underline"
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          <FaLink className="db-text-accent" size={14} />
          <h3 className="text-sm font-semibold db-text">
            {title}
          </h3>
          <span className="px-1.5 py-0.5 text-xs rounded db-surface-alt-text">
            Admin
          </span>
        ...
        <div className={`${compact ? "p-2" : "p-4"}`}>
          ...
          {isAdmin && (
            <details className="mt-4">
              <summary className="text-xs cursor-pointer db-text-dim">
                View raw JSON
              </summary>
              <pre
                className="mt-2 text-xs font-mono p-2 rounded overflow-x-auto max-h-48 whitespace-pre-wrap db-surface-alt-text"
              >
                <HighlightedJson json={JSON.stringify(data, null, 2)} />
              </pre>
            </details>
          )}
        </div>
