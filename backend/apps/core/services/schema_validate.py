...
def enforce_pjpv_schemas(
    model_filter: str | None = None,
    dry_run: bool = False,
    stdout=None,
    style=None,
) -> dict:
    """PJPV enforcement: reshape all JSON envelopes to current Pydantic schema.

    For each record in each model:
    1. Parse each JSON envelope through its Pydantic schema
    2. Unknown keys → moved to prefs.userdefined (user extensions stay accessible)
    3. Missing keys → filled with schema defaults
    4. Type mismatches → coerced where safe, flagged where not

    User-defined data is NEVER lost — it moves to prefs.userdefined{} where
    Alice can track it and users can still access it.

    Args:
        model_filter: Check only this model key (e.g. 'contact')
        dry_run: Report only, don't save
        stdout: Management command stdout for progress output
        style: Management command style for colored output

    Returns:
        Summary dict with counts of records checked, reshaped, errors.
    """
    ...
    result = {
        'total_records': 0,
        'reshaped': 0,
        'unknown_keys_moved': 0,
        'missing_keys_filled': 0,
        'errors': 0,
    }

    defaults = _get_defaults()

    # Envelope fields and their default structures
    ENVELOPE_FIELDS = ('metadata', 'refs', 'prefs', 'comments', 'config')
    ...
    for key, meta in sorted(MODEL_REGISTRY.items()):
        ...
        # ── WALL: Setting records are NEVER bulk-modified ──
        # Settings drive all layouts, behaviors, selectlists, and schema maps.
        # Bulk changes destroy the UI. Any Setting modification must be
        # individual, reviewed, and confirmed. Use --model setting with
        # the management command's --records --fix mode instead.
        if key == 'setting':
            if stdout:
                stdout.write(f"\n  setting: PROTECTED — skipped (use --model setting --records --fix for individual review)")
            continue

        # Load Pydantic schemas for this model
        schemas = _load_model_schemas(key)

        # Skip config enforcement on models whose ConfigSchema uses extra="allow"
        config_schema = schemas.get('config')
        if config_schema:
            model_config = getattr(config_schema, 'model_config', {})
            config_extra = model_config.get('extra', 'ignore')
            if not isinstance(model_config, dict):
                cfg_cls = getattr(config_schema, 'Config', None)
                config_extra = getattr(cfg_cls, 'extra', 'ignore') if cfg_cls else 'ignore'
            if config_extra == 'allow':
                schemas.pop('config', None)

        try:
            record_count = Model.objects.count()
        except Exception as e:
            if stdout:
                stdout.write(f"\n  {key}: SKIP — {type(e).__name__}: {e}")
            continue
        ...
        if stdout:
            stdout.write(f"\n  {key} ({record_count} records)...")

        model_reshaped = 0
        try:
            records_list = list(Model.objects.all()[:5000])
        except Exception as e:
            if stdout:
                stdout.write(f"    SKIP — query error: {type(e).__name__}: {e}")
            continue
        ...
        for record in records_list:
          try:
            ...
            for field_name in ENVELOPE_FIELDS:
                ...
                # If we have a Pydantic schema for this field, validate
                schema_cls = schemas.get(field_name)
                if schema_cls:
                    try:
                        schema_cls.model_validate(value)
                    except ValidationError as e:
                        # Unknown keys — move to prefs.userdefined
                        extra_fields = set()
                        for error in e.errors():
                            if error['type'] == 'extra_forbidden':
                                field_path = '.'.join(str(p) for p in error['loc'])
                                extra_fields.add(error['loc'][0] if error['loc'] else '')

                        if extra_fields:
                            # Move unknown keys to prefs.userdefined
                            prefs = getattr(record, 'prefs', None) or {}
                            ud = prefs.setdefault('userdefined', {})
                            moved_from = ud.setdefault(f'_moved_from_{field_name}', {})

                            for extra_key in extra_fields:
                                if extra_key in value:
                                    moved_from[extra_key] = value.pop(extra_key)
                                    result['unknown_keys_moved'] += 1
                                    changed = True

                            if field_name != 'prefs':
                                setattr(record, 'prefs', prefs)
                                if 'prefs' not in update_fields:
                                    update_fields.append('prefs')

                            if stdout and extra_fields:
                                prefix = "Would move" if dry_run else "Moved"
                                stdout.write(
                                    f"    {prefix} {len(extra_fields)} unknown key(s) "
                                    f"from {key}#{record.pk}.{field_name} → "
                                    f"prefs.userdefined._moved_from_{field_name}"
                                )

                if changed and field_name not in update_fields:
                    update_fields.append(field_name)
            ...
            if changed:
                ...
                if not dry_run and update_fields:
                    try:
                        record.save(update_fields=update_fields + ['dt_modified'])
                    except Exception as exc:
                        result['errors'] += 1
                        if stdout:
                            stdout.write(f"    ERROR saving {key}#{record.pk}: {exc}")
          except Exception as exc:
            result['errors'] += 1
            if stdout:
                stdout.write(f"    ERROR processing {key}#{getattr(record, 'pk', '?')}: {type(exc).__name__}: {exc}")

        result['reshaped'] += model_reshaped
        if stdout and model_reshaped:
            icon = '(dry run)' if dry_run else ''
            stdout.write(f"    → {model_reshaped} reshaped {icon}")
