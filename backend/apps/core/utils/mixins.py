...
class SettingsDrivenCRUDMixin:
    ...
    def _model_key(self, model: Type[Model]) -> str:
        """The registry key a Setting names (proposal_line), not Django's model_name
        (proposalline). Setting.clean() validates parent_model against the registry, so
        looking up the Django name could never match a valid row (found 2026-09-17)."""
        django_name = getattr(model._meta, "model_name", model.__name__).lower()
        try:
            from apps.core.constants.model_registry import get_model_meta
            meta = get_model_meta(django_name) or get_model_meta(model.__name__)
            if meta is not None and getattr(meta, "key", None):
                return meta.key
        except Exception:
            pass
        return django_name
    ...
    def _roles_for(self, request) -> List[str]:
        """Roles for this request, most specific first — the first one with a block wins.

        Contact.role (user | employee | admin) is how WC3 assigns a role; it is separate
        from is_staff. Reading only is_superuser/is_staff/groups gave a contact whose role
        is admin the USER rules (found 2026-09-17).
        """
        roles: List[str] = []
        ...
        record_role = (getattr(user, "role", "") or "").strip().upper()
        if record_role and record_role not in roles:
            # admin by record, staff by flag — same block
            roles.append(record_role)
    ...
    def get_view_edit_allowlists(
        ...
        # A Setting row carries its payload in config; the JSON fallback file used
        # "data". Reading only "data" made every database policy inert — empty view/edit
        # lists, which every caller reads as "no restriction" (found 2026-09-17).
        data_map = rec.get("config") or rec.get("data") or {}
