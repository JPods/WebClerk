...
class ContactAdmin(SchemaLabelsAdminMixin, BaseUserAdmin):
    ...
    raw_id_fields = ('customer', 'vendor', 'manufacturer', 'rep')
...
class PendingAdmin(ScalarFirstFieldsetMixin, SchemaLabelsAdminMixin, admin.ModelAdmin):
    ...
    def _data_field(self, obj, key):
        # The quantity buckets are written to .changes by the pending builders
        # (see line_manage._create_pending_for_line_add). This read .config,
        # which those builders leave empty, so every bucket column showed '-'
        # even when the record held a value. .config is kept as a fallback for
        # any older row that used it.
        val = (obj.changes or {}).get(key)
        if val is None:
