...
/** WCHQ base URL — all training content, flowcharts, and shared resources served from here. */
const WCHQ_URL = "https://www.webclerk.com";
...
export default function AliceDashboard() {
  ...
  const [importConnection] = useState<any>(null);
            ...
            <div className="p-4 border-t border-indigo-200 dark:border-indigo-800">
              {[
                ...
                { title: 'Financial', charts: [
                  { name: 'Cash & GL', file: 'wc3-cash-gl', desc: 'Cash → journals → GL; aging; erosion detection' },
                      ...
                      {section.charts.map((chart) => (
                        <tr key={chart.file} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                          <td className="py-1.5 pr-3">
                            <a href={`${WCHQ_URL}/static/training/flowcharts/${chart.file}.pdf`} target="_blank" rel="noopener noreferrer"
    ...
    "webclerk": {
      ...
      "args": ["/path/to/scripts/wc_mcp_server.py"]
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              ...
              <div className="border border-gray-200 dark:border-gray-700 rounded p-2">
                <div className="font-semibold text-gray-700 dark:text-gray-300">Cash Gateways</div>
                <div className="text-[10px] text-gray-500">Stripe, PayPal — process cash, check status. MCP server wraps their SDK.</div>
              </div>
              <div className="border border-gray-200 dark:border-gray-700 rounded p-2">
...
└──────────────┘           └──────────────┘           └──────┬──────┘
      ...
      {activeTab === 'reports' && (
        <div className="p-4 space-y-4 text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mt-4">
            {[
              { name: 'Invoice', model: 'invoice', cat: 'customer_facing', desc: 'Line items, totals, cash terms' },
          ...
          <div className="max-h-[70vh] overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-lg">
            {[
              ...
              { section: 'Transactions', docs: [
                ...
                { ida: 'README-CASH-GL', title: 'Cash & GL', desc: 'Cash → journals → GL; aging; erosion detection' },
                { ida: 'README-PENDING', title: 'Pending Policy', desc: 'When changes queue vs apply immediately' },
                { ida: 'README-TERMS-LEDGERS', title: 'Terms & Ledgers', desc: 'Cash terms, aging buckets, ledger entries' },
      ...
      {activeTab === 'parades' && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {[
              { label: 'Panel Parade', desc: 'db.panel columns for every linked record panel', url: '/component-parade?type=panel' },
              { label: 'Form Parade', desc: 'db.form layouts — App view detail sections', url: '/component-parade?type=form' },
              { label: 'Detail Parade', desc: 'db.detail field order for Admin view', url: '/component-parade?type=detail' },
              { label: 'List Parade', desc: 'db.list column order for all list views', url: '/component-parade?type=list' },
              { label: 'Card Parade', desc: 'db.card layouts for Kanban and compact views', url: '/component-parade?type=card' },
