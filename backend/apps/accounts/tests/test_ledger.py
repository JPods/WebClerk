...
class TestComputeSchedule:
    ...
    def test_single_cash_net30(self, term_net30):
...
class TestCreateLedgerRecords:
    ...
    def test_no_term_uses_company_default(self, org):
        """Invoice without terms → company default term (N30); never no due date."""
        from apps.transactions.models import Invoice
        from apps.accounts.services.terms_ledger import apply_terms_for_invoice
    ...
    def test_resolves_term_by_name(self, org, term_net30):
        """Term resolution via prefs.terms.name fallback."""
        from apps.transactions.models import Invoice
        from apps.accounts.services.terms_ledger import apply_terms_for_invoice
...
class TestCashLedger:
    """Tests for record_cash() — negative value ledger."""

    def test_creates_negative_ledger(self, invoice):
        from apps.accounts.services.terms_ledger import record_cash
        from apps.accounts.models import Ledger
...
class TestAgingBuckets:
    ...
    def test_cash_reduces_balance(self, invoice, term_net30):
        """Invoice $1000 - Cash $400 = $600 balance."""
        from apps.accounts.services.terms_ledger import apply_terms_for_invoice, record_cash
        from apps.accounts.services.ledger_balance import calculate_aging_buckets
        from datetime import date

        apply_terms_for_invoice(invoice, term=term_net30, replace=True)
        record_cash(invoice=invoice, amount=Decimal('400'), dt_paid=datetime.now(timezone.utc))
