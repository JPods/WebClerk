        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6">
          ...
          {isEditing && !customerInfo ? (
            <CustomerSelector
              ...
              onChange={(p) =>
                onChange && onChange("customer_id", p?.id ?? null)
              }
              label="Customer"
              required
            />
          ...
          <div className="relative max-h-[480px] overflow-auto">
            <table className="w-full text-xs">
              <thead className="bg-slate-50 dark:bg-slate-900/50 sticky top-0 z-10">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide w-16">
                    #
                  </th>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-700 text-xs">
                {lines.map((line, idx) => (
                  <tr
                    key={line.id ?? idx}
                    className={`${
                      idx % 2 === 1 ? "bg-slate-50 dark:bg-slate-900/30" : ""
                  ...
                  >
                    ...
                    <td className="px-2 py-1 text-sm font-mono font-medium text-slate-900 dark:text-white">
                      {line.item?.ida_item ?? "--"}
                    </td>
                    <td className="px-2 py-1 text-sm text-slate-600 dark:text-slate-300">
                      {line.item?.description ?? "--"}
                    </td>
                    <td className="px-2 py-1 text-sm text-right text-slate-900 dark:text-white">
                      {formatNumber(line.quantity?.ordered)}
                    </td>
                    <td className="px-2 py-1 text-sm text-right text-slate-600 dark:text-slate-300">
                    ...
                    <td className="px-2 py-1 text-sm text-right font-medium text-slate-900 dark:text-white">
