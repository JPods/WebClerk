...
export default function GLJournalDisplay({
  inline = false,
  modeProp,
  dataProp,
  onSaved,
  onCancelInline,
}: GLJournalDisplayProps) {
  ...
  useEffect(() => {
    const fetchFields = async () => {
      try {
        const rec = await getRecord('gl_journal', 1);
        const record = rec.record || rec;
        setFields(Object.keys(record));
      } catch (error) {
        console.error("Failed to fetch fields", error);
        setFields(['id', 'date', 'description', 'amount']); // fallback
      }
    };
    fetchFields();
  }, []);
  ...
  useEffect(() => {
    if (modeProp === "edit" && dataProp?.id) {
      const fetchData = async () => {
        try {
          setLoading(true);
          const rec = await getRecord('gl_journal', dataProp.id);
  ...
  const handleSave = async () => {
    try {
      setLoading(true);
      await saveRecord('gl_journal', data);
      dispatch(showToast({ message: "GL Journal saved successfully", type: "success" }));
      onSaved?.();
    } catch (error) {
      console.error("Failed to save", error);
      dispatch(showToast({ message: "Failed to save gl journal", type: "error" }));
    } finally {
      setLoading(false);
    }
  };
