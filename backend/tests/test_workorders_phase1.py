...
class WorkOrderPhase1Tests(TestCase):
    def setUp(self):
        ...
        make_setting(purpose='wc:view_edit', parent_model='workorder', is_active=True,
                               config={'USER': {'view': ['id','ida','status'], 'edit': ['id','status']}})
        make_setting(purpose='wc:view_edit', parent_model='workorder_line', is_active=True,
    ...
    def test_unknown_filter_rejected_with_coaching(self):
        """An unknown filter is an error, not a silent wider result set (Bill, 2026-09-17).
        ...
        self.assertIn('unknown_field', message)
        self.assertIn('Fields you can filter include', message)
        self.assertTrue(
            AliceObservation.objects.filter(
                dedup_key='unknown_filter:workorderline:unknown_field').exists())
    ...
    def test_json_envelope_path_is_coached(self):
        ...
        self.assertEqual(resp.status_code, 400)
        self.assertIn("filter it as 'quantity__active'", resp.json().get('message', ''))

    def test_json_envelope_filter_works(self):
        """quantity__active filters on the value inside the JSON envelope."""
        self.line.quantity = {'staged': 3, 'active': 3}
