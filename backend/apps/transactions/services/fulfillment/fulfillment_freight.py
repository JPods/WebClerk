...
def shipping_service_for(ship_via: str) -> dict | None:
    """The wc:shipping_service entry a transaction's ship_via names.

    Matches name, carrier_code, or account, case-insensitive ("USPS" = "usps").
    """
    wanted = (ship_via or '').strip().lower()
    if not wanted:
        return None
...
def percent_of_goods_shipping(ship_via: str, goods: Decimal) -> Decimal | None:
    """Customer shipping charge for a service priced as a percent of goods.

    Service entry: {"rate_method": "percent_of_goods", "rate_percent": 4}.
    Returns None when the service is not priced that way.
    """
    svc = shipping_service_for(ship_via)
    if not svc or svc.get('rate_method') != 'percent_of_goods':
        return None
    pct = Decimal(str(svc.get('rate_percent') or 0))
    return (goods * pct / Decimal(100)).quantize(Decimal('0.01'))
