...
Current definitions: readmes/transactions/line-quantity.md (supersedes the notes below
where they differ — staged is a creation snapshot; remaining = active − Σ children.active).

1. USER INPUT
   - User always enters `active` as the primary quantity input
   - staged snapshot = active when a standalone line is created
...
class TestNormalizeQuantityMap:
    ...
    def test_invoice_standalone_remaining_equals_active(self):
        """Standalone invoice at creation: staged snapshot = active, remaining = active."""
        result = normalize_quantity_map(
            {"active": 5},
            "invoice",
            is_new=True,
    ...
    def test_receipt_standalone_remaining_equals_active(self):
        """Standalone receipt at creation: staged snapshot = active, remaining = active."""
        result = normalize_quantity_map(
            {"active": 8},
            "receipt",
            is_new=True,
    ...
    def test_null_staged_takes_snapshot_on_creation(self):
        """On creation, a missing staged takes the snapshot of active."""
        result = normalize_quantity_map(
    ...
    def test_staged_snapshot_not_retaken_on_existing_line(self):
        """staged is a snapshot: saving an existing line never re-mirrors active."""
        result = normalize_quantity_map({"staged": 0, "active": 5}, "order")
        assert result["staged"] == 0
        result = normalize_quantity_map({"staged": 3, "active": 5}, "order")
        assert result["staged"] == 3

    def test_remaining_subtracts_children_sum(self):
        """remaining = active − Σ children.active, signed, never floored."""
        assert normalize_quantity_map({"active": 10}, "order", children_sum=6)["remaining"] == 4.0
        assert normalize_quantity_map({"active": 10}, "order", children_sum=12)["remaining"] == -2.0
        assert normalize_quantity_map({"active": -10}, "order", children_sum=-6)["remaining"] == -4.0

    def test_is_complete_forces_remaining_zero(self):
        result = normalize_quantity_map({"active": 10, "is_complete": True}, "order", children_sum=6)
        assert result["remaining"] == 0

    def test_unknown_keys_dropped(self):
        """children_active, parent_* and legacy keys never persist (Rule 6)."""
        result = normalize_quantity_map(
            {"active": 5, "children_active": {"sum": 3}, "parent_remaining": 7,
             "parent_active": 9, "ordered": 5, "converted_from_order": {}},
            "invoice",
        )
        for key in ("children_active", "parent_remaining", "parent_active", "ordered", "converted_from_order"):
