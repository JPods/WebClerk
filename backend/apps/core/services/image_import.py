...
Two file categories:
  Images    — product photos, lifestyle shots → resize to tn/md/hr .jpg
...
def process_source_files(
    ...
    For images: resize to tn/md/hr .jpg, store in image library.
...
def _process_and_store_image(
    ...
    # Save hr — convert to RGB .jpg
    img = Image.open(io.BytesIO(hr_bytes))
    if img.mode in ("RGBA", "P", "LA"):
        img = img.convert("RGB")

    hr_path = dir_path / "hr.jpg"
    img.save(str(hr_path), "JPEG", quality=90, optimize=True)
    result["hr"] = str(hr_path.relative_to(MEDIA_ROOT))
