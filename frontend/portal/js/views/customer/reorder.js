...
function render(el, cart) {
  ...
  document.getElementById('btn-submit-order').addEventListener('click', async () => {
    ...
    try {
      // Items and quantities only. The server sets customer (from the login's
      // org), status, and prices — client-side prices are display-only.
      const lines = cart.map((c) => ({
        item: { item_id: c.item_id },
        quantity: { active: c.quantity },
      }));
