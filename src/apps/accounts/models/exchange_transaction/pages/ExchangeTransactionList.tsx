...
import ExchangeTransactionDisplay from "./ExchangeTransactionDisplay";
...
export default function ExchangeTransactionList() {
  ...
  const getExchangeTransactionData = useCallback(async () => {
    try {
      setLoading(true);
      const list = await getRecords('exchange_transaction');
  ...
  const handleDelete = async (row: any) => {
    if (window.confirm(`Delete exchange transaction ${row.id}?`)) {
      try {
        await deleteRecord('exchange_transaction', row.id);
  ...
  const userColumns: TableColumn<any>[] = [
    ...
    { name: "Currency", selector: (row) => row.currency || "--", sortable: true, width: "20%" },
