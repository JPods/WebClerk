...
from apps.jpods.services.pricing import quote_trip
...
# ── Rider selection (staff only — returns rider identities and tokens) ────────

class ContactsView(APIView):
    """GET /wcapi/jpods/ui/contacts/ — contacts with a myCarryOn entry. Staff only."""
...
class IdentifyView(APIView):
    ...
    Staff only (demo rider switching). Riders get tokens through myCarryOn, never here.
    """
    permission_classes = [IsAdminUser]
...
class UIPriceView(APIView):
    ...
    def post(self, request):
        ...
        quote, err = quote_trip(contact, origin, destination, network_id)
        if err:
            return Response({"error": err[0]}, status=err[1])
        customer = quote["customer"]
        item = quote["item"]
        price_level = quote["price_level"]
        currency = quote["currency"]
        final_price = quote["price"]
        discount_label = quote["discount_applied"]
...
class TravelView(APIView):
    ...
    Body: origin_station_id, destination_station_id, network_id, dispatch_target.
    Price is recomputed server-side — client-supplied price/item/discount are ignored.
    ...
    def post(self, request):
        ...
        return Response({
            ...
            "status":                 "dispatched" if dispatch_result.get("ok") else "invoiced",
