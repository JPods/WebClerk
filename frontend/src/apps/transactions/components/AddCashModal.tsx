...
/**
 * CashCard — Enter a new cash or apply an existing one.
 *
 * Top:    Cash entry fields (amount, method, reference, date, notes)
 * Bottom: Panel of available cash from this customer — click to apply
 *
 * From an Order: enter cash → amount = available (not yet applied to invoice)
 * From an Invoice: enter new OR pick existing → applies via a Pending application record
 */
...
const AddCashModal: React.FC<AddCashModalProps> = ({
  ...
  const [cashMethods, setCashMethods] = useState<CashMethod[]>([]);
  const [availableCashEntries, setAvailableCashEntries] = useState<AvailableCash[]>([]);
  const [loadingCashEntries, setLoadingCashEntries] = useState(false);
  const [saving, setSaving] = useState(false);

  // New cash form state
  const [amount, setAmount] = useState('');
  const [cashMethodId, setCashMethodId] = useState<number | null>(null);
  ...
  // Apply existing cash state
  const [selectedCash, setSelectedCash] = useState<AvailableCash | null>(null);
  const [applyAmount, setApplyAmount] = useState('');

  // Adjustment controls
  const [, setDiscountPct] = useState('');
  const [, setDiscountAmt] = useState('');
  const [dismissBalance, setDismissBalance] = useState(false);

  // Load cash methods + available cash when modal opens
  useEffect(() => {
    if (!isOpen) return;

    getRecords('cash_method', { is_active: true, limit: 50 })
      .then((response) => {
        const methods = response?.results || response || [];
        setCashMethods(methods);
        if (methods.length > 0 && !cashMethodId) {
          setCashMethodId(methods[0].id);
        }
      })
      .catch(console.error);

    // Load available cash for this customer (invoice or order context)
    if ((invoice_id || order_id) && customer_id) {
      ...
      getRecords('cash', {
        ...
        .catch(console.error)
        .finally(() => setLoadingCashEntries(false));
    }
  ...
  // Load prior cash for this order/invoice and compute received total
  ...
  const handleSaveNew = useCallback(async () => {
    ...
    setSaving(true);
    try {
      // parent_id/parent_model = where this cash was entered
      ...
      const cashData: Record<string, unknown> = {
        ...
        dt_cash: new Date(cashDate).toISOString(),
      ...
      if (invoice_id) cashData.invoice_id = invoice_id;

      const saved = await saveRecord('cash', cashData) as any;
      const newCashId = saved?.record?.id || saved?.id;

      // If from an invoice, apply the cash immediately (with adjustments)
      if (invoice_id && newCashId) {
        ...
        await manageAction('apply_cash_to_invoice', applyParams);

        const parts = [`Cash of ${formatCurrency(parseFloat(amount))} applied`];
        if (dismissBalance) parts.push('balance dismissed (write-off cash)');
        dispatch(showToast({
          message: parts.join(' + '),
          type: 'success',
        }));
        ...
        dispatch(showToast({
          message: `Cash of ${formatCurrency(parseFloat(amount))} recorded against order`,
          type: 'success',
        }));
      } else {
        dispatch(showToast({
          message: `Cash of ${formatCurrency(parseFloat(amount))} recorded`,
          type: 'success',
        }));
      }
      onCashAdded?.();
      ...
      const msg = error.response?.data?.detail || error.response?.data?.message || 'Failed to create cash';
      dispatch(showToast({ message: msg, type: 'error' }));
    } finally {
      setSaving(false);
    }
  }, [amount, contact_id, customer_id, cashDate, cashMethodId, referenceNumber, notes, order_id, invoice_id, dispatch, onCashAdded, onClose]);

  // ── Apply existing cash ────────────────────────────────────────
  const handleApplyExisting = useCallback(async () => {
    if (!selectedCash || !invoice_id) return;
    const amt = parseFloat(applyAmount);
    if (isNaN(amt) || amt <= 0) return;

    setSaving(true);
    try {
      await manageAction('apply_cash_to_invoice', {
        cash_id: selectedCash.id,
        invoice_id: invoice_id,
        amount: amt,
      });
      dispatch(showToast({
        message: `Applied ${formatCurrency(amt)} from ${selectedCash.reference_number || selectedCash.ida || `#${selectedCash.id}`}`,
        type: 'success',
      }));
      onCashAdded?.();
      onClose();
    } catch (error: any) {
      const msg = error.response?.data?.error || error.message || 'Failed to apply cash';
      dispatch(showToast({ message: msg, type: 'error' }));
    } finally {
      setSaving(false);
    }
  }, [selectedCash, invoice_id, applyAmount, dispatch, onCashAdded, onClose]);

  if (!isOpen) return null;

  const applyAmountNum = parseFloat(applyAmount) || 0;
  const selectedAvail = selectedCash
    ? (selectedCash.available ?? 0)
    : 0;
  const isValidApply = selectedCash && applyAmountNum > 0
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-lg mx-4 max-h-[calc(100vh-4rem)] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select data-wc="select"
                value={cashMethodId || ''}
                onChange={(e) => setCashMethodId(e.target.value ? parseInt(e.target.value) : null)}
          ...
          {(() => {
            ...
            return (
              <>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        value={amount}
                        onChange={(e) => { setAmount(e.target.value); setAmountTouched(true); setSelectedCash(null); }}
                        placeholder="0.00"
                ...
                {/* Dismiss row */}
                <label className="flex items-center gap-1.5 cursor-pointer" title="Creates a write-off cash for the remaining balance">
                  <input
                    type="checkbox"
                  ...
                  <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 p-3 text-sm font-mono space-y-1">
                    ...
                    {remainder > 0 && dismissBalance && (
                      <div className="flex justify-between text-amber-600">
                        <span>Write-off — separate cash</span>
                ...
                <button
                  ...
                  disabled={saving || !amount || parseFloat(amount) <= 0 || !!selectedCash}
                ...
                >
                  {saving && !selectedCash ? (
                    ...
                    <><FaCheck size={14} /> Enter Cash {amount ? formatCurrency(parseFloat(amount)) : ''}</>
                  )}
                </button>
              </>
            );
        ...
        {/* Available cash panel — invoice context only */}
        {(invoice_id || order_id) && (
          <div className="border-t border-slate-200 dark:border-slate-700">
            <div className="px-6 py-3">
              <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                Available Cash from {customer_name || 'Customer'}
              </h3>
            </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {availableCashEntries.map((p) => {
                      ...
                      // Highlight if this cash was entered on the current document
                      ...
                      return (
                        <tr
                          key={p.id}
                          onClick={() => {
                            setSelectedCash(isSelected ? null : p);
                        ...
                        >
                          ...
                          <td className="py-1.5 text-slate-500 dark:text-slate-400">
                            {formatDt(p.dt_cash || p.dt_created, 'date')}
                          </td>
                        </tr>
                      );
                    })}
