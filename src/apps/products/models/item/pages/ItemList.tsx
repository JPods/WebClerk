...
export default function ItemList() {
  ...
  const getItemData = useCallback(async () => {
    try {
      setLoading(true);
      const list = await getRecords('item');
  ...
  const handleDelete = async (row: any) => {
    if (window.confirm(`Delete item ${row.id}?`)) {
      try {
        await deleteRecord('item', row.id);
  ...
  const userColumns: TableColumn<any>[] = [
    ...
    { name: "SKU", selector: (row) => row.sku || "--", sortable: true, width: "20%" },
    { name: "Price", selector: (row) => row.price || "--", sortable: true, width: "20%" },
    { name: "Stock", selector: (row) => row.stock || "--", sortable: true, width: "20%" },
  ];
