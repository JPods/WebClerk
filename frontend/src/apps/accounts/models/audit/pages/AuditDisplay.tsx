...
export default function AuditDisplay({
  ...
  useEffect(() => {
    const fetchFields = async () => {
      try {
        const rec = await getRecord('audit', 1);
        const record = rec.record || rec;
        setFields(Object.keys(record));
      } catch (error) {
        console.error("Failed to fetch fields", error);
        setFields(['id', 'name', 'description']); // fallback
      }
    };
    fetchFields();
  }, []);
  ...
  useEffect(() => {
    if (modeProp === "edit" && dataProp?.id) {
      const fetchData = async () => {
        try {
          setLoading(true);
          const rec = await getRecord('audit', dataProp.id);
  ...
  const handleSave = async () => {
    try {
      setLoading(true);
      await saveRecord('audit', data);
      dispatch(showToast({ message: "Audit saved successfully", type: "success" }));
      onSaved?.();
    } catch (error) {
      dispatch(showToast({ message: "Failed to save audit", type: "error" }));
    } finally {
      setLoading(false);
    }
  };
