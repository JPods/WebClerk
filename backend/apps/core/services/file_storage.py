...
Image sizes: tn(90), md(256), hr(original)
Standard format: .jpg
...
def image_set_paths(org_id: int, model: str, record_id: int, purpose: str, ext: str) -> dict:
    ...
    result["hr"] = str(base / f"{purpose}_hr.{ext}")
    return result
...
def save_image(
    ...
    Returns:
        Dict with tn, md, hr paths (relative to MEDIA_ROOT)
    """
    try:
    ...
    # Save hr (original resolution)
    hr_path = MEDIA_ROOT / paths["hr"]
    hr_path.write_bytes(file_data)
