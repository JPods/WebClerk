...
interface TaxJurisdictionDisplayProps {
  inline?: boolean;
...
export default function TaxJurisdictionDisplay({
  inline = false,
  modeProp,
  dataProp,
  onSaved,
  onCancelInline,
}: TaxJurisdictionDisplayProps) {
  ...
  useEffect(() => {
    const fetchFields = async () => {
      try {
        const rec = await getRecord('tax_jurisdiction', 1);
        const record = rec.record || rec;
        setFields(Object.keys(record));
      } catch (error) {
        console.error("Failed to fetch fields", error);
        setFields(['id', 'name', 'code', 'rate']); // fallback
      }
    };
    fetchFields();
  }, []);
  ...
  useEffect(() => {
    if (modeProp === "edit" && dataProp?.id) {
      const fetchData = async () => {
        try {
          setLoading(true);
          const rec = await getRecord('tax_jurisdiction', dataProp.id);
  ...
  const handleSave = async () => {
    try {
      setLoading(true);
      await saveRecord('tax_jurisdiction', data);
      dispatch(showToast({ message: "Tax Jurisdiction saved successfully", type: "success" }));
      onSaved?.();
    } catch (error) {
      console.error("Failed to save", error);
      dispatch(showToast({ message: "Failed to save tax jurisdiction", type: "error" }));
    } finally {
      setLoading(false);
    }
  };
