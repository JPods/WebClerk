    ...
    for row in wc2_rows:
        ...
        financial = {
            'payables': {
                ...
                'last_cash_date': safe_str(row.get('dateLastCash')),
                'last_cash_amount': safe_float(row.get('amountLastPay')),
            },
            'credit': {
                'limit': safe_float(row.get('creditLimit')),
...
def convert_cash_entries(wc2_rows, lookup, stats, dry_run=False):
    ...
    for row in wc2_rows:
        ...
        defaults = {
            ...
            'dt_modified': now_ms(),
            'is_active': True,
            'config': build_config(row, extract_keys=CASH_EXTRACT),
        }
        ...
        if not dry_run:
            obj, was_created = Cash.objects.update_or_create(
                ida=ida, defaults=defaults
            )
            if was_created:
                created += 1
            lookup['cash'][ida] = obj.pk
        else:
            created += 1
    stats['Cash'] = {'total': len(wc2_rows), 'created': created}
...
def compute_audit(folder):
    ...
    # Unapplied cash
    cash_entries = load_wc2(folder, 'Cash')
    unapplied = [p for p in cash_entries if safe_float(p.get('amountAvailable', 0)) != 0]
    audit['cash_entries'] = {
        'total_count': len(cash_entries),
        'unapplied_count': len(unapplied),
        'unapplied_total': round(sum(safe_float(p.get('amountAvailable', 0)) for p in unapplied), 2),
        'total_paid': round(sum(safe_float(p.get('amount', 0)) for p in cash_entries), 2),
    }
...
class Command(BaseCommand):
    ...
    def _import(self, folder, dry_run=False, tables_filter=None, fast=False):
        ...
        converters = {
            ...
            'Cash': lambda rows: convert_cash_entries(rows, lookup, stats, dry_run),
    ...
    def _import_fast(self, folder, dry_run=False, tables_filter=None):
        ...
        if not _skip('Customer'):
            ...
            for r in rows:
                ...
                financial = {
                    ...
                    'cash_entries': {'last_date': safe_str(r.get('lastPayDate')), 'last_amount': safe_float(r.get('lastPayAmount')), 'days_avg_paid': safe_float(r.get('daysAvgPaid'))},
        ...
        # ── Cash ──
        # Cash model uses: amount, available, method, invoice_id, customer_id
        # NOT TransactionBaseModel fields (total, balance, totals, company)
        if not _skip('Cash'):
            rows = load_wc2(folder, 'Cash')
            objs = []
            for r in rows:
                ida = safe_str(r.get('idNum'))
                if not ida:
                ...
                obj = Cash(
                    ...
                    method=safe_str(r.get('typeCash'), 50),
                    is_active=True,
                    dt_created=wc2_date_to_epoch_ms(r.get('dateDocument')),
                    dt_modified=now_ms(),
                    config=build_config(r, extract_keys=CASH_EXTRACT),
                )
                objs.append(obj)
            _bulk(Cash, objs, 'Cash')
    ...
    def _verify(self, folder):
        ...
        from apps.transactions.models import Invoice, Cash, Order, Purchase
        from apps.products.models import Item
        from apps.orgs.models import OrgBase
        ...
        # Cash unapplied (Cash.available is a real field, not a shadow)
        pay_agg = Cash.objects.filter(available__isnull=False).exclude(available=0).aggregate(
            count=models.Count('id'), total=Sum('available')
        )
        wc3['cash_entries_unapplied'] = {
        ...
        checks = [
            ...
            ('Unapplied cash (count)',
             dump_audit['cash_entries']['unapplied_count'],
             wc3['cash_entries_unapplied']['count']),
            ('Unapplied cash ($)',
             dump_audit['cash_entries']['unapplied_total'],
             wc3['cash_entries_unapplied']['balance']),
        ]
