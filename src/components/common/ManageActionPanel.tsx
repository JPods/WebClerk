...
const ACTION_CONFIGS: Record<string, ActionDef[]> = {
  order: [
    { key: 'spawn_work_order', label: 'Create Work Order', icon: '🔧', variant: 'primary',
      confirm: 'Create a work order from this order?',
      params: (r) => ({ order_id: r.id }) },
    { key: 'partial_ship', label: 'Partial Ship', icon: '📦', variant: 'success', needsDialog: true },
    { key: 'complete_order', label: 'Complete Order', icon: '✓', variant: 'default',
  ...
  invoice: [
    ...
    { key: 'post_gl_entries', label: 'Post to GL', icon: '📒', variant: 'success',
      confirm: 'Post GL journal entries? This cannot be undone.',
      params: (r) => ({ model_name: 'invoice', id: r.id }) },
    { key: 'link_transaction_to_campaign', label: 'Link Campaign', icon: '📣', needsDialog: true },
  ],
  purchase: [
