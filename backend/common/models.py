...
Layering overview (left = earlier in MRO resolution):
    ...
    CoreModel (identity + timestamps + version)
      ↑
    Lifecycle / Config / Refs  / Metadata / Prefs / Comments / Health / AtomicJSON
      ↑
    ...
    BaseModel (full composition: change tracking, size enforcement, helpers)
      ↑    
    PydanticBaseModel - strongly typed schema layer
      ↑    
    size contraints - json object envelopes
...
def default_metadata() -> dict:
    ...
    return {
        ...
        "temp": [],
        "userdefined": {},
        "images": {"source": "", "tn": False, "md": False, "hr": False},
        "documents": [],
        "audit_trail": [],
        "import_data": None,
    }
...
class CoreModel(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # Ensure ida is never empty: transaction documents take the next number
        # in their company sequence (1001-inv); everything else uses its pk.
        if hasattr(self, 'ida') and (not getattr(self, 'ida')) and self.pk:
            from common.ida import DOCUMENT_SEQUENCES, generate_ida, next_document_ida
            model_name = self._meta.model_name
            if model_name in DOCUMENT_SEQUENCES:
                # Training mode (flight simulator): marker set by save_view
                training = bool(getattr(self, '_training_prefix', None))
                ida_value = next_document_ida(model_name, training=training)
            else:
                ida_value = generate_ida(self.pk)
            type(self).objects.filter(models.Q(ida="") | models.Q(ida__isnull=True), pk=self.pk).update(ida=ida_value)
            self.ida = ida_value
...
class MetadataMixin(models.Model):
    ...
    # to keep semantics clear and indexing straightforward.
        ...
        if isinstance(img, dict):
            ...
            img.setdefault("md", False)
            img.setdefault("hr", False)
        self.metadata.setdefault("documents", [])
