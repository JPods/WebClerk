...
export interface DetailToolbarProps {
  ...
  /** Add Cash (order) — creates a new cash record */
  onAddCash?: () => void;
  /** Apply Cash (invoice) — applies existing cash to this invoice */
  onApplyCash?: () => void;
  className?: string;
}
...
const DetailToolbar: React.FC<DetailToolbarProps> = ({
  ...
  onAddNew, onSave, onCancel, onClose, onDelete,
  onWorkflowComplete,
  onAddCash, onApplyCash,
  className = '',
}) => {
    ...
    <div className={`flex items-center gap-1 px-2 py-1 no-print ${className}`}
      ...
      {onAddCash && data?.id && (
        <button
          ...
          title="Enter a single installment"
        >
