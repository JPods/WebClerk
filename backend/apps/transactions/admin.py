...
@admin.register(Cash)
class CashAdmin(SchemaLabelsAdminMixin, JSONBFieldsetMixin, admin.ModelAdmin):
    list_display = ("ida", "status", "amount", "dt_cash", "dt_processed", "dt_reconciliation", "is_active", "dt_created")
    list_filter = ("status", "gateway", "reconciled", "is_active")
    search_fields = ("reference_number", "gateway_transaction_id", "gateway_payment_intent_id", "ida")
