...
captured rate and the current bank rate creates a balancing cash
...
def settle_fx_difference(
    ...
    """Create a balancing cash record for FX gain/loss at settlement.

    When cash is received, the difference between the rate captured
    ...
    Returns:
        {fx_amount, direction, cash_id} or {error}
    """
    ...
    if fx_cash:
        ...
        fx_cash.refs = meta
        fx_cash.save(update_fields=['refs'])
    ...
    return {
        ...
        'direction': direction,
        'fx_ledger_id': fx_cash.pk if fx_cash else None,
        'captured_rate': float(captured),
        'current_rate': float(current),
        'currency': currency,
    }
