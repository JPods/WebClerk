...
const CASH_FIELDS: Record<string, SchemaNode> = {
...
const COPYRIGHT_ENVELOPE: Record<string, SchemaNode> = {
  ...
  authors:      ARR('Authors'),
  year:         NUM('Copyright year'),
  license:      STR('proprietary | CC-BY-4.0 | public-domain | …'),
  license_url:  STR('Link to the license text'),
  level:        NUM('0 = open … higher = more restricted'),
  jurisdiction: STR('Whose law governs (US, EU, …)'),
  path:         STR('Where the notice / license text lives'),
  grant: OBJ('What the holder permits — anything not granted is withheld', {
    distribute:  STR('none | company | by_uuid | public'),
    modify:      BOOL('May be modified'),
    derive:      BOOL('Derivative works allowed'),
    commercial:  BOOL('Commercial use allowed'),
    resell:      BOOL('May be resold'),
    share_alike: BOOL('Derivatives carry these same terms'),
    attribution: STR('Credit line that must ride along'),
    conditions:  ARR('Extra conditions'),
  }),
  source: OBJ('Where the rights came from', {
    origin:        STR('original work | licensed | public domain | assigned'),
    acquired_from: STR('Party the rights came from'),
    dt_acquired:   NUM('Epoch ms UTC'),
    evidence_uuid: STR('Document UUID of the agreement / license file'),
    evidence_path: STR('Path to the evidence'),
  }),
  term: OBJ('Rights are revocable and they end', {
    dt_start:   NUM('Epoch ms UTC'),
    dt_expires: NUM('Epoch ms UTC; empty = no stated end'),
    on_expiry:  STR('revert | archive | renew | public_domain'),
    revocable:  BOOL('May be revoked'),
    dt_revoked: NUM('Epoch ms UTC'),
  }),
  contact: OBJ('Who to ask for permission beyond the grant', {
    name:       STR('Name'),
    email:      STR('Email'),
...
export const ENVELOPE_SCHEMAS: Record<string, ModelSchema> = {
  ...
  cash: {
    ...
    envelopes: {
      _core:    OBJ('CoreModel fields (all models)', CORE_FIELDS),
      _base:    OBJ('BaseModel fields', BASE_EXTRA_FIELDS),
      _cash: OBJ('Cash fields', CASH_FIELDS),
      ...BASE_ENVELOPES,
    },
  },
  ...
  term: baseSchema('Cash Term'),

  // Documents
  document: baseSchema('Document', {
    config: OBJ('Document configuration — terms that stay put', {
      copyright: OBJ('Copyright — holder, license, terms', COPYRIGHT_ENVELOPE),
    }),
