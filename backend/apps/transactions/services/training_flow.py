...
class TrainingFlow:
    ...
    def step_4_record_cash(self, amount: Optional[float] = None) -> Dict[str, Any]:
        """Record cash against invoice."""
        ...
        _stamp_training(self.cash)

        # Apply cash via pending (one path, one audit trail)
        application = None
        if self.invoice and amount and amount > 0:
            try:
                application = apply_cash_to_invoice(
                    cash_id=self.cash.pk,
                    invoice_id=self.invoice.pk,
                    amount=amount,
                )
            except Exception as e:
                logger.warning("Cash application failed in training: %s", e)
        ...
        step = {
            'step': 4,
            'action': f'Recorded cash of ${amount:.2f} against invoice {self.invoice.ida}',
            ...
            'cash_applied': application is not None,
            'note': 'Cash is editable until journalized. GL entries staged in metadata.',
            'record_id': self.cash.pk,
            'record_type': 'cash',
        }
        self.steps.append(step)
        return step
...
def cleanup_training_records():
    ...
    for Model in [Invoice, Purchase, Order, Quote]:
        ...
        if training_ids:
            # Delete cash linked to training invoices
            if Model == Invoice:
                d, _ = Cash.objects.filter(invoice_id__in=training_ids).delete()
                count += d
