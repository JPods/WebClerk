...
class BillOfMaterial(BaseModel):
    ...
    parent_ida = models.CharField(max_length=120, blank=True, db_index=True, help_text="String identifier for this BOM parent")
    parent_description = models.CharField(max_length=255, blank=True, help_text="Description for this BOM line")

    child_id = models.ForeignKey(Item, on_delete=models.PROTECT, related_name="bom_child")
    child_ida = models.CharField(max_length=120, blank=True, db_index=True, help_text="String identifier for this BOM child")
    child_description = models.CharField(max_length=255, blank=True, help_text="Description for this BOM line")
    ...
    @property
    def parent_ida_value(self):
        return self.parent_ida or str(self.pk)

    @property
    def description_value(self):
        return self.description or str(self.child_id) if hasattr(self, 'child_id') else ""
    ...
    Extended with:
      ...
      - data JSON for lightweight routing/tooling notes
    ...
    data = models.JSONField(default=dict, blank=True, help_text="Lightweight routing/tooling notes JSON")
    change_reason = models.CharField(max_length=120, blank=True, default="")
    dt_last_recalc = models.DateTimeField(null=True, blank=True)

    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["parent_id", "child_id"], name="uniq_bom_parent_child"),
            models.CheckConstraint(check=~models.Q(parent_id=models.F('child_id')), name='ck_bom_parent_ne_child'),
        ...
        indexes = [
            ...
            models.Index(fields=("child_id",), name="bom_child_idx"),
    ...
    def clean(self):  # pragma: no cover - simple validations
        ...
        component_pk = self.child_id.pk if self.child_id else None
    ...
    def save(self, *args, **kwargs):  # pragma: no cover - indirect validation
        ...
        if creating and self.cost_snapshot is None and component_pk:
            val = getattr(self.child_id, 'cost', None)
        ...
        for line in lines:
            try:
                ...
                if unit_cost is None:
                    ...
                    comp_cost = getattr(line.child_id, 'cost', None)
    ...
    @staticmethod
    def _descendant_child_ids(root_parent_id: int, max_depth: int = 20) -> set[int]:  # pragma: no cover - helper
        ...
        while queue:
            ...
            if depth >= max_depth:
                continue
            child_lines = BillOfMaterial.objects.filter(item_id=current_parent_id).values_list('child_id', flat=True)
    ...
    def _would_create_cycle(self, parent_id: int, child_id: int) -> bool:  # pragma: no cover - helper
        """Check if introducing (parent_id -> child_id) creates a cycle.

        Cycle exists if parent_id reachable from child_id through existing chains.
        """
        if parent_id == 0 or child_id == 0:
            return False
        # Fast reject: if parent not among descendant components of proposed component
        descendants = self._descendant_child_ids(child_id)
