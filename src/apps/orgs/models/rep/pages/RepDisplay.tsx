...
import { createRep, updateRep } from "../services/repApi";
...
export default function RepDetail({
  ...
  const {
    ...
    control,
    watch,
  } = useForm<z.infer<typeof repSchema>>({
  ...
  const onSubmit = async (formData: z.infer<typeof repSchema>) => {
    try {
      const res =
        mode === "add"
          ? await createRep(formData)
          : await updateRep({ ...formData, id: data && data.id });
      if (res) {
        dispatch(
          showToast({
            message: `Rep ${
  ...
  const headerDisplayName = watch("display_name") || data?.display_name || "New Rep";
  const headerIsActive = Boolean(watch("is_active") ?? data?.is_active);
  const headerOrgType = watch("org_type") || data?.org_type || "Rep";
          ...
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100 truncate">
                <DevBadge label="RepDisplay" className="mr-2" />
                {headerDisplayName}
              </h2>
