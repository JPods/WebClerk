...
# Pending purposes that are permanent records, not queue entries
RETAINED_PURPOSES = ('cash_application', 'cash_application_receipt')
...
def _archive_generic_pending(batch_size: int = 1000) -> Dict[str, int]:
    ...
    qs = (
        ...
        # Cash applications are the record of what paid each invoice — never archived.
        .exclude(purpose__in=RETAINED_PURPOSES)
        .order_by('pk')[:batch_size]
    )
...
def archive_processed_pending(batch_size: int = 1000) -> Dict[str, Any]:
    """Archive processed inventory and queue Pending records (cash applications are retained).
