...
Four stages:
  ...
  2. Invoice → Cash : collection lag   (invoice.dt_created → cash.dt_created)
  3. Cash → GL      : posting lag      (cash.dt_created → GlJournal.dt_created)
...
# ---------------------------------------------------------------------------
# Stage 2: Invoice → Cash (collection lag)
# ---------------------------------------------------------------------------

def _stage_invoice_to_cash(start_ms, end_ms) -> Dict[str, Any]:
    """Invoices that have a cash entry vs. still outstanding."""
    Invoice = dj_apps.get_model('transactions', 'Invoice')
    ...
    # Build map: invoice_id → earliest cash.dt_created
    ...
    for inv in inv_qs.values('id', 'dt_created', 'total', 'balance', 'status'):
        ...
        if iid in pay_map:
            ...
            # No cash yet — if balance > 0, it's stalled
    ...
    return _stage_dict(
        name='invoice_to_cash',
        label='Invoice → Cash (Collection Lag)',
        avg_days=avg_d,
        median_days=med_d,
        count_in_stage=len(stalled_ids),
        count_completed=count_completed,
...
# ---------------------------------------------------------------------------
# Stage 3: Cash → GL (posting lag)
# ---------------------------------------------------------------------------

def _stage_cash_to_gl(start_ms, end_ms) -> Dict[str, Any]:
    """Cash that have a GL journal entry vs. awaiting posting."""
    Cash = dj_apps.get_model('transactions', 'Cash')
    ...
    # Build map: cash_id → earliest GlJournal.dt_created for that cash
    gl_map: Dict[int, int] = {}
    gl_qs = (
        GlJournal.objects
        .filter(source_model='cash')
...
def get_cash_conversion(year: Optional[int] = None, month: Optional[int] = None) -> Dict[str, Any]:
    ...
    stages = [
        ...
        _stage_invoice_to_cash(start_ms, end_ms),
        _stage_cash_to_gl(start_ms, end_ms),
