...
def _fx_absorption_account() -> str:
    """FX rounding residue posts to the company's fx_gain_loss account."""
    from apps.accounts.services.chart import role_account
    return role_account('fx_gain_loss', used_by='FX rounding absorption')
...
def _check_balance(postings: dict, has_exchange_rate: bool = False) -> dict:
    ...
    if has_exchange_rate and abs(residual) < FX_ABSORPTION_LIMIT:
        ...
        fx_account = _fx_absorption_account()
        if fx_account not in postings:
            postings[fx_account] = {
                'debit': Decimal('0'), 'credit': Decimal('0'),
                'purpose': 'fx_rounding_absorption',
            }
        postings[fx_account][side] += abs(residual)
        return {
            'balanced': True,
            'residual': residual,
            'absorbed': True,
            'absorption_account': fx_account,
...
def force_to_balance(
    ...
    Args:
        ...
        source_model: 'invoice', 'cash', or 'purchase'
        user_statement: required explanation from user
        forced_by: contact_id of the user approving the force
        adjustment_account: GL account for the adjustment (defaults to the company fx_gain_loss account)
    ...
    adj_account = adjustment_account or _fx_absorption_account()
...
def _get_category_gl(category: str) -> str:
    """GL account for an expense category bucket.

    Reads gl_map from the cash model definition Setting (select_lists.category).
    A category with no mapping posts to the company default_expense account.
    """
    from apps.accounts.services.chart import role_account
    gl = None
    if category:
        Setting = dj_apps.get_model('core', 'Setting')
        setting = Setting.objects.filter(purpose='wc:model', parent_model='cash').first()
        sl = (setting.config or {}).get('select_lists', {}).get('category', {}) if setting else {}
        gl = sl.get('gl_map', {}).get(category)
    return gl or role_account('default_expense', used_by=f'expense category "{category or "(none)"}"')
...
def _role(role: str, used_by: str = '') -> str:
    """Account for a posting role from the company GL role map (chart-checked)."""
    from apps.accounts.services.chart import role_account
...
def journalize_invoice(invoice_id: int, ida_prefix: str = '') -> dict:
    ...
    Header (from the totals envelope):
      - Debit AR for totals.total (customer's AR account or the company role)
      - Credit sales_tax_payable for totals.tax
    ...
    ar_account = _role('accounts_receivable')
    if customer_id:
    ...
    for line in lines:
        ...
        rev_account = item_gls.get('revenue') or _role('sales_revenue')
        if customer_id:
    ...
    def _amt(key):
        return Decimal(str(totals.get(key, 0) or 0)).quantize(Decimal('0.01'))
    ar_total = _amt('total')
    if ar_total:
        _add(ar_account, 'debit', ar_total, 'accounts_receivable')
    # Tax posts to the jurisdiction's payable account when it names one.
    finance = invoice.finance if isinstance(getattr(invoice, 'finance', None), dict) else {}
    tax_account = finance.get('tax_gl_account')
    if tax_account:
        from apps.accounts.services.chart import require_account
        require_account(tax_account, used_by=f'invoice {invoice.ida} finance.tax_gl_account')
    for key, role, purpose in (('tax', 'sales_tax_payable', 'sales_tax'),
                               ('shipping', 'shipping_revenue', 'shipping'),
                               ('finance_charge', 'finance_charge_income', 'finance_charge'),
                               ('other', 'other_income', 'other_charges')):
        amount = _amt(key)
        if amount:
            account = tax_account if key == 'tax' and tax_account else _role(role, f'invoice {invoice.ida} {key}')
            _add(account, 'credit', amount, purpose)
...
def journalize_cash(cash_id: int, ida_prefix: str = '') -> dict:
    """Journalize a cash entry — checkbook convention (signed amounts).
    ...
    pay_status = (getattr(cash, 'status', '') or '').lower()
    ...
    cash_account = _role('undeposited_funds')

    if is_received:
        # AR receipt: Cash debit, AR credit
        offset_account = _role('accounts_receivable')
        invoice_id = getattr(cash, 'invoice_id', None)
        if invoice_id:
            try:
                Invoice = dj_apps.get_model('transactions', 'Invoice')
        ...
        offset_account = _get_category_gl(getattr(cash, 'category', ''))
        purchase_id = getattr(cash, 'purchase_id', None)
        if purchase_id:
            try:
    ...
    with transaction.atomic():
        ...
        fee_amount = Decimal(str(getattr(cash, 'fee_amount', 0) or 0))
        if is_received and fee_amount > 0:
            meta = cash.metadata if isinstance(getattr(cash, 'metadata', None), dict) else {}
    ...
    # FX settlement — if cash is against a foreign-currency invoice,
        ...
        try:
            ...
            if captured_rate and currency and captured_rate != 1:
                ...
                if current_rate is not None:
                    ...
                    if captured != current:
                        # FX difference on the cash amount
                        ...
                        if fx_amount != 0:
                            ...
                            logger.info(
                                'FX settlement: cash %s — %s %.2f (%s rate %.4f→%.4f)',
                                cash_id, direction, abs(fx_amount), currency, captured, current,
                            )
        except Exception as e:
            logger.warning('FX settlement check failed for cash %s: %s', cash_id, e)

    result = {'created': created, 'postings': posting_list, 'cash_ida': cash.ida}
    if fx_result:
        result['fx'] = fx_result
    return result
...
def journalize_invoice_and_cash_entries(invoice_id: int, ida_prefix: str = '') -> dict:
    """Journalize an invoice AND all its linked cash in one call.

    Walks the invoice's cash (via invoice_id FK and parent_model/parent_id),
    journals the invoice first, then each un-journalized cash.

    Returns:
        {invoice: {...}, cash: [{...}], total_created: int}
    """
    Cash = dj_apps.get_model('transactions', 'Cash')
    Invoice = dj_apps.get_model('transactions', 'Invoice')
    ...
    for pay in cash_entries:
        pay_result = journalize_cash(pay.pk, ida_prefix)
        pay_results.append({
            'cash_id': pay.pk,
            'cash_ida': pay.ida,
            **pay_result,
...
def batch_journalize(ida_prefix: str = 'zzz-', run_by_id: int = None) -> dict:
    ...
    Returns:
        {batch_id: int, batch_ida: str, invoices: [...], cash: [...],
    ...
    absorbed += GlJournal.objects.filter(
        batch_id=batch_ida, account=_fx_absorption_account(),
    ).count()
...
def journalize_bom_build(batch_id: str, parent_item_id: int, qty: Decimal, component_costs: list[dict], ida_prefix: str = '') -> dict:
    ...
    fg_account = parent_gls.get('inventory') or _role('inventory')
    ...
    for comp in component_costs:
        ...
        comp_account = comp_gls.get('inventory') or _role('inventory')
...
def journalize_adjustment(item_id: int, qty: Decimal, cost: Decimal, reason: str = '', ida_prefix: str = '') -> dict:
    ...
    inv_account = item_gls.get('inventory') or _role('inventory')
    adj_account = item_gls.get('cogs') or _role('cost_of_goods_sold')  # adjustments hit COGS by default
