...
def test_note_with_password_is_refused():
    r = _post("/wcapi/save/", {"model": "action", "fields": {"comments": {"notes": "router wifi password is Summer2026!"}}})
...
def test_clean_note_passes():
    assert _post("/wcapi/save/", {"model": "action", "fields": {"name": "Change the WiFi password"}}).content == b"stored"


def test_password_forms_and_login_are_exempt():
    assert _post("/wcapi/token/", {"email": "a@b.test", "password": "Summer2026!"}).content == b"stored"
    assert _post("/wcapi/save/", {"model": "contact", "fields": {"password": "Summer2026!"}}).content == b"stored"
