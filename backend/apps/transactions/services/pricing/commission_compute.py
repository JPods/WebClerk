    ...
    with transaction.atomic():
        for rep in reps:
            ...
            comm_expense = (
                ...
                or role_account('commission_expense', used_by=f'commission on {header.ida}')
            )
            comm_payable = (
                comm_gl.get('payable')
                ...
                or role_account('commission_payable', used_by=f'commission on {header.ida}')
            )
...
def scale_commission_on_cash(invoice_id: int) -> dict:
    ...
    Call this after a cash entry is applied to an invoice.

    Returns: {invoice_id, cash_fraction, reps: [{rep_id, original, scaled}]}
    """
    ...
    if not comm.get('reps'):
        return {'invoice_id': invoice_id, 'cash_fraction': 0, 'reps': []}
    ...
    total_paid = sum(
        abs(Decimal(str(l.value_original)))
        for l in ledgers if l.model_name == 'cash'
    )

    if total_original <= 0:
        return {'invoice_id': invoice_id, 'cash_fraction': 0, 'reps': []}

    cash_fraction = float(min(total_paid / total_original, Decimal('1')))

    # Scale each rep's commission
    scaled_reps = []
    for rep in comm.get('reps', []):
        original = rep.get('amount', 0)
        scaled = round(original * cash_fraction, 2)
        rep['amount_scaled'] = scaled
        rep['cash_fraction'] = cash_fraction
        scaled_reps.append({
            'rep_id': rep.get('rep_id'),
            'name': rep.get('name', ''),
            'original': original,
    ...
    comm['cash_fraction'] = cash_fraction
