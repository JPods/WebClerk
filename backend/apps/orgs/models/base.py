...
class OrgBase(StandardLinksMixin, RelationshipStatsMixin, StatsMixin, BaseModel):
	...
	def build_company_snapshot(self, contact=None) -> dict:
		...
		Contact drives communication fields (attention, full_address, phone,
		email, domain).  OrgBase provides company identity (id, ida, name).
		One contact = all roles until explicitly split.

		Args:
			contact: a Contact instance to pull communication from.
			         Falls back to self.contact_id → Contact lookup.
			         Falls back to OrgBase's own FK pointers as last resort.
		"""
		# Resolve contact: explicit arg → org's primary contact → None
		if contact is None and self.contact_id:
		...
		if contact:
			...
			full_address = contact.address_full or ''
			phone = contact.phone or ''
			email = contact.email or ''
			domain = contact.domain or ''
			contact_id = contact.pk
		else:
			# Last resort: read from OrgBase's own FK pointers
			attention = getattr(self, 'attention', '') or ''
			full_address = self.address_full or ''
			phone = self.phone or ''
	...
	def save(self, *args, **kwargs):
		...
		from apps.accounts.services.chart import validate_gl_map
		validate_gl_map(self.gl_accounts, owner=f'{self.org_type or "org"} {self.ida or "(new)"} gl_accounts')
