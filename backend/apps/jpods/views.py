...
from apps.jpods.services.pricing import quote_trip, resolve_price_query
...
class InvoiceView(APIView):
    ...
    def post(self, request):
        # Alice owns pricing: re-quote server-side; Natalie's price field is ignored.
        data = request.data.dict() if hasattr(request.data, "dict") else dict(request.data)
        contact = Contact.objects.filter(pk=data.get("contact_id"), is_active=True).first()
        if not contact:
            return api_response(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
        ...
        quote, err = quote_trip(contact, data.get("origin_station_id", ""),
                                data.get("destination_station_id", ""), data.get("network_id", "default"))
        if err:
            return api_response(status_code=err[1], message=err[0], error_code="invoice_failed")
        data.update({
        ...
        result = create_trip_invoice(data)
        if "error" in result:
            return api_response(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                message=result["error"],
