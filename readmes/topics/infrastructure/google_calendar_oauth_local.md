...
- The Google Calendar service resolves credentials in this order:
  ...
  1) Connection.config.credentials_file (relative paths resolved from BASE_DIR)
  1) Inline values in Connection.config (not recommended for secrets)
Quick setup
...
1) Create a Connection configured to use env or a credentials file and file-based token storage:
...
1) Start OAuth (as a staff/admin user):
   GET /sync/google/calendar/start?connection_id=<ID>
   - Redirect user to the provided authorization_url.

1) After consent, Google redirects to your redirect_uri (e.g., <http://localhost:8000/sync/google/calendar/callback>).
