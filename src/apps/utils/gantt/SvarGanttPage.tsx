...
import { createInitialTaskFormState } from "./GanttPage";
...
const mapKanbanTaskToSvarTask = (
  ...
  return {
    id: task.id,
    text: task.title ?? task.properties.action?.en ?? '',
    parent: parentId,
    start,
    end,
    duration,
    type: "task",
    progress: toProgressRatio(task.progress ?? task.properties.percent_complete),
    details: task.description ?? task.properties.description?.en ?? '',
    assignee: task.assignee ?? (task.properties.assigned_to?.[0]?.id ? String(task.properties.assigned_to[0].id) : undefined),
    priority: task.priority_value ?? task.properties.priority,
    status: task.status ?? task.properties.status,
    tags: task.tags,
    columnId,
    columnTitle,
  };
...
const mapBoardToSvarGantt = (boardData: BoardData): GanttDataset => {
  ...
  boardData.column_order.forEach((columnId) => {
    ...
    column.task_ids.forEach((taskId, index) => {
      ...
      // If you need to handle subtasks, adapt this section to your data model.
    });
...
const SvarGanttPage: React.FC = () => {
  ...
  const editDifficultyOptions = useMemo(
    () => extendNumericOptionStrings(DIFFICULTY_OPTIONS, editTaskState.difficulty_value),
    [editTaskState.difficulty_value]
  );

  const editProgressOptions = useMemo(
    () => extendNumericOptionStrings(PROGRESS_OPTIONS, editTaskState.progress_value),
    [editTaskState.progress_value]
  );
  ...
  const deriveFormStateFromTask = useCallback(
    (task: KanbanTask, overrides?: Partial<TaskFormState>): TaskFormState => {
      ...
      const normalizedEnd = normalizeIncomingDateValue(task.dt_expected);
      ...
      const normalizedDifficulty = normalizeNumericSelectValue(
        task.difficulty ?? PRIORITY_TO_VALUE['medium'],
        DEFAULT_DIFFICULTY
      );
      ...
      const normalizedProgress = normalizeNumericSelectValue(normalizedProgressValue, DEFAULT_PROGRESS);
       ...
       return {
         ...
         priority: task.priority_value ?? task.properties?.priority ?? 2,
  ...
  const buildEditActionPayload = useCallback(
    (state: TaskFormState, baseTask: KanbanTask | null): { payload: Record<string, unknown> } | { error: string } => {
      ...
      const assignedTo = state.actor_id
        ? [{ name: state.actor_id }]
        : baseTask.properties.assigned_to?.map((assignment) => ({ name: assignment.id })) ?? [];
      ...
      const resolvedProgress = Number(state.progress_value) || 0;
      ...
      const payloadItem: Record<string, unknown> = {
        ...
        priority: {
          mode: "update",
          value: state.priority_value,
        },
        difficulty: {
          mode: "update",
          value: Number(state.difficulty_value) || state.priority_value,
        },
        status: {
          mode: "update",
          value: baseTask.status ?? baseTask.properties.status ?? "In progress",
        },
        dt_deadline: {
          mode: "update",
          value: dueTimestamp,
      ...
      if (!state.actor_id && assignedTo.length === 0) {
  ...
  const fetchGanttTasks = useCallback(async () => {
    ...
    try {
      ...
      if (!response || typeof response !== 'object' || !('status' in response) || typeof response.status !== 'number' || (response.status !== 200 && response.status !== 201)) {
