...
export default function OtherDisplay({
  ...
  useEffect(() => {
    const fetchFields = async () => {
      try {
        const rec = await getRecord('other', 1);
        const record = rec.record || rec;
        setFields(Object.keys(record));
      } catch (error) {
        console.error("Failed to fetch fields", error);
        setFields(['id', 'name', 'type', 'description']); // fallback
      }
    };
    fetchFields();
  }, []);
  ...
  useEffect(() => {
    if (modeProp === "edit" && dataProp?.id) {
      const fetchData = async () => {
        try {
          setLoading(true);
          const rec = await getRecord('other', dataProp.id);
  ...
  const handleSave = async () => {
    try {
      setLoading(true);
      await saveRecord('other', data);
      dispatch(showToast({ message: "Other saved successfully", type: "success" }));
      onSaved?.();
    } catch (error) {
      console.error("Failed to save", error);
      dispatch(showToast({ message: "Failed to save other", type: "error" }));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-4 space-y-4">
      <h2 className="text-xl font-semibold">
        {modeProp === "add" ? "Add Other" : modeProp === "edit" ? "Edit Other" : "View Other"}
      </h2>
