...
def clone_record(model_name: str, record_id: int, include_children: bool = True, contact_id: int = None) -> dict:
    ...
    with transaction.atomic():
        ...
        # balance is derived from totals; a clone has no cash applied yet
        if isinstance(getattr(clone, 'totals', None), dict) and 'received' in clone.totals:
            from apps.transactions.services.cash.cash_pending import cash_state
            total = clone.totals.get('total') or 0
            clone.totals = {**clone.totals, 'received': 0, 'balance': total,
                            'cash_state': cash_state(total, 0)}
        ...
        if include_children and model_name in CHILDREN_MAP:
            ...
            for line in source_lines:
                ...
                # A clone is standalone: the parent line link is removed, and the
                # origin becomes history (Bill, 2026-09-17). To make another child,
                # start from the parent.
                line_clone.parent_line_id = None
                refs = dict(line_clone.refs or {}) if isinstance(line_clone.refs, dict) else {}
                source = refs.pop('source', None)
                if source:
                    refs['cloned_from'] = source
                line_clone.refs = refs
                quantity = dict(line_clone.quantity or {}) if isinstance(line_clone.quantity, dict) else {}
                if quantity:
                    # standalone: staged is its own creation snapshot (D2), nothing consumed yet
                    quantity['staged'] = quantity.get('active', quantity.get('staged', 0))
                    quantity['remaining'] = quantity.get('active', 0)
                    line_clone.quantity = quantity
