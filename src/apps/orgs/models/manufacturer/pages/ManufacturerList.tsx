...
import ManufacturerListMob from "./ManufacturerListMob";
export default function ManufacturerList() {
  ...
  const getLocationData = useCallback(async (manufacturerId?: number) => {
    setLoading(true);
    try {
      ...
      if (manufacturerId) {
        const contactRes = await getRecord("Manufacturer", manufacturerId);
        setSelectedManufacturers(contactRes.record);
  ...
  const handleEdit = async (row: dynamicData) => {
    ...
    console.log("res.", res);
    if (res.status === 200) setSelectedManufacturers(res.data.data.record);
    else setSelectedManufacturers(row);
    setFormMode("edit");
    console.log("res", res);
  };
  ...
  const handleDelete = async (row: dynamicData) => {
    if (window.confirm(`Delete Manufacturer ${row.name}?`)) {
      try {
        ...
        if (selectedManufacturers && selectedManufacturers.id === row.id) {
          setFormMode(null);
          setSelectedManufacturers(null);
        }
      } catch (error) {
        ...
        {formMode && (
          <div className="lg:col-span-2">
            <ManufacturerDetail
              inline
              modeProp={formMode}
              dataProp={selectedManufacturers}
