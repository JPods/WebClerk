...
def test_proposal_to_order_action(django_user_model):
    ...
    Setting.objects.create(
        purpose='wc:view_edit', parent_model='proposal', is_active=True,
        config={"USER": {"view": ["id"], "edit": ["id"]}}
    )
    ...
    resp = client.post(f'/wcapi/transactions/proposals/{proposal.pk}/convert-to-order/', {}, format='json')
...
def test_order_to_invoice_action(django_user_model):
    ...
    Setting.objects.create(
        purpose='wc:view_edit', parent_model='order', is_active=True,
        config={"USER": {"view": ["id"], "edit": ["id"]}}
    )
    ...
    resp = client.post(f'/wcapi/transactions/orders/{so.pk}/convert-to-invoice/', {}, format='json')
    assert resp.status_code == 201  # type: ignore[attr-defined]
    body = resp.data  # type: ignore[attr-defined]
    payload = body.get('data') if isinstance(body, dict) else None
    assert isinstance(payload, dict)
...
def test_receive_purchase_action(django_user_model):
    ...
    Setting.objects.create(
        purpose='wc:view_edit', parent_model='purchase', is_active=True,
        config={"USER": {"view": ["id"], "edit": ["id"]}}
    )
    ...
    resp = client.post(f'/wcapi/transactions/purchases/{po.pk}/receive-goods/', payload, format='json')
