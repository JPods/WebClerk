...
export default function QuestionAnswerDisplay({
  ...
  useEffect(() => {
    const fetchFields = async () => {
      try {
        const rec = await getRecord('question_answer', 1);
        const record = rec.record || rec;
        setFields(Object.keys(record));
      } catch (error) {
        console.error("Failed to fetch fields", error);
        setFields(['id', 'question', 'answer', 'status']); // fallback
      }
    };
    fetchFields();
  }, []);
  ...
  return (
    <div className="p-4 space-y-4">
      <h2 className="text-xl font-semibold">
        {modeProp === "add" ? "Add Question Answer" : modeProp === "edit" ? "Edit Question Answer" : "View Question Answer"}
      </h2>
