/**
 * SerialDisplay - Follows 3-column standard with tab navigation
...
const STORAGE_KEY = "serialDisplay_columnCount";
...
export default function SerialDisplay({
  ...
  // Tab navigation
  const { activeTab, setActiveTab } = useDetailTabs("serial_display", "actions", [
  ...
  useEffect(() => {
    if (modeProp === "edit" && dataProp?.id) {
      const fetchData = async () => {
        try {
          setLoading(true);
          const rec = await getRecord('serial', dataProp.id);
      ...
      {!loading && (
        <ComponentCard>
          ...
          <div className={getGridClassName(columnCount)}>
            <HorizontalField label="Serial Number" htmlFor="serial_number" icon={<Hash size={14} />}>
              <Input
                type="text"
                id="serial_number"
                placeholder="Serial Number"
                value={data?.serial_number || ""}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleFieldChange("serial_number", e.target.value)}
