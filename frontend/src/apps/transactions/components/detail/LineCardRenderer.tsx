...
import { TransactionItemSearch, resolveItemCode, resolveItemDescription, resolveUnitPrice, resolveUnitCost, resolveQtyOnHand, DEFAULT_UNIT_COST_BASIS } from '../TransactionItemSearch';
...
const LineCardRenderer: React.FC<LineCardRendererProps> = ({ section, data, isEditing, isLocked, onLinesChange, onEnterCash }) => {
  ...
  const handleAddItem = useCallback((item: ItemSearchResult, quantity: number) => {
    ...
    const unitCost = resolveUnitCost(item, unitCostBasis);
    ...
    const newLine: any = {
      ...
      price: { unit: unitPrice, discount_percent: 0 },
      cost: { unit: unitCost },
      totals: { amount: unitPrice * effectiveQty, cost: unitCost * effectiveQty },
  ...
  }, [lines, data?.price_level, onLinesChange, dispatch, companyInventory, unitCostBasis]);
        ...
        {onEnterCash ? (
          <>
            <button
              ...
              style={{ fontSize: 'inherit' }}
              title="Enter a cash entry or apply an existing one"
            >
              Cash
            </button>
            <span
    ...
    <div style={{ display: 'flex', gap: 0, minHeight: 200 }}>
      <div style={{ flex: '1 1 50%', overflow: 'auto' }}>
        <DataGrid
          ...
          onToggleRow={(id) => { lc.setSelectedLineIds(prev => { const n = new Set(prev); if (n.has(id)) { n.delete(id); } else { n.add(id); } return n; }); }}
