...
class TransactionEmailService:
    ...
    @classmethod
    def send_cash_received_notification(cls, cash: Cash) -> bool:
        """Send notification when a cash entry is received."""
        if not getattr(settings, 'EMAIL_NOTIFICATIONS_ENABLED', True):
            return False
        if not getattr(settings, 'EMAIL_CASH_RECEIVED_ENABLED', True):
            return False

        recipient_emails = cls.get_recipient_emails(cash.contact_id)
        if not recipient_emails:
            return False

        context = {
            'cash': cash,
            'contact': Contact.objects.get(id=cash.contact_id),
            'company_name': getattr(settings, 'COMPANY_NAME', 'WebClerk3'),
        }
