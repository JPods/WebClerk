 ...
 */
const roleMatches = (userRole: string | null | undefined, allowedRoles: UserRole[]): boolean => {
...
export const usePermissions = (options: UsePermissionsOptions = {}): UsePermissionsResult => {
  ...
  const userRole: string | null = typeof user?.role === 'string' ? user.role : null;
  
  // Check if user is admin
  const isAdmin = roleMatches(userRole, ADMIN_ROLES);
  
  // Get default permissions for panel type
  const defaults: PanelPermissions = panelType && DEFAULT_PANEL_PERMISSIONS[panelType]
  ...
  const canView = isAdmin || roleMatches(userRole, effectiveViewRoles);
  const canEdit = !forceReadOnly && (isAdmin || roleMatches(userRole, effectiveEditRoles));
