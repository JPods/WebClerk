...
To build a database from scratch after this squash:

```bash
# Drop any old local DB (adjust name/user as needed)
psql -c 'DROP DATABASE IF EXISTS commerce_expert;' -d postgres
psql -c 'CREATE DATABASE commerce_expert;' -d postgres

# (Optional) Clear migration pyc cruft
find apps -path '*/migrations/*.pyc' -delete

# Run migrations
python manage.py migrate

# Create a superuser (script wrapper provided)
...
- Tests pass on a clean DB (spot checks + targeted suites; full run pending in CI).
