...
JSON_KEY = re.compile(r'^[A-Za-z0-9_]{1,40}$')


def _json_scalar(value: Any) -> Any:
    """A JSON envelope stores typed values: "3" must match the number 3,
    "true" the boolean. Anything else stays the string it was."""
...
class UnknownFilterError(ValueError):
    """A query filter names a field or lookup the model does not have.

    Dropping it silently returns a wider result set than asked for — the caller
    cannot tell "no such field" from "nothing matched". The message coaches:
    it names what was rejected, the nearest real field, and how to filter JSON.
    """

    # Dots are echoed too: 'totals.balance' is an honest mistake worth naming.
    SAFE_NAME = re.compile(r'^[A-Za-z0-9_.]{1,40}$')

    def __init__(self, model_key: str, keys, field_names=None, allowed_lookups=None,
                 filterable=None):
        # Never echo a caller-supplied name that is not a plain identifier.
        self.keys = [k if self.SAFE_NAME.match(str(k)) else '<invalid>' for k in keys]
        self.model_key = model_key
        self.hints = self._coach(self.keys, set(field_names or ()), set(allowed_lookups or ()),
                                 list(filterable or sorted(field_names or ())))
        lead = (f"{model_key}: no filter field named "
                f"{', '.join(repr(k) for k in self.keys)}.")
        super().__init__(' '.join([lead] + self.hints))

    @staticmethod
    def _coach(keys, field_names, allowed_lookups, filterable) -> List[str]:
        hints: List[str] = []
        for key in keys:
            base, _, lookup = str(key).partition('__')
            if '.' in base:
                envelope, _, rest = base.partition('.')
                path = rest.replace('.', '__')
                if envelope in field_names:
                    hints.append(
                        f"'{base}' is a value inside the {envelope} JSON envelope: "
                        f"filter it as '{envelope}__{path}'.")
                else:
                    hints.append(f"'{base}' is not a field; JSON paths are written with __.")
                continue
            if base in field_names and lookup:
                hints.append(
                    f"'{base}' is a field but '{lookup}' is not a comparison; "
                    f"use one of: {', '.join(sorted(allowed_lookups))}.")
                continue
            near = difflib.get_close_matches(base, sorted(field_names), n=2, cutoff=0.6)
            if near:
                hints.append(f"Did you mean {' or '.join(repr(n) for n in near)}?")
            elif filterable:
                hints.append(f"Fields you can filter include: {', '.join(filterable[:8])}.")
        return hints
...
class WCAPIGetView(APIView):
    ...
    def _parse_filters(self, request, model_key: str, ModelCls, reserved_keys: Optional[Set[str]] = None) -> Dict[str, Any]:
        ...
        for key, value in request.query_params.items():
            ...
            if key in {'model_name', 'model', 'id', 'fields', 'limit', 'offset', 'page', 'page_size',
                    'q', 'search', 'keyword', 'search_fields', 'saved_search', 'saved_search_name',
                    'saved_search_id', 'search_id', 'order_by', 'ordering', 'sort',
                    'format', 'raw', 'include_resolved'}:
                continue
            ...
            if '__' in key:
                ...
                if field_types.get(field_base) == 'JSONField':
                    # A JSON envelope is filterable by key path: quantity__active,
                    # totals__cash_state, item__item_id, quantity__active__gte.
                    if parts and parts[-1] in ALLOWED_LOOKUPS:
                        lookup_type, path = parts[-1], parts[:-1]
                    else:
                        lookup_type, path = 'exact', parts
                    if not path or not all(JSON_KEY.match(part) for part in path):
                        unknown.append(key)
                        continue
                    if lookup_type == 'in':
                        filters[key] = self._coerce_filter_value('', value, 'in')
                    else:
                        filters[key] = _json_scalar(value)
                    continue
                lookup_type = parts[0]
        ...
        if unknown:
            # Coach with columns a caller can actually filter — not JSON envelopes
            # or reverse relations.
            filterable = sorted(
                name for name, kind in field_types.items()
                if kind not in {'JSONField', 'ManyToManyField', 'OneToManyRel', 'ManyToOneRel'}
                and not name.startswith('_')
            )
            raise UnknownFilterError(model_key, unknown, field_names, ALLOWED_LOOKUPS,
    ...
    def _handle(
        ...
        try:
            ...
            # Alice tracks rejected filters and recommends the fix; repeats escalate
            # to WC_HQ, then to Bill (apps/ai_assistant/services/filter_coach.py).
            from apps.ai_assistant.services.filter_coach import observe_unknown_filter
            observe_unknown_filter(
                model_key, e.keys, e.hints,
                caller=request.META.get('HTTP_REFERER', '') or request.path,
                contact_id=getattr(getattr(request, 'user', None), 'pk', None),
            )
            return api_response(
                success=False,
                status_code=status.HTTP_400_BAD_REQUEST,
                message=str(e),
                error={"code": "unknown_filter",
                       "details": {"model_name": model_key, "fields": e.keys}},
            )
