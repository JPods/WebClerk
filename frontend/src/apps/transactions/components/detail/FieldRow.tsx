...
/** Envelopes a journalized document locks: its total and the values that make it up.
 *  Mirrors apps/core/views/field_rules_view.JOURNALIZED_LOCKED; the endpoint
 *  wcapi/<model_name>/fields/?id=<pk> is authoritative. */
export const JOURNALIZED_LOCKED = ['totals', 'quantity', 'price', 'cost', 'tax', 'commission'];
...
export interface FieldRowProps {
  ...
  /** Envelopes this record's own state locks (a journalized document locks its totals
   *  and the values that make them up). A viewable-but-not-editable field reads italic. */
  lockedFields?: string[];
  help?: string;       // Shift+hover shows Alice's field help
  onChange: (field: string, value: unknown) => void;
}

const FieldRow: React.FC<FieldRowProps> = ({ field, label, data, isEditing, options, fieldType, lockedFields, help, onChange }) => {
  ...
  const handleLabelClick = (e: React.MouseEvent) => {
    if ((e.metaKey || e.ctrlKey) && !e.shiftKey) {
      e.preventDefault();
      e.stopPropagation();
      // Cmd/Ctrl+click on record reference fields → open in new window
      if (val && typeof val === 'object' && (val as any).id) {
        const leaf = field.split('.').pop()?.replace(/\[\d+\]$/, '') || '';
        // Map field names to their model routes
        const FIELD_TO_ROUTE: Record<string, string> = {
          company: 'customer', customer: 'customer', vendor: 'customer',
          manufacturer: 'customer', employee: 'contact', rep: 'contact',
        };
        const route = FIELD_TO_ROUTE[leaf] || leaf;
        window.open(`/${route}?id=${(val as any).id}`, '_blank');
        return;
      }
  ...
  const isRecordRef = val && typeof val === 'object' && (val as any)?.id;
  const baseType = fieldType || nameType || (options ? 'select' : (isRecordRef ? 'action' : 'editable'));
  // Viewable but not editable reads as readonly: italic label, no input (Bill, 2026-09-17)
  const isLocked = (lockedFields || []).some(
    (locked) => field === locked || field.startsWith(`${locked}.`));
  const resolvedType = isLocked ? 'readonly' : baseType;
  ...
  return (
    <div className="flex items-baseline gap-2 py-0.5 relative group" data-field-path={field}>
      ...
      {/* Record reference field — locked when populated, searchable when empty */}
      {(() => {
        const AFF_FIELDS = ['customer','vendor','manufacturer','rep'];
        const isAffiliation = AFF_FIELDS.includes(leafName);
        // All org affiliations search the same org table
        const FIELD_TO_MODEL: Record<string, string> = {
          customer: 'org', vendor: 'org', manufacturer: 'org', rep: 'org',
        };
        ...
        const doSearch = (q: string) => {
          ...
          orgTimerRef.current = setTimeout(async () => {
            ...
            try {
              const model = FIELD_TO_MODEL[leafName] || 'customer';
              const res = await getRecords(model, { keyword: q, limit: 10 });
              setOrgResults(res?.results || res?.records || []);
            } catch { setOrgResults([]); }
            setOrgSearching(false);
          }, 300);
        };

        const handleOrgSelect = (record: any) => {
          onChange(field, record.id);
          setOrgQuery(''); setOrgResults([]); setRefUnlocked(false);
        };

        const searchInput = (showLockBtn: boolean) => (
          <div className="flex-1 relative min-w-0 flex items-center gap-1">
            <div className="flex-1 relative min-w-0">
              <input type="text" value={orgQuery}
                placeholder={`Search ${label}...`}
                onChange={(e) => doSearch(e.target.value)}
                onKeyDown={(e) => { if (e.key === 'Escape') { setOrgQuery(''); setOrgResults([]); if (showLockBtn) setRefUnlocked(false); } }}
                autoFocus
                className="w-full db-font-sm px-2 py-0.5 rounded"
                style={{ border: `1px solid var(--db-${showLockBtn ? 'accent' : 'border'}, #cbd5e1)`, background: 'var(--db-surface-alt, #fff)', color: 'var(--db-text, #1e293b)', maxWidth: 220 }}
              />
              {orgSearching && <span className="absolute right-2 top-0.5 db-font-xs text-slate-400">...</span>}
              {orgResults.length > 0 && (
                <div className="absolute top-7 left-0 w-80 rounded shadow-lg z-50 max-h-48 overflow-y-auto"
                  style={{ background: 'var(--db-surface, #fff)', border: '1px solid var(--db-border)' }}>
                  {orgResults.map((c: any) => (
                    <button key={c.id} type="button"
                      className="w-full text-left px-3 py-1.5 db-font-sm last:border-0"
                      style={{ borderBottom: '1px solid var(--db-border-light)' }}
                      onMouseEnter={(e) => (e.currentTarget.style.background = 'var(--db-row-hover, #f1f5f9)')}
                      onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}
                      onClick={() => handleOrgSelect(c)}
                    >
                      <span className="font-medium" style={{ color: 'var(--db-text)' }}>
                        {c.display_name || c.company || c.ida || `#${c.id}`}
                      </span>
                      <span className="font-mono db-font-xs ml-2" style={{ color: 'var(--db-text-dim)' }}>#{c.id}</span>
                    </button>
                  ))}
                </div>
              )}
              {orgQuery.length >= 2 && !orgSearching && orgResults.length === 0 && (
                <div className="absolute top-7 left-0 w-48 rounded shadow-lg z-50 px-3 py-1.5 db-font-sm"
            ...
            {showLockBtn && (
              <>
                <button type="button" className="db-font-xs px-1 text-slate-400 hover:text-blue-500"
                  title="Lock (keep current)" onClick={() => { setRefUnlocked(false); setOrgQuery(''); setOrgResults([]); }}>🔓</button>
                <button type="button" className="db-font-xs px-1 text-slate-400 hover:text-red-500"
                  title="Clear" onClick={() => { onChange(field, null); setRefUnlocked(false); setOrgQuery(''); setOrgResults([]); }}>✕</button>
              </>
            )}
          </div>
        );

        if (isRecordRef && !refUnlocked) {
          return (
            <div className="flex-1 flex items-center gap-1 min-w-0">
              <span className="db-font-sm truncate" style={{ color: 'var(--db-text, #1e293b)', maxWidth: 200 }}>
                {(val as any)?.name || (val as any)?.ida || '—'}
              </span>
              {isEditing && (
                <button type="button" className="db-font-xs px-1 text-slate-400 hover:text-red-500"
                  title="Unlock to change" onClick={() => setRefUnlocked(true)}>🔒</button>
              )}
            </div>
          );
        }
        if (refUnlocked) return searchInput(true);
        if (isAffiliation && !val && isEditing) return searchInput(false);
        return null;
      })() ||
      ...
      (fieldType && WIDGETS[fieldType] ? (() => {
        ...
        // Convert string[] options to WidgetOption[] for SelectWidget
        const widgetOptions = options?.map((o: string) => {
          const [v, lbl] = o.split('|');
          return { value: v, label: lbl || v };
        });
        // For readonly widgets, pass formatted display value (dates, phones, etc.)
        const widgetValue = fieldType === 'readonly' ? displayVal : val;
        return (
        ...
        <span className="flex-1 db-font-sm min-w-0 truncate" style={{ color: 'var(--db-text, #1e293b)', maxWidth: 240 }} title={isDate && val ? formatDt(val, 'datetime') : pathDisplay}>{typeof displayVal === 'string' ? displayVal.split('|')[0] : displayVal}</span>
      ))}
    </div>
  );
};
