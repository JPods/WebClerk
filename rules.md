...
2. Setup automatic testing and reporting funcions.
...
12. ONLY use uuid for communicating between databases with sycns records. Examples, product catalog updates, security issues, default changes, etc... 
13. Implement rate limiting DRF throttling classes for all endpoints to prevent abuse.
14. All API requests and errors must be logged. Integrate with monitoring tools for proactive alerts.

Related Data in jsons
1. QQQ fix our standard of sending related data, choose and document either data.related or data.data for consistency (recommend data.related).
...
Version Control
1. Automate verion mismatches by comparing expected schema with incoming data. See and add to current settings purpose view_edit for example.
...
1. Structure of multiple langauges and localization.
...
5. All endpoints must be documented in and kept up to dateOpenAPI/Swagger. Use tools like drf-yasg or drf-spectacular to auto-generate Swagger/OpenAPI docs. Use syncs records to distribute updates to all users.
