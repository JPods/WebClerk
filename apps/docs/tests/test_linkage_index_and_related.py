...
def test_related_actions_endpoint_lists_action_for_linkage(client, django_user_model):
    ...
    data = res.json()
    # Responses are wrapped in a standard envelope: { code, data: { count, results, ... }, ... }
    assert 'data' in data and 'results' in data['data']
    ids = [row['id'] for row in data['data']['results']]
