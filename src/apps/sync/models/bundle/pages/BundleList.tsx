...
export default function BundleList() {
  ...
  const getBundleData = useCallback(async () => {
    try {
      setLoading(true);
      ...
      if (res.status === 200) {
        ...
        dispatch(
          showToast({ message: "Failed to fetch bundles", type: "error" })
        );
      }
    } catch (error) {
      console.error("Failed to fetch bundles", error);
      dispatch(showToast({ message: "Failed to fetch bundles", type: "error" }));
    } finally {
      setLoading(false);
    }
  }, [dispatch]);
  ...
  const handleDelete = async (row: any) => {
    if (window.confirm(`Delete bundle ${row.name}?`)) {
      try {
        await deleteAction(row.id);
        dispatch(showToast({ message: "Bundle deleted successfully", type: "success" }));
        getBundleData(); // Refresh data
      } catch (error) {
        dispatch(showToast({ message: "Failed to delete bundle", type: "error" }));
      }
    }
  };
  ...
  const columnCtx = useColumnContextMenu("bundle_list", userColumns);
  return (
    <>
      <ButtonToolbar
        pageTitle="Bundle List"
        title="Bundle"
        modelKey="bundle"
