...
# ── Generate instance UUID if missing ──────────────────────────
if [ -f "$WC3_DIR/.env" ] && ! grep -q "WC_INSTANCE_UUID" "$WC3_DIR/.env" 2>/dev/null; then
    INSTANCE_UUID=$("$PY" -c "import uuid; print(uuid.uuid4())")
    echo "" >> "$WC3_DIR/.env"
    echo "# Unique identity for this WebClerk instance — generated once, never change" >> "$WC3_DIR/.env"
    echo "WC_INSTANCE_UUID=$INSTANCE_UUID" >> "$WC3_DIR/.env"
    echo "[webclerk] Generated instance UUID: $INSTANCE_UUID"
fi
