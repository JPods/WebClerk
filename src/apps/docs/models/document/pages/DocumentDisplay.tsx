...
export default function DocumentDisplay({
  ...
  useEffect(() => {
    const fetchFields = async () => {
      try {
        const rec = await getRecord('document', 1);
        const record = rec.record || rec;
        setFields(Object.keys(record));
      } catch (error) {
        console.error("Failed to fetch fields", error);
        setFields(['id', 'title', 'type', 'status', 'content']); // fallback
      }
    };
    fetchFields();
  }, []);
  ...
  return (
    <div className="p-4 space-y-4">
      <h2 className="text-xl font-semibold">
        {modeProp === "add" ? "Add Document" : modeProp === "edit" ? "Edit Document" : "View Document"}
      </h2>
