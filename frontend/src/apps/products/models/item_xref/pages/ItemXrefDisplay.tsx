...
/**
 * ItemXrefDisplay - Follows 3-column standard with tab navigation
...
interface ItemXrefDisplayProps {
  inline?: boolean;
...
const STORAGE_KEY = "itemXrefDisplay_columnCount";

export default function ItemXrefDisplay({
  inline = false,
  modeProp,
  dataProp,
  onSaved,
  onCancelInline,
}: ItemXrefDisplayProps) {
  ...
  // Tab navigation
  const { activeTab, setActiveTab } = useDetailTabs("item_xref_display", "actions", [
  ...
  useEffect(() => {
    if (modeProp === "edit" && dataProp?.id) {
      const fetchData = async () => {
        try {
          setLoading(true);
          const rec = await getRecord('item_xref', dataProp.id);
  ...
  const handleSave = async () => {
    try {
      setIsSaving(true);
      await saveRecord('item_xref', data);
      dispatch(showToast({ message: "Item Xref saved successfully", type: "success" }));
      onSaved?.();
      if (currentMode === "add") {
        onCancelInline?.();
      } else {
      ...
      dispatch(showToast({ message: "Failed to save item xref", type: "error" }));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <>
      ...
      <SimpleDetailHeader
        ...
        recordId={data?.id}
        recordName={`${data?.item_id_1 || ""} ↔ ${data?.item_id_2 || ""}`}
        mode={currentMode}
        backUrl=""
      ...
      {!loading && (
        <ComponentCard>
          ...
          <div className={getGridClassName(columnCount)}>
            <HorizontalField label="Item ID 1" htmlFor="item_id_1" icon={<Package size={14} />}>
              <Input
                type="text"
                id="item_id_1"
                placeholder="Item ID 1"
                value={data?.item_id_1 || ""}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleFieldChange("item_id_1", e.target.value)}
                disabled={currentMode === "view"}
              />
            </HorizontalField>
            <HorizontalField label="Item ID 2" htmlFor="item_id_2" icon={<Link2 size={14} />}>
              <Input
                type="text"
                id="item_id_2"
                placeholder="Item ID 2"
                value={data?.item_id_2 || ""}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleFieldChange("item_id_2", e.target.value)}
                disabled={currentMode === "view"}
              />
            </HorizontalField>
            <HorizontalField label="Relationship" htmlFor="relationship_type" icon={<GitBranch size={14} />}>
              <Input
                type="text"
                id="relationship_type"
                placeholder="Relationship Type"
                value={data?.relationship_type || ""}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleFieldChange("relationship_type", e.target.value)}
                disabled={currentMode === "view"}
              />
            </HorizontalField>
          </div>
