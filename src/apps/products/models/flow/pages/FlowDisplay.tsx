/**
 * FlowDisplay - Follows 3-column standard with tab navigation
...
const STORAGE_KEY = "flowDisplay_columnCount";
...
export default function FlowDisplay({
  ...
  // Tab navigation
  const { activeTab, setActiveTab } = useDetailTabs("flow_display", "actions", [
  ...
  useEffect(() => {
    if (modeProp === "edit" && dataProp?.id) {
      const fetchData = async () => {
        try {
          setLoading(true);
          const rec = await getRecord('flow', dataProp.id);
  ...
  const handleSave = async () => {
    try {
      setIsSaving(true);
      await saveRecord('flow', data);
      dispatch(showToast({ message: "Flow saved successfully", type: "success" }));
      onSaved?.();
      if (currentMode === "add") {
        onCancelInline?.();
      } else {
      ...
      dispatch(showToast({ message: "Failed to save flow", type: "error" }));
    } finally {
      setIsSaving(false);
    }
  };
      ...
      {!loading && (
        <ComponentCard>
          ...
          <div className={getGridClassName(columnCount)}>
            <HorizontalField label="Name" htmlFor="name" icon={<GitBranch size={14} />}>
              <Input
                type="text"
                id="name"
            ...
            <HorizontalField label="Steps" htmlFor="steps" icon={<ListOrdered size={14} />}>
              <Input
                type="text"
                id="steps"
                placeholder="Steps (JSON)"
                value={data?.steps || ""}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleFieldChange("steps", e.target.value)}
                disabled={currentMode === "view"}
              />
            </HorizontalField>
          </div>
