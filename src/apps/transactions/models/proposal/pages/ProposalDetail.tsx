    ...
    try {
      const res =
        mode === "add"
          ...
          : await updateProposal(data && data.id, formData);
      if (res) {
        dispatch(
          showToast({
            message: `Proposal ${
    ...
    <>
      ...
      <ComponentCard>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div>
            <Label htmlFor="proposal_no">Proposal Number</Label>
            <Input
              type="text"
              id="proposal_no"
              placeholder="Proposal Number"
              {...register("proposal_no")}
              error={errors.proposal_no && errors.proposal_no.message ? true : false}
              hint={errors.proposal_no && errors.proposal_no.message}
              disabled={mode === "view"}
            />
          </div>
          {mode === "view" && data && (
