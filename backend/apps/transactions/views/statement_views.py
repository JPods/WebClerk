...
Only promoted business lines become Cash records in psql.
...
Endpoints:
    ...
    POST /api/transactions/statements/promote/    — promote business lines to Cash records
...
def _get_harvester():
    ...
    return {
        ...
        'promote_to_cash_entries': promote_to_cash_entries,
    }
...
def promote_statements(request):
    """Promote business lines from JSON to Cash records in psql."""
    if not request.user.is_staff:
        return JsonResponse({'error': 'Staff access required'}, status=403)
    try:
        body = json.loads(request.body)
    ...
    h = _get_harvester()
    result = h['promote_to_cash_entries'](str(safe_path))

    return JsonResponse({
        'created': result['created'],
        'skipped': result['skipped'],
        'already_promoted': result['already_promoted'],
        'message': f"Promoted {result['created']} lines to Cash records.",
    })
