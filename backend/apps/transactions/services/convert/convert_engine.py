...
from .convert_utils import build_line_payload, select_lines
...
def _clone_quantity(src_qty: Dict) -> Dict:
    """Clone mode: a standalone line (Bill, 2026-09-17) — it has no parent, so it copies
    the source's live quantity and takes its own creation snapshot (D2: standalone
    staged = active at creation). To make another child, start from the parent."""
    active = src_qty.get("active", src_qty.get("staged", 0))
    return {
        "staged":     active,
        "active":     active,
        "remaining":  active,
...
def _convert_quantity(src_qty: Dict) -> Dict:
    ...
    # Nothing left to transfer. A negative remaining is a real quantity (returns,
    # credits): quantities may always go negative and the user is in control.
    if remaining == 0:
    ...
    if not src_qty.get("is_blanket", False) or increment == 0 or abs(increment) >= abs(remaining):
        ...
        transfer_qty = abs(increment) * (1 if remaining > 0 else -1)
...
def _cross_quantity(src_qty: Dict) -> Dict:
    """Cross-type mode (order -> purchase, etc.): history, not parent-child. The new line
    is standalone: it copies the source's live quantity and snapshots its own staged."""
    active = src_qty.get("active", src_qty.get("staged", 0))
    return {
        "staged":     active,
        "active":     active,
...
def _build_line_kwargs(
    ...
    kwargs: Dict[str, Any] = {
        ...
        # A clone is standalone: it has no parent, so its origin is history under
        # 'cloned_from', never 'source' (parent_line_id is derived from refs.source).
        "refs":        {
            ("cloned_from" if mode == "clone" else "source"): {
...
def execute_transfer(
    *,
    ...
    for src_line in selected_lines:
        ...
        # Source line remaining/status: recomputed when target_line saved
        # (services/line_parent.py). Not decremented here.
