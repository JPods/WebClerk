...
LEDGER_CHOICES = [
    ('post', 'Post'),        # create GL entries when promoted to Cash
...
class StatementLine(BaseModel):
    ...
    Staging table for statement import → categorize → promote to Cash.
    ...
    category = models.CharField(max_length=100, blank=True,
        help_text="Expense category — same select list as Cash.category")
    ...
    promoted = models.BooleanField(default=False, db_index=True,
        help_text="True = already created a Cash record from this line")
    cash_id = models.BigIntegerField(null=True, blank=True,
        help_text="Cash record ID created from this line")
