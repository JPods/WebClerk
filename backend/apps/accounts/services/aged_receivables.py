...
def get_company_receivables_config() -> dict:
    """company profile config.receivables — finance charge rate, grace periods,
    dunning headings. Raises when the section is missing: a report must not
    quietly print a 0% finance charge and blank headings."""
    from apps.accounts.services.finance_charges import receivables_config
...
def customer_statement(customer_id: int, as_of_date: Optional[date] = None) -> dict:
    ...
    report['response_area'] = [
        ...
        'Cash is due to be mailed on ____________.',
        'Other:',
    ]
