...
export default function UserDropdown() {
  ...
  const logoutData = async () => {
    try {
      ...
      // ignore logout errors so user can still sign out
    }
    clearTokens();
    ...
    <div className="relative">
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-11 w-11">
          <img src={user?.image_url || "/images/no-image.svg"} alt="User" />
        </span>
      ...
      >
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            type="button"
            onClick={logoutData}
            className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-gray-200 text-gray-600 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800"
            title="Sign out"
            aria-label="Sign out"
          >
