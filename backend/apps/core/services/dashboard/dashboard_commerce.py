...
Five actions matching the CommerceDashboard.tsx tabs:
  ...
  get_accounting_dashboard  — AR/AP aging, cash summary
"""
import logging
...
def get_sales_dashboard(params):
    ...
    for line in InvoiceLine.objects.filter(lines_q).iterator(chunk_size=200):
        ...
        line_totals = line.totals if isinstance(line.totals, dict) else {}
        sell = line_totals.get('amount') or 0
        cost = line_totals.get('cost') or 0
        tax = line_totals.get('tax') or 0
...
def get_accounting_dashboard(params):
    ...
    # Cash in period (Cash.amount is a real field, not a shadow)
    cash_entries = Cash.objects.filter(dt_created__gte=cutoff).aggregate(
    ...
    return {
        ...
        'cash_entries': {
            'count': cash_entries['count'] or 0,
            'total': float(cash_entries['total'] or 0),
        },
    }
