...
/**
 * VariantDisplay - Follows 3-column standard with tab navigation
...
const STORAGE_KEY = "variantDisplay_columnCount";
...
export default function VariantDisplay({
  ...
  // Tab navigation
  const { activeTab, setActiveTab } = useDetailTabs("variant_display", "actions", [
  ...
  useEffect(() => {
    if (modeProp === "edit" && dataProp?.id) {
      const fetchData = async () => {
        try {
          setLoading(true);
          const rec = await getRecord('variant', dataProp.id);
  ...
  const handleSave = async () => {
    try {
      setIsSaving(true);
      await saveRecord('variant', data);
      dispatch(showToast({ message: "Variant saved successfully", type: "success" }));
      onSaved?.();
      if (currentMode === "add") {
        onCancelInline?.();
      } else {
      ...
      dispatch(showToast({ message: "Failed to save variant", type: "error" }));
    } finally {
      setIsSaving(false);
    }
  };
      ...
      {!loading && (
        <ComponentCard>
          ...
          <div className={getGridClassName(columnCount)}>
            <HorizontalField label="Name" htmlFor="name" icon={<Palette size={14} />}>
              <Input
                type="text"
                id="name"
            ...
            <HorizontalField label="Attributes" htmlFor="attributes" icon={<Settings2 size={14} />}>
              <Input
                type="text"
                id="attributes"
                placeholder="Attributes (JSON)"
                value={data?.attributes || ""}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleFieldChange("attributes", e.target.value)}
                disabled={currentMode === "view"}
              />
            </HorizontalField>
          </div>
