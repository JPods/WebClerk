#!/usr/bin/env python
"""
Backfill Item.quantity.on_qt from existing active quotes.
...
print("=== Backfilling Item.quantity.on_qt from active quotes ===")
print()
...
for quote in active_quotes:
    ...
    for line in quote.lines.all():
        ...
        # Calculate on_qt contribution (qty * probability)
        ...
        print(f"  Quote #{quote.pk}, Line {line.pk}: item_id={item_id}, qty={qty_staged}, prob={probability:.0%} -> on_qt +{on_p_contribution}")
...
for item_id, total_on_p in item_qty_map.items():
    try:
        ...
        old_on_p = quantity.get('on_qt', 0)
        quantity['on_qt'] = float(total_on_p)
        
        # Use .update() to avoid triggering full save hooks
        Item.objects.filter(pk=item_id).update(quantity=quantity)
        
        print(f"  Item {item_id} ({item.name[:30]}): on_qt {old_on_p} -> {float(total_on_p)}")
        updated_count += 1
