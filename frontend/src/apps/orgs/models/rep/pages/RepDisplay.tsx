...
export const REP_STANDARD_TABS = ["actions", "comments", "documents", "raw"];

export const REP_ADDITIONAL_TABS = [
  { id: "contacts", label: "Contacts", icon: <FaAddressCard size={14} /> },
  { id: "qa", label: "Q&A", icon: <FaQuestionCircle size={14} /> },
];

export const buildRepTabBadges = (
  data: any,
...
function RepDetail({
  ...
  const routeMode = useMemo(() => {
    ...
    if (path.includes("/rep/add")) return "add";
    if (path.includes("/rep/edit/")) return "edit";
    if (path.includes("/rep/detail/")) return "view";
    return "view";
  }, [location.pathname]);
  ...
  const tabBadges = useMemo(
    () => buildRepTabBadges(data, fkContacts),
    [data, fkContacts],
  );
  ...
  }, [repId, dataProp, routeMode, modeProp]);
  ...
  const fetchFkContacts = useCallback(async (rId: number) => {
    if (!Number.isFinite(rId) || rId <= 0) return;
    setFkContactsLoading(true);
    try {
      const res = await getRecords("contact", { rep: rId });
      ...
      console.error("[RepDetail] FK contact fetch failed:", err);
    } finally {
  ...
  useEffect(() => {
    if (modeProp === "add" || routeMode === "add") return;
    fetchFkContacts(resolvedRepId);
  }, [resolvedRepId, routeMode, fetchFkContacts, fkRefreshTrigger, modeProp]);
    ...
    if (data) {
      Object.keys(data).forEach((key) => {
        if (key in JSON_DEFAULTS) {
          ...
          setValue(key as keyof RepFormValues, data[key]);
        }
      });
    } else {
      reset({ is_active: false, version: 1, org_type: "rep" });
    }
  }, [data, mode, reset, setValue]);

  // ── Save ──────────────────────────────────────────────────────────────────

  const onSubmit = async (fd: RepFormValues) => {
    try {
      ...
      if (mode === "add") {
        await createRep(payload);
        dispatch(showToast({ message: "Rep created", type: "success" }));
        if (onSaved) onSaved();
        if (!inline) navigate(PageRoutes.repList);
      } else {
        await updateRep({ ...payload, id: data?.id });
        const refreshed = await getRecord("rep", data.id);
        const next = (refreshed as any)?.record ?? refreshed;
        if (next) setFetchedRecord(next as any);
        dispatch(showToast({ message: "Rep saved", type: "success" }));
        if (onSaved) onSaved();
        setIsEditing(false);
      }
    } catch (err: any) {
  ...
  const handleSetPrimary = useCallback(
    async (contact: {
      ...
      try {
        ...
        setValue("contact_id" as keyof RepFormValues, contact.contact_id);
        if (contact.attention)
          setValue(
            "attention" as keyof RepFormValues,
            contact.attention as any,
          );
        if (emailStr)
          setValue("email" as keyof RepFormValues, emailStr as any);
        if (phoneStr)
          setValue("phone" as keyof RepFormValues, phoneStr as any);
        const res = await getRecord("rep", recId);
        ...
        console.error("[RepDetail.handleSetPrimary] failed:", err);
        dispatch(
          showToast({
  ...
  const repData = useMemo(() => {
    ...
    return {
      ...
      org_type: formData.org_type ?? data?.org_type ?? "rep",
    ...
    <>
      ...
      <div className="h-full flex flex-col bg-white dark:bg-slate-900">
          ...
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100 truncate">
                <DevBadge label="RepDetail" className="mr-2" />
                {repData.display_name || "New Rep"}
              </h2>
              <div className="flex items-center gap-3 mt-1">
                ...
                >
                  {repData.is_active ? "Active" : "Inactive"}
                </span>
                <span className="text-sm text-slate-500 dark:text-slate-400">
                  {repData.org_type || "rep"} • v{repData.version ?? 1}
                </span>
        ...
        <div className="shrink-0">
          <BasicInformationPanel data={repData} columns={columnCount} />
        </div>
          ...
          <div className="space-y-2 px-4 py-2">
            <CoreTabPanel
              ...
              onSaveActions={async (actions) => {
                if (!data?.id) return;
                await updateRep({ id: data.id, actions } as any);
              }}
              ...
              orgDisplayName={repData.display_name}
