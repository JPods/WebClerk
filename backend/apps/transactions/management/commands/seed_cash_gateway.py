"""Seed the cash gateway Setting record.

Creates a Setting with purpose='wc:cash_gateway' that holds:
...
Usage: ./manage.py seed_cash_gateway
"""
...
class Command(BaseCommand):
    help = "Seed the cash gateway Setting record"

    def handle(self, *args, **options):
        setting, created = Setting.objects.get_or_create(
            purpose='wc:cash_gateway',
            parent_model='setting',
            defaults={
                'ida': 'cash-gateway',
                'config': CASH_GATEWAY_CONFIG,
            },
        )

        if created:
            self.stdout.write(self.style.SUCCESS(f"Created cash_gateway Setting #{setting.id}"))
            self.stdout.write("Seeded gateways:")
            for gw in CASH_GATEWAY_CONFIG['gateway']:
            ...
            for key, val in CASH_GATEWAY_CONFIG.items():
                if key == 'gateway':
            ...
            self.stdout.write(self.style.SUCCESS(f"Updated cash_gateway Setting #{setting.id} (merged new keys)"))
