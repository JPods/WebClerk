...
class Pending(CoreModel):
    ...
    def try_apply(self):
        ...
        # ── Payment application (AR — Invoice) ───────────────────
        if self.purpose == 'payment_application':
            return self._apply_payment()

        # ── Payment application (AP — Receipt) ───────────────────
        if self.purpose == 'payment_application_receipt':
            return self._apply_receipt_payment()
    ...
    def _apply_payment(self):
        """Apply payment to invoice (AR). Delegates to payment_pending service."""
        try:
    ...
    def _apply_receipt_payment(self):
        """Apply payment to receipt (AP). Delegates to payment_pending_receipt service."""
        try:
            from apps.transactions.services.payment.payment_pending_receipt import apply_receipt_payment_pending
            return apply_receipt_payment_pending(self)
        except Exception:
            logger.debug("Pending %s: receipt payment apply failed, queued for celery", self.pk, exc_info=True)
            return False
