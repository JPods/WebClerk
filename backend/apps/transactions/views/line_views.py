...
from apps.transactions.serializers.quote_serializer import QuoteSerializer, QuoteLineSerializer
...
class QuoteListCreate(EnvelopeResponseMixin, ListResponseEnvelopeMixin, generics.ListCreateAPIView):
    queryset = Quote.objects.active().order_by('-id')
    serializer_class = QuoteSerializer
    permission_classes = [BasePermission]
    pagination_class = DefaultPagination

class QuoteRetrieveUpdate(EnvelopeResponseMixin, generics.RetrieveUpdateDestroyAPIView):
    queryset = Quote.objects.active()
    serializer_class = QuoteSerializer
    permission_classes = [BasePermission]

@extend_schema(
    summary="List/Create quote lines",
    parameters=[OpenApiParameter(name='parent_id', description='Filter by parent_id', required=False, type=int)],
)
class QuoteLineListCreate(EnvelopeResponseMixin, ListResponseEnvelopeMixin, generics.ListCreateAPIView):
    queryset = QuoteLine.objects.active().order_by('-id')
    serializer_class = QuoteLineSerializer
    permission_classes = [BasePermission]
    throttle_scope = 'tx_line'
    filterset_fields = ['quote', 'status']
    search_fields = ['item__description', 'item__uuid_item']
    ordering_fields = ['id', 'quote', 'status']
    pagination_class = DefaultPagination
    ...
    def perform_create(self, serializer):
        ...
        service._create_pending_for_new_line(
            parent=instance.quote,
            parent_model_key='quote',
            line=instance,
            line_data=serializer.validated_data,
        )


class QuoteLineRetrieveUpdate(EnvelopeResponseMixin, generics.RetrieveUpdateDestroyAPIView):
    queryset = QuoteLine.objects.active()
    serializer_class = QuoteLineSerializer
    permission_classes = [BasePermission]
    ...
    def perform_update(self, serializer):
        ...
        old_qty = float((old_instance.quantity or {}).get('active', 0) or 0)
        old_item_id = (old_instance.item or {}).get('item_id') or (old_instance.item or {}).get('id')
        
        instance = serializer.save()
        
        new_qty = float((instance.quantity or {}).get('active', 0) or 0)
    ...
    def perform_destroy(self, instance):
        ...
        qty_to_release = float((instance.quantity or {}).get('active', 0) or 0)
...
class PurchaseLineRetrieveUpdate(EnvelopeResponseMixin, generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        ...
        if isinstance(old_instance.quantity, dict):
            old_qty = float(old_instance.quantity.get('active', 0) or 0)
        old_item_id = None
        ...
        if isinstance(instance.quantity, dict):
            new_qty = float(instance.quantity.get('active', 0) or 0)
        new_item_id = None
    ...
    def perform_destroy(self, instance):
        ...
        qty_to_release = 0
        if isinstance(instance.quantity, dict):
            qty_to_release = float(instance.quantity.get('active', 0) or 0)
...
@extend_schema(
    ...
    OpenApiParameter(name='model', description='Optional line model code to scope aggregation (e.g., quote-line)', required=False, type=str,
              enum=['quote-line','order-line','invoice-line','purchase-order-line','workorder-line','requisition-line']),
...
@extend_schema(
    ...
    parameters=[OpenApiParameter(
      ...
      enum=['quote-line','order-line','invoice-line','purchase-order-line','workorder-line','requisition-line',
          'quote','sales-order','invoice','purchase-order','workorder','requisition']
    )],
