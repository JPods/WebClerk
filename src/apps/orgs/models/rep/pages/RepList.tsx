...
import RepListMob from "./RepListMob";
export default function RepList() {
  ...
  const getLocationData = useCallback(async (repId?: number) => {
    setLoading(true);
    try {
      ...
      if (repId) {
        const contactRes = await getRecord("rep", repId);
        setSelectedRep(contactRes.record);
  ...
  const handleEdit = async (row: dynamicData) => {
    ...
    console.log("res.", res);
    if (res.status === 200) setSelectedRep(res.data.data.record);
    else setSelectedRep(row);
    setFormMode("edit");
    console.log("res", res);
  };
  ...
  const handleDelete = async (row: dynamicData) => {
    if (window.confirm(`Delete Rep ${row.name}?`)) {
      try {
