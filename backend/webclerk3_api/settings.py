...
# DEBUG hostname guard — refuse to start with DEBUG=True on a production hostname.
# Prevents CF_ACCESS_DEV_EMAIL backdoor from being possible in production.
# Security policy §3: "make the backdoor impossible, not just unlikely"
if DEBUG:
    import socket as _sock
    _hostname = _sock.gethostname().lower()
    _PROD_INDICATORS = ('.webclerk.com', 'andi', 'prod', 'staging')
    if any(ind in _hostname for ind in _PROD_INDICATORS):
        raise RuntimeError(
            f'DEBUG=True on production hostname "{_hostname}" — '
            f'refusing to start. Set DEBUG=False in .env'
        )

ALLOWED_HOSTS = ['localhost', '127.0.0.1', '.webclerk.com']
...
# NOTE: do not set MIGRATION_MODULES to None for auth/contenttypes/admin/sessions.
# It was done here under test to skip contrib migrations for speed, and it made
# the test suite unrunnable: `MIGRATION_MODULES = {'auth': None}` tells Django
# the auth app has no migrations at all, while core/0002_initial depends on
# ('auth', '0012_alter_user_first_name_max_length') because Contact carries the
# M2M to auth.group. Building the graph then failed with
#   NodeNotFoundError: core.0002_initial dependencies reference nonexistent
#   parent node ('auth', '0012_alter_user_first_name_max_length')
# before a single test ran. Those dependencies are real; the contrib migrations
# have to run. A slower test database beats a suite nobody can execute.
...
MIDDLEWARE = [
    ...
    'common.middleware.ActorMiddleware',                         # request.actor, lazy: built after DRF auth
    'common.middleware.CloudflareAccessMiddleware',             # CF email header → auto-login
    'django.contrib.messages.middleware.MessageMiddleware',
    'common.middleware.HttpsEnforcementMiddleware',             # HTTPS-only on wcapi/sync
    'common.middleware.WriteGateMiddleware',
    'common.middleware.SecretGuardMiddleware',                  # refuse passwords typed into shared records
    'common.middleware.AthenaValidationMiddleware',
    'common.middleware.RequestLogMiddleware',
    'common.middleware.SecurityAlertMiddleware',                  # auth failures, empty results, unsigned saves
...
REST_FRAMEWORK = {
    ...
    "DEFAULT_THROTTLE_RATES": {
        "user": "1000/minute",
        "anon": "60/minute",
        "auth": "10/minute",         # auth endpoints — tighter limit for brute force prevention
        "cash": "10/minute",
        "webhook": "30/minute",
        "tx_line": "60/minute",
...
# form_url is on this server (webclerk.com). These stay in code: they are the security
# boundary. Who answers and what the form asks are data — each site's Report
# (category='form', config.inquiry.site=<key>), edited in WebClerk.
INQUIRY_SITES = {
...
WCAPI_BLESSED_MODELS = {
    ...
    "cash_entry": "transactions.Cash",    # alias
...
SECURE_REDIRECT_EXEMPT = [r'^wcapi/ai/device-status/$', r'^wcapi/health/$', r'^wcapi/_settings_health/$']
...
try:
    ...
    # WC_TEST_DB_NAME lets parallel worktrees run tests without sharing one test DB.
    DATABASES["default"]["TEST"]["NAME"] = _os.environ.get('WC_TEST_DB_NAME', 'test_commerce_expert_new')
    # Every other alias (webserving, conversion) gets its own name from the same var,
    # otherwise parallel runs still collide on e.g. test_commerce_webserving.
    if _os.environ.get('WC_TEST_DB_NAME'):
        for _alias, _db in DATABASES.items():
            if _alias != 'default':
                _db.setdefault("TEST", {})["NAME"] = f"{_os.environ['WC_TEST_DB_NAME']}_{_alias}"
except Exception:
    pass

# Cash Gateway Settings
...
# Cash Processing Settings
CASH_CURRENCY = config('CASH_CURRENCY', default='USD')
CASH_SUCCESS_URL = config('CASH_SUCCESS_URL', default='http://localhost:5173/cash/success')
CASH_CANCEL_URL = config('CASH_CANCEL_URL', default='http://localhost:5173/cash/cancel')
CASH_WEBHOOK_URL = config('CASH_WEBHOOK_URL', default='http://localhost:8000/api/cash/webhook/')
...
CELERY_BEAT_SCHEDULE = {
    ...
    # GL balance, invoice balance, unapplied cash, aging
