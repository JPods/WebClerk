/**
 * OrgItemDisplay - Follows 3-column standard with tab navigation
...
const STORAGE_KEY = "orgItemDisplay_columnCount";

export default function OrgItemDisplay({
  inline = false,
  modeProp,
  dataProp,
  onSaved,
  ...
  // Tab navigation
  const { activeTab, setActiveTab } = useDetailTabs("org_item_display", "actions", [
  ...
  useEffect(() => {
    if (modeProp === "edit" && dataProp?.id) {
      const fetchData = async () => {
        try {
          setLoading(true);
          const rec = await getRecord('org_item', dataProp.id);
  ...
  const handleSave = async () => {
    try {
      setIsSaving(true);
      await saveRecord('org_item', data);
      dispatch(showToast({ message: "Org Item saved successfully", type: "success" }));
      onSaved?.();
      if (currentMode === "add") {
        onCancelInline?.();
      } else {
      ...
      dispatch(showToast({ message: "Failed to save org item", type: "error" }));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <>
      ...
      <SimpleDetailHeader
        entityName="Org Item"
        recordId={data?.id}
        recordName={`${data?.org_id || ""} - ${data?.item_id || ""}`}
        mode={currentMode}
        backUrl=""
      ...
      {!loading && (
        <ComponentCard>
          ...
          <div className={getGridClassName(columnCount)}>
            <HorizontalField label="Org ID" htmlFor="org_id" icon={<Building2 size={14} />}>
              <Input
                type="text"
                id="org_id"
                placeholder="Org ID"
                value={data?.org_id || ""}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleFieldChange("org_id", e.target.value)}
                disabled={currentMode === "view"}
              />
            </HorizontalField>
            <HorizontalField label="Item ID" htmlFor="item_id" icon={<Package size={14} />}>
              <Input
                type="text"
                id="item_id"
                placeholder="Item ID"
                value={data?.item_id || ""}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleFieldChange("item_id", e.target.value)}
                disabled={currentMode === "view"}
              />
            </HorizontalField>
            <HorizontalField label="Quantity" htmlFor="quantity" icon={<Hash size={14} />}>
              <Input
                type="number"
                id="quantity"
                placeholder="Quantity"
                value={data?.quantity ?? ""}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleFieldChange("quantity", Number(e.target.value))}
                disabled={currentMode === "view"}
              />
            </HorizontalField>
          </div>
