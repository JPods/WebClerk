...
class Command(BaseCommand):
    ...
    def _seed_cycle(self, cycle):
        ...
        # Update invoice totals with cash info
        ...
        # Cash GL: debit Cash, credit AR — one pair per cash entry
        for i, pay_spec in enumerate(cycle['cash_entries']):
            pay_ida, fraction, method, _ = pay_spec
            pay_amount = float((total * fraction).quantize(Decimal('0.01')))
            pay_obj = Cash.objects.filter(ida=pay_ida).first()
            pay_src_id = pay_obj.id if pay_obj else 0
            suffix = f'-P{i + 1}' if len(cycle['cash_entries']) > 1 else '-P'
