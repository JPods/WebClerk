    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6">
          ...
          <dl className="space-y-3 text-sm">
            <div className="flex justify-between items-center">
              <FieldLabel label="Proposal No" mandatory locked className="text-slate-500 dark:text-slate-400" />
              <dd className="font-mono font-medium text-slate-900 dark:text-white">{data.ida ?? data.proposal_no ?? '--'}</dd>
            </div>
            <div className="flex justify-between items-center">
            ...
            <div className="flex justify-between items-center">
              <FieldLabel label="Valid Until" className="text-slate-500 dark:text-slate-400" />
              {isEditing && onChange ? (
                <input
                  type="date"
                  value={data.valid_until ? new Date(data.valid_until).toISOString().split('T')[0] : ''}
          ...
          {customerInfo ? (
            <dl className="space-y-3 text-sm">
              <div className="flex justify-between">
                ...
                <dd className="text-slate-900 dark:text-white font-medium">{customerInfo.name ?? '--'}</dd>
              </div>
              {customerInfo.contact && (
                <div className="flex justify-between">
                  <FieldLabel label="Contact" className="text-slate-500 dark:text-slate-400" />
                  <dd className="text-slate-900 dark:text-white">{customerInfo.contact}</dd>
                </div>
              )}
              {customerInfo.email && (
                <div className="flex justify-between">
                  <FieldLabel label="Email" className="text-slate-500 dark:text-slate-400" />
                  <dd className="text-slate-900 dark:text-white">{customerInfo.email}</dd>
                </div>
              )}
              {customerInfo.phone && (
                <div className="flex justify-between">
                  <FieldLabel label="Phone" className="text-slate-500 dark:text-slate-400" />
                  <dd className="text-slate-900 dark:text-white">{customerInfo.phone}</dd>
                </div>
              )}
            </dl>
          ) : data.company ? (
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 italic">No customer assigned</p>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6">
          ...
          <dl className="space-y-3 text-sm">
            <div className="flex justify-between items-center">
              <FieldLabel label="Address 1" className="text-slate-500 dark:text-slate-400" />
              {isEditing && onChange ? (
                <input
                  type="text"
                  value={data.address1 ?? ''}
                ...
                <dd className="text-slate-900 dark:text-white">{data.address1 ?? '--'}</dd>
              )}
            </div>
            <div className="flex justify-between items-center">
              <FieldLabel label="Address 2" className="text-slate-500 dark:text-slate-400" />
              {isEditing && onChange ? (
                <input
                  type="text"
                  value={data.address2 ?? ''}
                ...
                <dd className="text-slate-900 dark:text-white">{data.address2 ?? '--'}</dd>
              )}
            </div>
            <div className="flex justify-between items-center">
              <FieldLabel label="City" className="text-slate-500 dark:text-slate-400" />
              {isEditing && onChange ? (
                <input
                  type="text"
                  value={data.city ?? ''}
                ...
                <dd className="text-slate-900 dark:text-white">{data.city ?? '--'}</dd>
              )}
            </div>
            <div className="flex justify-between items-center">
              <FieldLabel label="State" className="text-slate-500 dark:text-slate-400" />
              {isEditing && onChange ? (
                <input
                  type="text"
                  value={data.state ?? ''}
                ...
                <dd className="text-slate-900 dark:text-white">{data.state ?? '--'}</dd>
              )}
            </div>
            <div className="flex justify-between items-center">
              <FieldLabel label="ZIP" className="text-slate-500 dark:text-slate-400" />
              {isEditing && onChange ? (
                <input
                  type="text"
                  value={data.zip ?? ''}
                ...
                <dd className="text-slate-900 dark:text-white">{data.zip ?? '--'}</dd>
              )}
            </div>
          </dl>
        </div>
...
const ProposalDetail: React.FC<ProposalDetailProps> = (props) => {
  ...
  const handleConvertToOrder = useCallback(async (data: Proposal) => {
    if (!data?.id) return;
    try {
  ...
  const handleGeneratePdf = useCallback(async (data: Proposal) => {
    if (!data?.id) return;
    try {
