...
def _compute_aging_for_source(source: str, as_of: date) -> int:
    ...
    entries = (
        Ledger.objects
        # No dt_applied filter: nothing set it, so it matched everything. Open is what
        # the excludes below say — value_available (fixed 2026-09-20).
...
def _lock_period_transactions(year: int, month: int) -> int:
    ...
    for model_name in ("Invoice", "Cash", "Purchase"):
        try:
...
def run_eom_close(
    ...
    Returns:
        {period, gl_balanced, total_sales, total_purchases, total_cash_entries,
    ...
    try:
        ...
        eom_doc = Document(
            ...
            data={
                ...
                "total_cash_entries": summary["total_cash_entries"],
...
def get_eom_status(
    ...
    Returns:
        {closed, dt_closed, pending_invoices, pending_cash_entries, gl_balance}
    """
    if not (1 <= period_month <= 12):
        raise ValueError("period_month must be 1-12")
    ...
    return {
        ...
        "pending_cash_entries": pending_cash_entries,
