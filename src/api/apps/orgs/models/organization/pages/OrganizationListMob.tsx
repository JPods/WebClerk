...
interface OrganizationListMobProps {
  dataProp: dynamicData[];
  handleView: (row: dynamicData) => void;
  handleEdit: (row: dynamicData) => void;
}

export default function OrganizationListMob({
  dataProp,
  handleView,
  handleEdit,
}: OrganizationListMobProps) {
  const [openIndex, setOpenIndex] = useState<number | null>(null);
  return (
    <div className="flex-1 overflow-y-auto px-2">
      {dataProp && dataProp.length > 0 ? (
        dataProp.map((organization, index) => (
          <AccordionItem
            key={organization.id}
            title={`display_name: ${organization.display_name ?? "--"} (id: ${
              organization.id
            })`}
          ...
          >
            ...
            <div className="flex flex-col min-h-[220px]">
              ...
              <div className="space-y-1 text-sm">
                <p>
                  <strong>org_type:</strong> {organization.org_type || "--"}
                </p>
                <p>
                  <strong>status:</strong> {organization.status || "--"}
                </p>
                <p>
                  <strong>version:</strong> {organization.version || "--"}
                </p>
