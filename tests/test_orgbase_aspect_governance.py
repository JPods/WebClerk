...
def test_aspect_prune_contacts_limit():
    org = OrgBase.objects.create(org_type=OrgType.CUSTOMER, company='Prune Co', status='active')
    # Add over limit
...
def test_refresh_aspects_updates_metadata_counts():
    org = OrgBase.objects.create(org_type=OrgType.CUSTOMER, company='Meta Co', status='active')
